...
def analyze_param(
    *,
    ...
    if get_origin(use_annotation) is Annotated:
        ...
        if isinstance(fastapi_annotation, FieldInfo):
            ...
            assert (
                ...
                or field_info.default is RequiredParam
            ), (
    ...
    # Handle default assignations, neither field_info nor depends was not found in Annotated nor default value
        ...
        default_value = (
            value if value is not inspect.Signature.empty else RequiredParam
        )
        if is_path_param:
    ...
    return ParamDetails(
        type_annotation=type_annotation, depends=depends, field=field
    )
...
def _get_multidict_value(
    ...
    if is_sequence_field(field) and isinstance(
        values, (ImmutableMultiDict, Headers)
    ):
...
def _should_embed_body_fields(fields: List[ModelField]) -> bool:
    ...
    if isinstance(
        first_field.field_info, params.Form
...
async def request_body_to_args(
    ...
    if single_not_embedded_field and lenient_issubclass(
    ...
    if isinstance(received_body, FormData):
        body_to_process = await _extract_form_body(
            fields_to_extract, received_body
        )
