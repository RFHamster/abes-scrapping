...
def generate_operation_id(
    *, route: routing.APIRoute, method: str
    ...
    return generate_operation_id_for_path(
        name=route.name, path=path, method=method
    )
...
def get_openapi_path(
    *,
    ...
    route_response_media_type: Optional[
        str
    ] = current_response_class.media_type
    if route.include_in_schema:
        for method in route.methods:
            ...
            (
                security_definitions,
                operation_security,
            ) = get_openapi_security_definitions(flat_dependant=flat_dependant)
            ...
            if route.responses:
                ...
                for (
                    ...
                    if field:
                        ...
                        deep_dict_update(
                            additional_schema, additional_field_schema
                        )
...
def get_fields_from_routes(
    ...
    for route in routes:
        if getattr(route, 'include_in_schema', None) and isinstance(
            ...
            if route.callbacks:
                callback_flat_models.extend(
                    get_fields_from_routes(route.callbacks)
                )
...
def get_openapi(
    *,
    ...
    all_fields = get_fields_from_routes(
        list(routes or []) + list(webhooks or [])
    )
    ...
    for webhook in webhooks or []:
        if isinstance(webhook, routing.APIRoute):
            ...
            if result:
                path, security_schemes, path_definitions = result
                if path:
                    webhook_paths.setdefault(webhook.path_format, {}).update(
                        path
                    )
                if security_schemes:
                    components.setdefault('securitySchemes', {}).update(
    ...
    if definitions:
        components['schemas'] = {
            k: definitions[k] for k in sorted(definitions)
        }
    if components:
