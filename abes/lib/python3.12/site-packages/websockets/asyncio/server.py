                ...
                if process_response is not None:
                    try:
                        response = process_response(
                            self, self.request, self.response
                        )
                        if isinstance(response, Awaitable):
                            response = await response
                    except Exception as exc:
    ...
    async def conn_handler(self, connection: ServerConnection) -> None:
        ...
        try:
            ...
            try:
                ...
                connection.logger.error(
                    'connection handler failed', exc_info=True
                )
...
def basic_auth(
    ...
    check_credentials: Callable[[str, str], Awaitable[bool] | bool]
    | None = None,
    ...
    if credentials is not None:
        if is_credentials(credentials):
            ...
            credentials_list = list(
                cast(Iterable[Tuple[str, str]], credentials)
            )
    ...
    async def process_request(
        ...
        try:
            ...
            response.headers[
                'WWW-Authenticate'
            ] = build_www_authenticate_basic(realm)
            return response
