...
class ClientProtocol(Protocol):
    ...
    def process_response(self, response: Response) -> None:
        ...
        if not (len(upgrade) == 1 and upgrade[0].lower() == 'websocket'):
            raise InvalidUpgrade(
                'Upgrade', ', '.join(upgrade) if upgrade else None
            )
        ...
        try:
            ...
            raise InvalidHeader(
                'Sec-WebSocket-Accept', 'multiple values'
            ) from None
        ...
        if subprotocols:
            ...
            parsed_subprotocols: Sequence[Subprotocol] = sum(
                [
                    ...
                    for header_value in subprotocols
                ],
                [],
            )
        ...
        if self.state is CONNECTING:
            ...
            if self.debug:
                ...
                if response.body is not None:
                    self.logger.debug(
                        '< [body] (%d bytes)', len(response.body)
                    )
...
BACKOFF_INITIAL_DELAY = float(
    os.environ.get('WEBSOCKETS_BACKOFF_INITIAL_DELAY', '5')
)
BACKOFF_MIN_DELAY = float(
    os.environ.get('WEBSOCKETS_BACKOFF_MIN_DELAY', '3.1')
)
BACKOFF_MAX_DELAY = float(
    os.environ.get('WEBSOCKETS_BACKOFF_MAX_DELAY', '90.0')
)
