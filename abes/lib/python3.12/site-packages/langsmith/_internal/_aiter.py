...
def py_anext(
    ...
    try:
        __anext__ = cast(
            Callable[[AsyncIterator[T]], Awaitable[T]],
            type(iterator).__anext__,
        )
...
class Tee(Generic[T]):
    ...
    def __init__(
        ...
        *,
        ...
        self._iterator = (
            iterable.__aiter__()
        )  # before 3.10 aiter() doesn't exist
        ...
        async for item in generator:
            if accepts_context:
                ...
                task = asyncio.create_task(
                    process_item(ix, item), context=context
                )
            else:
...
def accepts_context(callable: Callable[..., Any]) -> bool:
    ...
    try:
        return (
            inspect.signature(callable).parameters.get('context') is not None
        )
    except ValueError:
        return False
