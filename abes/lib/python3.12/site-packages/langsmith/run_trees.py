...
class RunTree(ls_schemas.RunBase):
        ...
        **kwargs: Any,
        ...
        if (
            ...
            if (
                ...
                kwargs['tags'] = sorted(
                    set(run.tags or [] + kwargs.get('tags', []))
                )
                ...
                hasattr(tracer, 'order_map')
                and cb.parent_run_id in tracer.order_map
            ):
    ...
    def from_headers(
        cls,
        headers: Mapping[Union[str, bytes], Union[str, bytes]],
        **kwargs: Any,
    ) -> Optional[RunTree]:
        ...
        langsmith_trace = cast(
            Optional[str], headers.get(LANGSMITH_DOTTED_ORDER)
        )
