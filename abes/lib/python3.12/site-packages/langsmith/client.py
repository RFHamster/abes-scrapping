...
def _parse_token_or_url(
    ...
    if len(path_parts) >= num_parts:
        ...
        raise ls_utils.LangSmithUserError(
            f'Invalid public {kind} URL: {url_or_token}'
        )
...
def _default_retry_config() -> Retry:
    ...
    use_allowed_methods = tuple(map(int, urllib3_version.split('.'))) >= (
        1,
        26,
    )
        ...
        for attr, exclude_none in serialization_methods:
            if hasattr(obj, attr) and callable(getattr(obj, attr)):
                try:
                    ...
                    response = (
                        method(exclude_none=exclude_none)
                        if exclude_none
                        else method()
                    )
...
def _get_write_api_urls(
    _write_api_urls: Optional[Dict[str, str]]
) -> Dict[str, str]:
...
def _ensure_uuid(
    value: Optional[Union[str, uuid.UUID]], *, accept_null: bool = False
):
    if value is None:
        if accept_null:
            return None
...
class _LangSmithHttpAdapter(requests_adapters.HTTPAdapter):
    ...
    def __init__(
        ...
        max_retries: Union[
            Retry, int, None
        ] = requests_adapters.DEFAULT_RETRIES,
...
class Client:
    ...
    def __init__(
        ...
        *,
        ...
        self._write_api_urls: Mapping[
            str, Optional[str]
        ] = _get_write_api_urls(api_urls)
        ...
        if self._info is None:
            try:
                response = self.request_with_retries(
                    ...
                    timeout=(
                        self.timeout_ms[0] / 1000,
                        self.timeout_ms[1] / 1000,
                    ),
                )
    ...
    def request_with_retries(
        ...
        *,
        ...
        handle_response: Optional[
            Callable[[requests.Response, int], Any]
        ] = None,
        _context: str = '',
        **kwargs: Any,
    ) -> requests.Response:
                ...
                try:
                    ...
                    if response is not None:
                        if handle_response is not None:
                            if idx + 1 < stop_after_attempt:
                                should_continue = handle_response(
                                    response, idx + 1
                                )
                                if should_continue:
                                    continue
                    ...
                    api_key = (
                        e.request.headers.get('x-api-key') or ''
                        if e.request
                        else ''
                    )
    ...
    def _run_transform(
        ...
        attachments_collector: Optional[
            Dict[str, ls_schemas.Attachments]
        ] = None,
    ) -> dict:
        ...
        if 'outputs' in run_create and run_create['outputs'] is not None:
            ...
            run_create['outputs'] = self._hide_run_outputs(
                run_create['outputs']
            )
        if not update and not run_create.get('start_time'):
            run_create['start_time'] = datetime.datetime.now(
    ...
    def _insert_runtime_env(runs: Sequence[dict]) -> None:
        ...
        for run_create in runs:
            ...
            metadata.update(
                {
                    k: v
                    for k, v in langchain_metadata.items()
        ...
        **kwargs : Any
        ...
        if (
            ...
            return self.tracing_queue.put(
                TracingQueueItem(
                    run_create['dotted_order'], 'create', run_create
                )
            )
    ...
    def batch_ingest_runs(
        ...
        *,
        ...
        create_dicts = [
            self._run_transform(run) for run in create or EMPTY_SEQ
        ]
        update_dicts = [
            self._run_transform(run, update=True)
        ...
        for key in ['post', 'patch']:
            ...
            while body:
                if (
                    body_size > 0
                    and body_size + len(body[0]) > size_limit_bytes
                ):
    ...
    def _post_batch_ingest_runs(self, body: bytes, *, _context: str):
        for api_url, api_key in self._write_api_urls.items():
            try:
                ...
                try:
                    exc_desc_lines = traceback.format_exception_only(
                        type(e), e
                    )
    ...
    def multipart_ingest_runs(
        ...
        *,
        ...
        update_dicts = [
            self._run_transform(
                run, update=True, attachments_collector=all_attachments
            )
            for run in update or EMPTY_SEQ
        ]
        ...
        for api_url, api_key in self._write_api_urls.items():
            try:
                ...
                try:
                    ...
                    exc_desc = ''.join(exc_desc_lines).rstrip()
                    logger.warning(
                        f'Failed to multipart ingest runs: {exc_desc}'
                    )
                except Exception:
                    logger.warning(
                        f'Failed to multipart ingest runs: {repr(e)}'
                    )
    ...
    def list_runs(
        ...
        **kwargs : Any
        ...
        if project_name is not None:
            ...
            project_ids.extend(
                [
                    self.read_project(project_name=name).id
    ...
    def list_shared_projects(
        ...
        *,
        ...
        for i, project in enumerate(
            ...
            yield ls_schemas.TracerSessionResult(
                **project, _host_url=self._host_url
            )
            if limit is not None and i + 1 >= limit:
                break
    ...
    def update_project(
        ...
        *,
        ...
        """
        endpoint = (
            f"{self.api_url}/sessions/{_as_uuid(project_id, 'project_id')}"
        )
        extra = project_extra
        if metadata:
            extra = {**(extra or {}), 'metadata': metadata}
        ...
        *,
        ...
        with ThreadPoolExecutor() as executor:
            ...
            if example_ids[cursor:]:
                futures.append(
                    executor.submit(fetch_examples, example_ids[cursor:])
                )
    ...
    def create_example_from_run(
        ...
        if dataset_type == ls_schemas.DataType.llm:
            ...
            if not run.outputs:
                ...
                try:
                    generation = ls_utils.get_llm_generation_from_outputs(
                        run.outputs
                    )
                except ValueError:
                    raise ValueError(
            ...
            try:
                inputs = {
                    'input': ls_utils.get_messages_from_inputs(run.inputs)
                }
            except ValueError:
                raise ValueError(
    ...
    def similar_examples(
        ...
        **Note**: Must have few-shot indexing enabled for the dataset. See
        ...
        for ex in resp.json()['examples']:
            examples.append(
                ls_schemas.ExampleSearch(**ex, dataset_id=dataset_id)
            )
        return examples
    ...
    def _select_eval_results(
        ...
        *,
        ...
        def _is_eval_results(
            results: Any,
        ) -> TypeGuard[ls_evaluator.EvaluationResults]:
    ...
    def create_feedback(
        ...
        **kwargs: Any,
        ...
        if run_id is not None and project_id is not None:
            raise ValueError(
                'Only one of run_id and project_id must be provided'
            )
        if kwargs:
            warnings.warn(
        ...
        if feedback_source_type == ls_schemas.FeedbackSourceType.API:
            ...
            feedback_source = ls_schemas.ModelFeedbackSource(
        ...
        feedback_source.metadata = (
            feedback_source.metadata
            if feedback_source.metadata is not None
            else {}
        )
        if (
            source_run_id is not None
            and '__run' not in feedback_source.metadata
        ):
        ...
        feedback = ls_schemas.FeedbackCreate(
            ...
            feedback_group_id=_ensure_uuid(
    ...
    def list_feedback(
        ...
        *,
        ...
        feedback_source_type: Optional[
            Sequence[ls_schemas.FeedbackSourceType]
        ] = None,
        limit: Optional[int] = None,
        **kwargs: Any,
    ...
    def create_presigned_feedback_tokens(
        ...
        *,
        ...
        """
        # validate
        if feedback_configs is not None and len(feedback_keys) != len(
            feedback_configs
        ):
            raise ValueError(
        ...
        with cf.ThreadPoolExecutor(
            max_workers=len(self._write_api_urls)
        ) as executor:
            futs = [
            ...
            for fut in cf.as_completed(futs):
                ...
                tokens.extend(
                    [
                        ls_schemas.FeedbackIngestToken(**part)
    ...
    def _get_prompt_url(self, prompt_identifier: str) -> str:
        ...
        if not self._current_tenant_is_owner(owner):
            return (
                f'{self._host_url}/hub/{owner}/{prompt_name}:{commit_hash[:8]}'
            )
    ...
    def create_commit(
        ...
        *,
        ...
        if parent_commit_hash == 'latest' or parent_commit_hash is None:
            parent_commit_hash = self._get_latest_commit_hash(
    ...
    def pull_prompt_commit(
        ...
        *,
        ...
        if not use_optimization and commit_hash == 'latest':
            latest_commit_hash = self._get_latest_commit_hash(
                f'{owner}/{prompt_name}'
            )
...
def _tracing_thread_handle_batch(
    ...
    try:
        if use_multipart:
            client.multipart_ingest_runs(
                create=create, update=update, pre_sampled=True
            )
        else:
...
def _tracing_control_thread_func(client_ref: weakref.ref[Client]) -> None:
    ...
    scale_up_nthreads_limit: int = batch_ingest_config[
...
def convert_prompt_to_openai_format(
    ...
    try:
        ...
        raise ls_utils.LangSmithError(
            f'Error converting to OpenAI format: {e}'
        )
...
def convert_prompt_to_anthropic_format(
    ...
    try:
        ...
        raise ls_utils.LangSmithError(
            f'Error converting to Anthropic format: {e}'
        )
