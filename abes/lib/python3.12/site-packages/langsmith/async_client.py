...
class AsyncClient:
    ...
    def __init__(
        ...
        if isinstance(timeout_ms, int):
            ...
            timeout_ = tuple(
                [t / 1000 if t is not None else None for t in timeout_ms]
            )
        else:
            timeout_ = 10
        ...
        **kwargs : Any
        ...
        if project_name is not None:
            ...
            projects = await asyncio.gather(
                *[
                    self.read_project(project_name=name)
        ...
        ix = 0
        async for run in self._aget_cursor_paginated_list(
            '/runs/query', body=body
        ):
    ...
    async def run_is_shared(self, run_id: ls_client.ID_TYPE) -> bool:
        ...
        link = await self.read_run_shared_link(
            ls_client._as_uuid(run_id, 'run_id')
        )
        return link is not None

    async def read_run_shared_link(
        self, run_id: ls_client.ID_TYPE
    ) -> Optional[str]:
    ...
    async def list_datasets(
        ...
        **kwargs: Any,
        ...
        """List datasets."""
        async for dataset in self._aget_paginated_list(
            '/datasets', params=kwargs
        ):
    ...
    async def read_example(
        self, example_id: ls_client.ID_TYPE
    ) -> ls_schemas.Example:
        """Read an example."""
        response = await self._arequest_with_retries(
    ...
    async def list_feedback(
        ...
        **kwargs: Any,
        ...
        params = {
            'run': (
                [str(ls_client._as_uuid(id_)) for id_ in run_ids]
                if run_ids
                else None
            ),
