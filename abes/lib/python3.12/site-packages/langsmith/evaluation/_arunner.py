...
async def aevaluate(
    ...
    /,
    data: Union[
        DATA_T, AsyncIterable[schemas.Example], Iterable[schemas.Example]
    ],
...
async def aevaluate_existing(
    ...
    project = (
        ...
        if isinstance(experiment, schemas.TracerSession)
            ...
            await aitertools.aio_to_thread(
                _load_experiment, experiment, client
            )
        )
    )
    ...
    data_map = await aitertools.aio_to_thread(
        _load_examples_map, client, project
    )
...
async def _aevaluate(
    ...
    is_async_target = asyncio.iscoroutinefunction(target) or (
        hasattr(target, '__aiter__')
        and asyncio.iscoroutine(target.__aiter__())
    )
    ...
    with ls_utils.with_optional_cache(
        ...
        if summary_evaluators:
            manager = await manager.awith_summary_evaluators(
                summary_evaluators
            )
...
class _AsyncExperimentManager(_ExperimentManagerMixin):
    ...
    def __init__(
        ...
        metadata: Optional[dict] = None,
        runs: Optional[
            Union[Iterable[schemas.Run], AsyncIterable[schemas.Run]]
        ] = None,
        client: Optional[langsmith.Client] = None,
        ...
        self._runs = (
            aitertools.ensure_async_iterator(runs)
            if runs is not None
            else None
        )
    ...
    async def aget_examples(self) -> AsyncIterator[schemas.Example]:
        ...
        self._examples, examples_iter = aitertools.atee(
            aitertools.ensure_async_iterator(self._examples),
            2,
            lock=asyncio.Lock(),
        )
    ...
    async def aget_runs(self) -> AsyncIterator[schemas.Run]:
        ...
        self._runs, runs = aitertools.atee(
            aitertools.ensure_async_iterator(self._runs),
            2,
            lock=asyncio.Lock(),
        )
    ...
    async def aget_evaluation_results(
        self,
    ...
    async def awith_evaluators(
        ...
        *,
        ...
        experiment_results = self._ascore(
            evaluators, max_concurrency=max_concurrency
        )
        r1, r2, r3 = aitertools.atee(
            experiment_results, 3, lock=asyncio.Lock()
        )
        return _AsyncExperimentManager(
            ...
            evaluation_results=(
                result['evaluation_results'] async for result in r3
            ),
    ...
    async def __anext__(self) -> ExperimentResultRow:
        ...
        while True:
            ...
            await asyncio.shield(
                asyncio.wait_for(
                    _wait_until_index(len(self._results)), timeout=None
                )
            )
