...
def evaluate_existing(
    ...
    data = [
        data_map[cast(uuid.UUID, run.reference_example_id)] for run in runs
    ]
    return _evaluate(
        runs,
        data=data,
...
class ExperimentResults:
    ...
    def __init__(
        self, experiment_manager: _ExperimentManager, blocking: bool = True
    ):
...
def evaluate_comparative(
    ...
    if len(experiments) < 2:
        raise ValueError(
            'Comparative evaluation requires at least 2 experiments.'
        )
    if not evaluators:
        raise ValueError(
    ...
    projects = [
        _load_experiment(experiment, client) for experiment in experiments
    ]
    ...
    example_ids_nullable = (
        ...
        if examples_intersection is not None
        else []
    )
    ...
    for i in range(0, len(example_ids), batch_size):
        ...
        for e in client.list_examples(
            ...
            data[e.id] = e
    runs_dict: Dict[uuid.UUID, List[schemas.Run]] = collections.defaultdict(
        list
    )
    for runs_list in runs:
        for run in runs_list:
            if run.reference_example_id in data:
                runs_dict[cast(uuid.UUID, run.reference_example_id)].append(
                    run
                )

    comparators = [
        comparison_evaluator(evaluator) for evaluator in evaluators or []
    ]
    results: dict = {}
...
def _is_callable(target: Union[TARGET_T, Iterable[schemas.Run]]) -> bool:
    return callable(target) or (
        hasattr(target, 'invoke') and callable(target.invoke)
    )
...
def _evaluate(
    ...
    client = client or rt.get_cached_client()
    runs = (
        None if _is_callable(target) else cast(Iterable[schemas.Run], target)
    )
...
def _load_traces(
    ...
    for run_id, child_runs in treemap.items():
        all_runs[run_id].child_runs = sorted(
            child_runs, key=lambda r: r.dotted_order
        )
    return results
...
class _ExperimentManagerMixin:
    def __init__(
        ...
        if experiment is None:
            ...
            self._experiment_name = (
                experiment + '-' + str(uuid.uuid4().hex[:8])
            )
        else:
    ...
    def _create_experiment(
        ...
        for _ in range(num_attempts):
            try:
                ...
                self._experiment_name = (
                    f'{starting_name}-{str(uuid.uuid4().hex[:6])}'
                )
        raise ValueError(
    ...
    def _get_project(
        self, first_example: schemas.Example
    ) -> schemas.TracerSession:
        if self._experiment is None:
...
class _ExperimentManager(_ExperimentManagerMixin):
    ...
    def dataset_id(self) -> str:
        ...
        return str(
            cast(
                schemas.TracerSessionResult, self._experiment
    ...
    def runs(self) -> Iterable[schemas.Run]:
        if self._runs is None:
            raise ValueError(
                'Runs not provided in this experiment.'
                ' Please predict first.'
            )
...
def _resolve_experiment(
    ...
    Optional[Union[schemas.TracerSession, str]],
    Optional[Iterable[schemas.Run]],
]:
