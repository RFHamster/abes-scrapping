...
def tracing_context(
    ...
    **kwargs: Any,
    ...
    enabled = (
        enabled if enabled is not None else current_context.get('enabled')
    )
...
def is_traceable_function(
    ...
    return (
        ...
        or (
            ...
            and _is_traceable_function(func.func)
        )
        or (
            hasattr(func, '__call__') and _is_traceable_function(func.__call__)
        )
    )
    ...
    **kwargs: Any,
    ...
    def decorator(func: Callable):
        ...
        func_accepts_parent_run = (
            func_sig.parameters.get('run_tree', None) is not None
        )
        func_accepts_config = (
            func_sig.parameters.get('config', None) is not None
        )
        ...
        def wrapper(
            ...
            **kwargs: Any,
            ...
            func_accepts_parent_run = (
                inspect.signature(func).parameters.get('run_tree', None)
                is not None
            )
            try:
                if func_accepts_parent_run:
        ...
        def generator_wrapper(
            ...
            **kwargs: Any,
            ...
            try:
                ...
                raise e
            _on_run_end(
                run_container, outputs=_get_function_result(results, reduce_fn)
            )
...
class trace:
    ...
    def _teardown(
        ...
        if self.old_ctx is not None:
            ...
            warnings.warn(
                'Tracing context was not set up properly.', RuntimeWarning
            )
    ...
    class RunnableTraceable(RunnableLambda):
        ...
        def __init__(
            ...
            if is_async(func):
                ...
                wrapped = cast(
                    Callable[[Input], Output], self._wrap_async(func)
                )
            elif is_traceable_function(func):
                wrapped = cast(
                    Callable[[Input], Output], self._wrap_sync(func)
                )
            if wrapped is None:
                raise ValueError(
        ...
        def _wrap_sync(
            ...
            def wrap_traceable(inputs: dict, config: RunnableConfig) -> Any:
                run_tree = run_trees.RunTree.from_runnable_config(
                    cast(dict, config)
                )
                return func(**inputs, langsmith_extra={'run_tree': run_tree})

            return cast(
                Callable[[Input, RunnableConfig], Output], wrap_traceable
            )
        ...
        def _wrap_async(
            ...
            async def awrap_traceable(
                ...
                return await afunc_(
                    **inputs, langsmith_extra={'run_tree': run_tree}
                )
...
def _setup_run(
    ...
    if not parent_run_ and not utils.tracing_is_enabled():
        utils.log_once(
            logging.DEBUG,
            'LangSmith tracing is enabled, returning original function.',
        )
    ...
    if invocation_params_fn:
        try:
            invocation_params = {
                k: v
                for k, v in invocation_params_fn(inputs).items()
                if v is not None
            }
...
class _TracedAsyncStream(_TracedStreamBase, Generic[T]):
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        try:
            return await self.__ls_stream__.__aexit__(
