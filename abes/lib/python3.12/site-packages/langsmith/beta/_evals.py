...
def _convert_root_run(
    root: ls_schemas.Run, run_to_example_map: dict
) -> List[dict]:
    ...
    while runs_:
        src = runs_.pop()
        src_dict = src.dict(
            exclude={'parent_run_ids', 'child_run_ids', 'session_id'}
        )
...
def convert_runs_to_test(
    ...
    *,
    ...
    if not runs:
        raise ValueError(
            f"""Expected a non-empty sequence of runs. Received: {runs}"""
        )
    ...
    if not load_child_runs:
        ...
        runs_to_copy = [
            client.read_run(r.id, load_child_runs=load_child_runs)
            for r in runs
        ]

    test_project_name = (
        test_project_name or f'prod-baseline-{uuid.uuid4().hex[:6]}'
    )
    ...
    dataset_version = (
        ...
        if examples[0].modified_at
        else examples[0].created_at
    )
