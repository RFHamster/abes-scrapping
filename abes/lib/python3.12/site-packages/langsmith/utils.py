...
def get_message_generation_from_outputs(
...
def with_cache(
    ...
    def _filter_request_headers(request: Any) -> Any:
        if ignore_hosts and any(
            request.url.startswith(host) for host in ignore_hosts
        ):
            return None
        request.headers = {}
        return request
...
def get_api_key(api_key: Optional[str]) -> Optional[str]:
    ...
    api_key_ = (
        ...
        else get_env_var('API_KEY', default=None)
    )
