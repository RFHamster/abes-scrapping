...
def _is_has_never_check_common_name_reliable(
    ...
    return is_openssl and (
        ...
        or _is_bpo_43522_fixed(
            implementation_name, version_info, pypy_version_info
        )
    )
...
try:  # Do we have ssl at all?
    ...
    if (
        HAS_NEVER_CHECK_COMMON_NAME
        and not _is_has_never_check_common_name_reliable(
            OPENSSL_VERSION,
            OPENSSL_VERSION_NUMBER,
    ...
    for attr in ('TLSv1', 'TLSv1_1', 'TLSv1_2'):
        try:
            _SSL_VERSION_TO_TLS_VERSION[
                getattr(ssl, f'PROTOCOL_{attr}')
            ] = getattr(TLSVersion, attr)
...
def is_ipaddress(hostname: str | bytes) -> bool:
    ...
    return bool(
        _IPV4_RE.match(hostname) or _BRACELESS_IPV6_ADDRZ_RE.match(hostname)
    )
