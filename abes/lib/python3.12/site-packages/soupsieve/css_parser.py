...
WSC = rf'(?:{WS}|{COMMENTS})'
# CSS escapes
CSS_ESCAPES = rf'(?:\\(?:[a-f0-9]{{1,6}}{WS}?|[^\r\n\f]|$))'
CSS_STRING_ESCAPES = rf'(?:\\(?:[a-f0-9]{{1,6}}{WS}?|[^\r\n\f]|$|{NEWLINE}))'
# CSS Identifier
IDENTIFIER = rf"""
...
"""
# `nth` content
NTH = rf'(?:[-+])?(?:[0-9]+n?|n)(?:(?<=n){WSC}*(?:[-+]){WSC}*(?:[0-9]+))?'
# Value: quoted string or identifier
VALUE = rf"""(?:"(?:\\(?:.|{NEWLINE})|[^\\"\r\n\f]+)*?"|'(?:\\(?:.|{NEWLINE})|[^\\'\r\n\f]+)*?'|{IDENTIFIER}+)"""
# Attribute value comparison. `!=` is handled special as it is non-standard.
ATTR = rf'(?:{WSC}*(?P<cmp>[!~^|*$]?=){WSC}*(?P<value>{VALUE})(?:{WSC}*(?P<case>[is]))?)?{WSC}*\]'

# Selector patterns
# IDs (`#id`)
PAT_ID = rf'\#{IDENTIFIER}'
# Classes (`.class`)
PAT_CLASS = rf'\.{IDENTIFIER}'
...
PAT_PSEUDO_CLASS_SPECIAL = rf'(?P<name>:{IDENTIFIER})(?P<open>\({WSC}*)'
# Custom pseudo class (`:--custom-pseudo`)
PAT_PSEUDO_CLASS_CUSTOM = rf'(?P<name>:(?=--){IDENTIFIER})'
# Nesting ampersand selector. Matches `&`
PAT_AMP = r'&'
# Closing pseudo group (`)`)
PAT_PSEUDO_CLOSE = rf'{WSC}*\)'
# Pseudo element (`::pseudo-element`)
PAT_PSEUDO_ELEMENT = rf':{PAT_PSEUDO_CLASS}'
# At rule (`@page`, etc.) (not supported)
PAT_AT_RULE = rf'@P{IDENTIFIER}'
# Pseudo class `nth-child` (`:nth-child(an+b [of S]?)`, `:first-child`, etc.)
PAT_PSEUDO_NTH_CHILD = rf"""
...
PAT_COMBINE = rf'{WSC}*?(?P<relation>[,+>~]|{WS}(?![,+>~])){WSC}*'
# Extra: Contains (`:contains(text)`)
PAT_PSEUDO_CONTAINS = rf'{PAT_PSEUDO_CLASS_SPECIAL}(?P<values>{VALUE}(?:{WSC}*,{WSC}*{VALUE})*){WSC}*\)'
...
RE_NTH = re.compile(
    rf'(?P<s1>[-+])?(?P<a>[0-9]+n?|n)(?:(?<=n){WSC}*(?P<s2>[-+]){WSC}*(?P<b>[0-9]+))?',
    re.I,
)
# Pattern to iterate multiple values.
RE_VALUES = re.compile(
    rf'(?:(?P<value>{VALUE})|(?P<split>{WSC}*,{WSC}*))', re.X
)
# Whitespace checks
RE_WS = re.compile(WS)
RE_WS_BEGIN = re.compile(rf'^{WSC}*')
RE_WS_END = re.compile(rf'{WSC}*$')
RE_CUSTOM = re.compile(rf'^{PAT_PSEUDO_CLASS_CUSTOM}$', re.X)
...
def _cached_css_compile(
    ...
    return cm.SoupSieve(
        pattern,
        CSSParser(
            pattern, custom=custom_selectors, flags=flags
        ).process_selectors(),
        namespaces,
        custom,
        flags,
...
class SelectorPattern:
    ...
    def match(
        self, selector: str, index: int, flags: int
    ) -> Match[str] | None:
        """Match the selector."""
...
class SpecialPseudoPattern(SelectorPattern):
    ...
    def __init__(
        self,
        patterns: tuple[
            tuple[str, tuple[str, ...], str, type[SelectorPattern]], ...
        ],
    ) -> None:
        """Initialize."""
        ...
        self.re_pseudo_name = re.compile(
            PAT_PSEUDO_CLASS_SPECIAL, re.I | re.X | re.U
        )
    ...
    def get_name(self) -> str:
        ...
        return (
            '' if self.matched_name is None else self.matched_name.get_name()
        )
...
class CSSParser:
    ...
    def parse_attribute_selector(
        ...
        ns = (
            css_unescape(m.group('attr_ns')[:-1]) if m.group('attr_ns') else ''
        )
        ...
        if not op:
            ...
            value = (
                r'[^\s\S]'
                if not value or RE_WS.search(value)
    ...
    def parse_tag_pattern(
        ...
        prefix = (
            css_unescape(m.group('tag_ns')[:-1]) if m.group('tag_ns') else None
        )
    ...
    def parse_pseudo_class(
        ...
        if complex_pseudo and pseudo in PSEUDO_COMPLEX:
            ...
            if pseudo == ':root':
                ...
                sel.nth.append(
                    ct.SelectorNth(
                        1, False, 0, False, False, ct.SelectorList()
                    )
                )
                ...
                sel.nth.extend(
                    [
                        ...
                        ct.SelectorNth(
                            1, False, 0, False, True, ct.SelectorList()
                        ),
                    ]
                )
                ...
                sel.nth.extend(
                    [
                        ct.SelectorNth(
                            1, False, 0, True, False, ct.SelectorList()
                        ),
                        ct.SelectorNth(
                            1, False, 0, True, True, ct.SelectorList()
                        ),
                    ]
                )
            has_selector = True
    ...
    def parse_pseudo_nth(
        ...
        if content == 'even':
            ...
            _s1 = (
                '-'
                if nth_parts.group('s1') and nth_parts.group('s1') == '-'
                else ''
            )
            a = nth_parts.group('a')
            var = a.endswith('n')
        ...
        if postfix == '_child':
            ...
            if pseudo_sel == ':nth-child':
                sel.nth.append(
                    ct.SelectorNth(s1, var, s2, False, False, nth_sel)
                )
            elif pseudo_sel == ':nth-last-child':
                sel.nth.append(
                    ct.SelectorNth(s1, var, s2, False, True, nth_sel)
                )
        else:
    ...
    def parse_selectors(
        ...
        try:
            while True:
                ...
                if key == 'at_rule':
                    ...
                    has_selector = self.parse_pseudo_class_custom(
                        sel, m, has_selector
                    )
                    ...
                    has_selector = self.parse_pseudo_contains(
                        sel, m, has_selector
                    )
                    ...
                    has_selector = self.parse_attribute_selector(
                        sel, m, has_selector
                    )
                elif key == 'tag':
                    if has_selector:
        ...
        if not has_selector:
            ...
            raise SelectorSyntaxError(
                f'Expected a selector at position {index}', self.pattern, index
            )
        ...
        return ct.SelectorList(
            [s.freeze() for s in selectors], is_not, is_html
        )
        ...
        while index <= end:
            ...
            if m is None:
                ...
                if c == '[':
                    ...
                    msg = (
                        f'Malformed pseudo-class selector at position {index}'
                    )
                else:
    ...
    def process_selectors(
        self, index: int = 0, flags: int = 0
    ) -> ct.SelectorList:
        """Process selectors."""

        return self.parse_selectors(
            self.selector_iter(self.pattern), index, flags
        )
