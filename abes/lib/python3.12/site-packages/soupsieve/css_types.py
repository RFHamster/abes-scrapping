...
class Immutable:
    ...
    def __repr__(self) -> str:  # pragma: no cover
        ...
        r = ', '.join(
            [f'{k}={getattr(self, k)!r}' for k in self.__slots__[:-1]]
        )
...
class ImmutableDict(Mapping[Any, Any]):
    ...
    def __init__(
        self, arg: dict[Any, Any] | Iterable[tuple[Any, Any]]
    ) -> None:
        """Initialize."""
        ...
        self._hash = hash(
            tuple(
                [(type(x), x, type(y), y) for x, y in sorted(self._d.items())]
            )
        )
    ...
    def _validate(
        ...
        if isinstance(arg, dict):
            if not all(isinstance(v, Hashable) for v in arg.values()):
                raise TypeError(
                    f'{self.__class__.__name__} values must be hashable'
                )
        elif not all(
...
class Namespaces(ImmutableDict):
    ...
    def __init__(
        self, arg: dict[str, str] | Iterable[tuple[str, str]]
    ) -> None:
        """Initialize."""
