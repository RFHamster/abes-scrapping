...
SRV_POLLING_TOPOLOGIES: tuple[int, int] = (
...
class TopologyDescription:
    def __init__(
        ...
        if not readable_servers:
            ...
            s.logical_session_timeout_minutes is None for s in readable_servers
        ):
            self._ls_timeout_minutes = None
        else:
    ...
    def __repr__(self) -> str:
        ...
        servers = sorted(
            self._server_descriptions.values(), key=lambda sd: sd.address
        )
...
def updated_topology_description(
    ...
    if topology_type == TOPOLOGY_TYPE.Single:
        ...
        if (
            ...
            and set_name != server_description.replica_set_name
        ):
...
def _update_rs_from_primary(
    ...
    if (
        ...
        if server_description.set_version is not None and (
            max_set_version is None
            or server_description.set_version > max_set_version
        ):
...
def _update_rs_with_primary_from_member(
    ...
    if replica_set_name != server_description.replica_set_name:
        ...
        server_description.me
        and server_description.address != server_description.me
    ):
        sds.pop(server_description.address)
