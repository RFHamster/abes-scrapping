...
def _maybe_add_read_preference(
    ...
    if mode and (
        mode != ReadPreference.SECONDARY_PREFERRED.mode or len(document) > 1
    ):
...
def _gen_find_command(
    ...
    if options:
        cmd.update(
            [(opt, True) for opt, val in _OPTIONS.items() if options & val]
        )
...
class _EncryptedBulkWriteContext(_BulkWriteContext):
    ...
    def batch_command(
        ...
        outgoing = _inflate_bson(
            memoryview(msg)[cmd_start:], DEFAULT_RAW_BSON_OPTIONS
        )
...
def _client_batched_op_msg_impl(
    ...
    if not ack:
        _check_doc_size_limits(
            'bulkWrite',
            len(command_encoded),
            max_bson_size + _COMMAND_OVERHEAD,
        )
    ...
    max_doc_sequences_bytes = max_message_size - (
        _OP_MSG_OVERHEAD + command_len_abridged
    )
    ...
    for (real_op_type, op_doc), namespace in zip(operations, namespaces):
        ...
        if real_op_type == 'replace':
            ...
            if not ack:
                doc_size = len(
                    _dict_to_bson(op_doc['updateMods'], False, opts)
                )
                _check_doc_size_limits(real_op_type, doc_size, max_bson_size)
        ...
        if not ack:
            _check_doc_size_limits(
                op_type, op_length, max_bson_size + _COMMAND_OVERHEAD
            )

        new_message_size = (
            total_ops_length + total_ns_length + op_length + ns_length
        )
        # We have enough data, return this batch.
...
class _OpMsg:
    ...
    def raw_response(
        ...
        inflated_response = bson._decode_selective(
            RawBSONDocument(self.payload_document),
            user_fields,
            _RAW_ARRAY_BSON_OPTIONS,
        )
