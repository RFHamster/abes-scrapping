...
def raise_config_error(
    key: str, suggestions: Optional[list] = None
) -> NoReturn:
...
def validate_boolean_or_string(option: str, value: Any) -> bool:
    ...
    if isinstance(value, str):
        if value not in ('true', 'false'):
            raise ValueError(
                f"The value of {option} must be 'true' or 'false'"
            )
        return value == 'true'
...
def validate_integer(option: str, value: Any) -> int:
    ...
    if isinstance(value, int):
        ...
        try:
            ...
            raise ValueError(
                f'The value of {option} must be an integer'
            ) from None
...
def validate_non_negative_integer(option: str, value: Any) -> int:
    ...
    if val < 0:
        raise ValueError(
            f'The value of {option} must be a non negative integer'
        )
    return val
...
def validate_positive_integer_or_none(
...
def validate_non_negative_integer_or_none(
    option: str, value: Any
) -> Optional[int]:
...
def validate_string(option: str, value: Any) -> str:
    ...
    if isinstance(value, str):
        return value
    raise TypeError(
        f'Wrong type for {option}, value must be an instance of str'
    )
...
def validate_int_or_basestring(option: str, value: Any) -> Union[int, str]:
    ...
    if isinstance(value, int):
        ...
        try:
            ...
            return value
    raise TypeError(
        f'Wrong type for {option}, value must be an integer or a string'
    )


def validate_non_negative_int_or_basestring(
...
def validate_positive_float(option: str, value: Any) -> float:
    ...
    if not 0 < value < 1e9:
        raise ValueError(
            f'{option} must be greater than 0 and less than one billion'
        )
    return value
...
def validate_timeout_or_none_or_zero(
    option: Any, value: Any
) -> Optional[float]:
    """Validates a timeout specified in milliseconds returning
...
def validate_read_preference_tags(
    ...
    for tag_set in value:
        ...
        try:
            ...
            raise ValueError(
                f'{tag_set!r} not a valid value for {name}'
            ) from None
    return tag_sets
...
def validate_server_api_or_none(
    option: Any, value: Any
) -> Optional[ServerApi]:
...
def validate_is_callable_or_none(
    option: Any, value: Any
) -> Optional[Callable]:
...
def validate_auto_encryption_opts_or_none(
    option: Any, value: Any
) -> Optional[Any]:
    """Validate the driver keyword arg."""
    if value is None:
...
def validate_datetime_conversion(
    ...
    raise TypeError(
        f'{option} must be a str or int representing DatetimeConversion'
    )
...
for optname, aliases in URI_OPTIONS_ALIAS_MAP.items():
    for alias in aliases:
        if alias not in URI_OPTIONS_VALIDATOR_MAP:
            URI_OPTIONS_VALIDATOR_MAP[alias] = URI_OPTIONS_VALIDATOR_MAP[
                optname
            ]

# Map containing all URI option and keyword argument validators.
VALIDATORS: dict[
    str, Callable[[Any, Any], Any]
] = URI_OPTIONS_VALIDATOR_MAP.copy()
