...
GSSAPIProperties = namedtuple(
    'GSSAPIProperties',
    ['service_name', 'canonicalize_host_name', 'service_realm'],
)
...
def _build_credentials_tuple(
    ...
    if (
        mech not in ('MONGODB-X509', 'MONGODB-AWS', 'MONGODB-OIDC')
        and user is None
    ):
    ...
    if mech == 'GSSAPI':
        ...
        return MongoCredential(
            mech, '$external', user, passwd, aws_props, None
        )
        ...
        return MongoCredential(
            mech, '$external', user, passwd, oidc_props, _Cache()
        )
