...
class AsyncCollection(common.BaseObject, Generic[_DocumentType]):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if '$' in name and not (name.startswith(('oplog.$main', '$cmd'))):
            raise InvalidName(
                "collection names must not contain '$': %r" % name
            )
        if name[0] == '.' or name[-1] == '.':
            raise InvalidName(
    ...
    def __eq__(self, other: Any) -> bool:
        if isinstance(other, AsyncCollection):
            return (
                self._database == other.database and self._name == other.name
            )
    ...
    async def _create(
        ...
        if encrypted_fields:
            ...
            await self._create_helper(
                _ecoc_coll_name(encrypted_fields, self._name),
                opts,
                None,
                session,
            )
    ...
    async def bulk_write(
        ...
        for request in requests:
            try:
                ...
                raise TypeError(
                    f'{request!r} is not a valid request'
                ) from None
    ...
    async def estimated_document_count(
        ...
        async def _cmd(
            ...
            cmd.update(kwargs)
            return await self._count_cmd(
                session, conn, read_preference, cmd, collation=None
            )

        return await self._retryable_non_cursor_read(
            _cmd, None, operation=_Op.COUNT
        )
    ...
    async def _create_indexes(
        ...
        **kwargs: Any,
        ...
        names = []
        async with await self._conn_for_writes(
            session, operation=_Op.CREATE_INDEXES
        ) as conn:
    ...
    async def _drop_index(
        ...
        **kwargs: Any,
        ...
        if comment is not None:
            cmd['comment'] = comment
        async with await self._conn_for_writes(
            session, operation=_Op.DROP_INDEXES
        ) as conn:
            await self._command(
                conn,
                cmd,
    ...
    async def create_search_indexes(
        ...
        **kwargs: Any,
        ...
        .. versionadded:: 4.5
        """
        return await self._create_search_indexes(
            models, session, comment, **kwargs
        )
    ...
    async def drop_search_index(
        ...
        **kwargs: Any,
        ...
        if comment is not None:
            cmd['comment'] = comment
        async with await self._conn_for_writes(
            session, operation=_Op.DROP_SEARCH_INDEXES
        ) as conn:
            await self._command(
                conn,
                cmd,
    ...
    async def update_search_index(
        ...
        **kwargs: Any,
        ...
        if comment is not None:
            cmd['comment'] = comment
        async with await self._conn_for_writes(
            session, operation=_Op.UPDATE_SEARCH_INDEX
        ) as conn:
            await self._command(
                conn,
                cmd,
    ...
    async def aggregate(
        ...
        **kwargs: Any,
        ...
        """
        async with self._database.client._tmp_session(
            session, close=False
        ) as s:
    ...
    async def distinct(
        ...
        **kwargs: Any,
        ...
        return await self._retryable_non_cursor_read(
            _cmd, session, operation=_Op.DISTINCT
        )
    ...
    async def _find_and_modify(
        ...
        **kwargs: Any,
        ...
        if projection is not None:
            cmd['fields'] = helpers_shared._fields_list_to_dict(
