...
if TYPE_CHECKING:
    ...
    from pymongo.asynchronous.mongo_client import (
        AsyncMongoClient,
...
if sys.platform == 'win32':
    ...
    def _set_keepalive_times(sock):
        ...
        if (
            idle_ms < _WINDOWS_TCP_IDLE_MS
            or interval_ms < _WINDOWS_TCP_INTERVAL_MS
        ):
...
async def _cond_wait(
    condition: _ACondition, deadline: Optional[float]
) -> bool:
...
class AsyncConnection:
    ...
    async def _next_reply(self) -> dict[str, Any]:
        ...
        helpers_shared._check_command_response(
            response_doc, self.max_wire_version
        )
        return response_doc
    ...
    async def command(
        ...
        if not (
            write_concern is None
            or write_concern.acknowledged
    ...
    async def send_message(self, message: bytes, max_doc_size: int) -> None:
        ...
        """
        if (
            self.max_bson_size is not None
    ...
    async def receive_message(
        ...
        try:
            return await receive_message(
                self, request_id, self.max_message_size
            )
        except BaseException as error:
            self._raise_connection_failure(error)
        ...
        if not self.ready:
            ...
            if creds:
                ...
                await auth.authenticate(
                    creds, self, reauthenticate=reauthenticate
                )
            self.ready = True
            duration = time.monotonic() - self.creation_time
            if self.enabled_for_cmap:
                assert self.listeners is not None
                self.listeners.publish_connection_ready(
                    self.address, self.id, duration
                )
            if self.enabled_for_logging and _CONNECTION_LOGGER.isEnabledFor(
                logging.DEBUG
            ):
                _debug_log(
                    _CONNECTION_LOGGER,
    ...
    def _raise_connection_failure(self, error: BaseException) -> NoReturn:
        ...
        if isinstance(error, (IOError, OSError, SSLError)):
            ...
            _raise_connection_failure(
                self.address, error, timeout_details=details
            )
        else:
            raise
...
async def _configured_socket(
    ...
    if (
        ...
        try:
            ssl.match_hostname(
                ssl_sock.getpeercert(), hostname=host
        ...
        async with self.lock:
            if self.state != PoolState.READY:
                ...
                if (
                    ...
                    and _CONNECTION_LOGGER.isEnabledFor(logging.DEBUG)
                ):
                    _debug_log(
                        _CONNECTION_LOGGER,
                        clientId=self._client_id,
    ...
    async def _reset(
        ...
        async with self.size_cond:
            if self.closed:
                return
            if (
                self.opts.pause_enabled
                and pause
                and not self.opts.load_balanced
            ):
    ...
    async def remove_stale_sockets(self, reference_generation: int) -> None:
        ...
        while True:
            async with self.size_cond:
                ...
                if (
                    len(self.conns) + self.active_sockets
        ...
        async with self.size_cond:
            ...
            while not (self.requests < self.max_pool_size):
                ...
                self._raise_if_not_ready(
                    checkout_started_time, emit_event=True
                )
            self.requests += 1
        ...
        try:
            ...
            while conn is None:
                ...
                async with self._max_connecting_cond:
                    ...
                    while not (
                        self.conns or self._pending < self._max_connecting
                    ):
                        if not await _cond_wait(
                            self._max_connecting_cond, deadline
                        ):
                            # Timed out, notify the next thread to ensure a
                            # timeout doesn't consume the condition.
                            if (
                                self.conns
                                or self._pending < self._max_connecting
                            ):
                                self._max_connecting_cond.notify()
                            emitted_event = True
