...
class AsyncCursor(Generic[_DocumentType]):
    ...
    def __init__(
        ...
        if projection is not None:
            projection = helpers_shared._fields_list_to_dict(
    ...
    def _clone(
        self, deepcopy: bool = True, base: Optional[AsyncCursor] = None
    ) -> AsyncCursor:
    ...
    def _query_spec(self) -> Mapping[str, Any]:
        ...
        elif 'query' in self._spec and (
            len(self._spec) == 1 or next(iter(self._spec)) == 'query'
        ):
        ...
        if _IS_SYNC:
            ...
            if isinstance(index, slice):
                if index.step is not None:
                    raise IndexError(
                        'AsyncCursor instances do not support slice steps'
                    )
                ...
                if index.start is not None:
                    if index.start < 0:
                        raise IndexError(
                            'AsyncCursor instances do not support negative indices'
                        )
                    skip = index.start
            ...
            raise TypeError(
                'index %r cannot be applied to AsyncCursor instances' % index
            )
        else:
    ...
    def _get_read_preference(self) -> _ServerMode:
        if self._read_preference is None:
            ...
            self._read_preference = self._collection._read_preference_for(
                self.session
            )
    ...
    ) -> tuple[int, Optional[_CursorAddress]]:
        self._killed = True
        if self._id and not already_killed:
            cursor_id = self._id
            assert self._address is not None
            address = _CursorAddress(
                self._address, f'{self._dbname}.{self._collname}'
            )
        else:
            # Skip killCursors.
            cursor_id = 0
    ...
    async def distinct(self, key: str) -> list:
        ...
        return await self._collection.distinct(
            key, session=self._session, **options
        )
...
class AsyncRawBatchCursor(AsyncCursor, Generic[_DocumentType]):
    ...
    def __getitem__(self, index: Any) -> NoReturn:
        raise InvalidOperation(
            'Cannot call __getitem__ on AsyncRawBatchCursor'
        )
