...
from pymongo.asynchronous.change_stream import (
    AsyncChangeStream,
...
_WriteCall = Callable[
    [Optional['AsyncClientSession'], 'AsyncConnection', bool],
    Coroutine[Any, Any, T],
]
_ReadCall = Callable[
...
class AsyncMongoClient(common.BaseObject, Generic[_DocumentType]):
        ...
        **kwargs: Any,
        ...
        if len([h for h in host if '/' in h]) > 1:
            raise ConfigurationError(
                'host must not contain multiple MongoDB URIs'
            )
        for entity in host:
        ...
        if srv_service_name is None:
            srv_service_name = opts.get(
                'srvServiceName', common.SRV_SERVICE_NAME
            )
        ...
        self._options = options = ClientOptions(
            username, password, dbase, opts, _IS_SYNC
        )
        ...
        if self._options.auto_encryption_opts:
            ...
            self._encrypter = _Encrypter(
                self, self._options.auto_encryption_opts
            )
    ...
    def _should_pin_cursor(
        self, session: Optional[AsyncClientSession]
    ) -> Optional[bool]:
        return self._options.load_balanced and not (
            session and session.in_transaction
        )
        ...
        def option_repr(option: str, value: Any) -> str:
            ...
            if option == 'document_class':
                if value is dict:
                    ...
                    return (
                        f'document_class={value.__module__}.{value.__name__}'
                    )
    ...
    def _start_session(
        ...
        return client_session.AsyncClientSession(
    ...
    def start_session(
        ...
        default_transaction_options: Optional[
            client_session.TransactionOptions
        ] = None,
        ...
        try:
            ...
            async with await self._conn_for_reads(
                ...
                for i in range(0, len(session_ids), common._MAX_END_SESSIONS):
                    spec = {
                        'endSessions': session_ids[
                            i : i + common._MAX_END_SESSIONS
                        ]
                    }
                    await conn.command(
    ...
    async def _checkout(
        ...
        async with _MongoClientErrorHandler(
            self, server, session
        ) as err_handler:
    ...
    async def _conn_for_writes(
        ...
        server = await self._select_server(
            writable_server_selector, session, operation
        )
    ...
    async def _conn_from_server(
        ...
        single = (
            self._topology.description.topology_type == TOPOLOGY_TYPE.Single
        )
        ...
        try:
            if conn_mgr:
                async with conn_mgr._alock:
                    ...
                    assert conn_mgr.conn is not None
                    await self._kill_cursor_impl(
                        [cursor_id], address, session, conn_mgr.conn
                    )
            else:
                await self._kill_cursors(
                    [cursor_id], address, await self._get_topology(), session
                )
        except PyMongoError:
    ...
    async def _kill_cursors(
        ...
        if address:
            ...
            server = await topology.select_server(
                writable_server_selector, _Op.KILL_CURSORS
            )
    ...
    async def _process_kill_cursors(self) -> None:
        ...
        for address, cursor_id, conn_mgr in pinned_cursors:
            try:
                await self._cleanup_cursor_lock(
                    cursor_id, address, conn_mgr, None, False
                )
            except Exception as exc:
                if (
                    isinstance(exc, InvalidOperation)
    ...
    async def _tmp_session(
        self,
        session: Optional[client_session.AsyncClientSession],
        close: bool = True,
    ) -> AsyncGenerator[
        Optional[client_session.AsyncClientSession], None, None
    ]:
        """If provided session is None, lend a temporary session."""
        if session is not None:
            if not isinstance(session, client_session.AsyncClientSession):
                raise ValueError(
                    "'session' argument must be an AsyncClientSession or None."
                )
    ...
    async def list_database_names(
        ...
        .. versionadded:: 3.6
        """
        res = await self._list_databases(
            session, nameOnly=True, comment=comment
        )
    ...
    async def drop_database(
        ...
        if not isinstance(name, str):
            raise TypeError(
                'name_or_database must be an instance of str or a AsyncDatabase'
            )

        async with await self._conn_for_writes(
            session, operation=_Op.DROP_DATABASE
        ) as conn:
...
class _MongoClientErrorHandler:
    ...
    def __init__(
        ...
        if not isinstance(client, AsyncMongoClient):
            ...
            if not any(
                cls.__name__ == 'AsyncMongoClient'
                for cls in type(client).__mro__
            ):
                raise TypeError(
                    f'AsyncMongoClient required but given {type(client).__name__}'
                )
    ...
    def contribute_socket(
        self, conn: AsyncConnection, completed_handshake: bool = True
    ) -> None:
...
class _ClientConnectionRetryable(Generic[T]):
                ...
                if self._is_read:
                    if isinstance(exc, (ConnectionFailure, OperationFailure)):
                        ...
                        if self._is_not_eligible_for_retry() or (
                            ...
                            and exc_code
                            not in helpers_shared._RETRYABLE_ERROR_CODES
                        ):
                            raise
                        self._retrying = True
    ...
    async def _write(self) -> T:
        ...
        try:
            ...
            async with self._client._checkout(
                self._server, self._session
            ) as conn:
    ...
    async def _read(self) -> T:
        ...
        self._server = await self._get_server()
        assert (
            self._read_pref is not None
        ), 'Read Preference required on read calls'
        async with self._client._conn_from_server(
