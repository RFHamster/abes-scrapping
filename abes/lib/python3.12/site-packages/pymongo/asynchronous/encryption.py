...
try:
    from pymongocrypt.asynchronous.auto_encrypter import (
        AsyncAutoEncrypter,
    )  # type:ignore[import]
...
class _EncryptionIO(AsyncMongoCryptCallback):  # type: ignore[misc]
    ...
    async def kms_request(self, kms_context: MongoCryptKmsContext) -> None:
        ...
        connect_timeout = max(
            _csot.clamp_remaining(_KMS_CONNECT_TIMEOUT), 0.001
        )
        opts = PoolOptions(
        ...
        try:
            ...
            try:
                ...
                while kms_context.bytes_needed > 0:
                    # CSOT: update timeout.
                    conn.settimeout(
                        max(_csot.clamp_remaining(_KMS_CONNECT_TIMEOUT), 0)
                    )
    ...
    async def collection_info(
        self, database: str, filter: bytes
    ) -> Optional[bytes]:
    ...
    async def insert_data_key(self, data_key: bytes) -> Binary:
        ...
        if (
            not isinstance(data_key_id, Binary)
            or data_key_id.subtype != UUID_SUBTYPE
        ):
...
class RewrapManyDataKeyResult:
    ...
    def __init__(
        self, bulk_write_result: Optional[BulkWriteResult] = None
    ) -> None:
...
class _Encrypter:
    ...
    def __init__(
        self,
        client: AsyncMongoClient[_DocumentTypeArg],
        opts: AutoEncryptionOpts,
    ):
        ...
        def _get_internal_client(
            ...
            mongo_client: AsyncMongoClient[_DocumentTypeArg],
    ...
    async def encrypt(
        ...
        with _wrap_encryption_errors():
            encrypted_cmd = await self._auto_encrypter.encrypt(
                database, encoded_cmd
            )
...
class AsyncClientEncryption(Generic[_DocumentType]):
    ...
    def __init__(
        ...
        if not isinstance(key_vault_client, AsyncMongoClient):
            ...
            if not any(
                ...
                for cls in type(key_vault_client).__mro__
            ):
                raise TypeError(
        ...
        self._encryption = AsyncExplicitEncrypter(
            self._io_callbacks,
            _create_mongocrypt_options(
                kms_providers=kms_providers, schema_map=None
            ),
        )
    ...
    async def create_encrypted_collection(
        ...
        **kwargs: Any,
        ...
        if not isinstance(database, AsyncDatabase):
            # This is for compatibility with mocked and subclassed types, such as in Motor.
            if not any(
                cls.__name__ == 'AsyncDatabase'
                for cls in type(database).__mro__
            ):
                raise TypeError(
                    f'AsyncDatabase required but given {type(database).__name__}'
                )
        ...
        for i, field in enumerate(encrypted_fields['fields']):
            if isinstance(field, dict) and field.get('keyId') is None:
                try:
                    encrypted_fields['fields'][i][
                        'keyId'
                    ] = await self.create_data_key(
                        kms_provider=kms_provider,  # type:ignore[arg-type]
                        master_key=master_key,
                    )
                except EncryptionError as exc:
                    raise EncryptedCollectionError(
                        exc, encrypted_fields
                    ) from exc
    ...
    async def decrypt(self, value: Binary) -> Any:
        ...
        with _wrap_encryption_errors():
            ...
            return decode(decrypted_doc, codec_options=self._codec_options)[
                'v'
            ]
    ...
    async def add_key_alt_name(self, id: Binary, key_alt_name: str) -> Any:
        ...
        assert self._key_vault_coll is not None
        return await self._key_vault_coll.find_one_and_update(
            {'_id': id}, update
        )
    ...
    async def get_key_by_alt_name(
        ...
        self._check_closed()
        assert self._key_vault_coll is not None
        return await self._key_vault_coll.find_one(
    ...
    async def rewrap_many_data_key(
        ...
        self._check_closed()
        with _wrap_encryption_errors():
            raw_result = await self._encryption.rewrap_many_data_key(
