...
class Topology:
    ...
    def __init__(self, topology_settings: TopologySettings):
        ...
        self._publish_tp = (
            self._listeners is not None
            and self._listeners.enabled_for_topology
        )
        ...
        for seed in topology_settings.seeds:
            if self._publish_server:
                ...
                self._events.put(
                    (
                        self._listeners.publish_server_opened,
        ...
        if (
            self._settings.fqdn is not None
            and not self._settings.load_balanced
        ):
    ...
    async def _process_srv_update(
        self, seedlist: list[tuple[str, Any]]
    ) -> None:
    ...
    async def receive_cluster_time(
        self, cluster_time: Optional[Mapping[str, Any]]
    ) -> None:
        async with self._lock:
        ...
        for address, sd in self._description.server_descriptions().items():
            if address not in self._servers:
                ...
                if was_writable != sd.is_writable:
                    await self._servers[address].pool.update_is_writable(
                        sd.is_writable
                    )
...
def _is_stale_error_topology_version(
    current_tv: Optional[Mapping[str, Any]],
    error_tv: Optional[Mapping[str, Any]],
) -> bool:
...
def _filter_servers(
    ...
    filtered = [
        server for server in candidates if server not in deprioritized_servers
    ]
