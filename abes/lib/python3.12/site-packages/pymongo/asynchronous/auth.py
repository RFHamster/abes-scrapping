...
async def _authenticate_gssapi(
...
async def _authenticate_mongo_cr(
    credentials: MongoCredential, conn: AsyncConnection
) -> None:
...
async def _authenticate_default(
    credentials: MongoCredential, conn: AsyncConnection
) -> None:
    if conn.max_wire_version >= 7:
        if conn.negotiated_mechs:
            ...
            mechs = (
                await conn.command(source, cmd, publish_events=False)
            ).get('saslSupportedMechs', [])
        if 'SCRAM-SHA-256' in mechs:
            return await _authenticate_scram(
                credentials, conn, 'SCRAM-SHA-256'
            )
        else:
...
class _AuthContext:
    def __init__(
        self, credentials: MongoCredential, address: tuple[str, int]
    ) -> None:
...
_SPECULATIVE_AUTH_MAP: Mapping[str, Any] = {
    ...
    'SCRAM-SHA-256': functools.partial(
        _ScramContext, mechanism='SCRAM-SHA-256'
    ),
