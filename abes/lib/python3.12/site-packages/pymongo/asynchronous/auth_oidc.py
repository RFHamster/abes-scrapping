...
def _get_authenticator(
    ...
    credentials.cache.data = _OIDCAuthenticator(
        username=principal_name, properties=properties
    )
    return credentials.cache.data
        ...
        if not prev_token and cb is not None:
            with self.lock:
                ...
                if is_human:
                    ...
                    timeout = int(
                        remaining() or MACHINE_CALLBACK_TIMEOUT_SECONDS
                    )
                ...
                if not isinstance(resp, OIDCCallbackResult):
                    raise ValueError(
                        'Callback result must be of type OIDCCallbackResult'
                    )
