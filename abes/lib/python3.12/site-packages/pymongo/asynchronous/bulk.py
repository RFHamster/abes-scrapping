...
class _AsyncBulk:
        ...
        while run:
            ...
            while run.idx_offset < len(run.ops):
                ...
                if session:
                    ...
                    session._apply_to(
                        cmd, retryable, ReadPreference.PRIMARY, conn
                    )
                ...
                if write_concern.acknowledged:
                    result, to_send = await self._execute_batch(
                        bwc, cmd, ops, client
                    )
                    ...
                    to_send = await self._execute_batch_unack(
                        bwc, cmd, ops, client
                    )
    ...
    async def execute_no_results(
        ...
        if self.ordered:
            return await self.execute_command_no_results(
                conn, generator, write_concern
            )
    ...
    async def execute(
        ...
        if not write_concern.acknowledged:
            async with await client._conn_for_writes(
                session, operation
            ) as connection:
                await self.execute_no_results(
                    connection, generator, write_concern
                )
                return None
        else:
            return await self.execute_command(
                generator, write_concern, session, operation
            )
