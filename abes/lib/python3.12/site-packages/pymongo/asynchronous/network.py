...
async def command(
    ...
    if (
        client
        and client._encrypter
        and not client._encrypter._bypass_auto_encryption
    ):
        spec = orig = await client._encrypter.encrypt(
            dbname, spec, codec_options
        )
    ...
    if client and client._encrypter and reply:
        decrypted = await client._encrypter.decrypt(
...
async def receive_message(
    ...
    if op_code == 2012:
        ...
        data = decompress(
            await _receive_data_on_socket(conn, length - 25, deadline),
            compressor_id,
        )
    else:
...
async def wait_for_read(
    conn: AsyncConnection, deadline: Optional[float]
) -> None:
    ...
    while True:
        ...
        if hasattr(sock, 'pending') and sock.pending() > 0:
            ...
            readable = conn.socket_checker.select(
