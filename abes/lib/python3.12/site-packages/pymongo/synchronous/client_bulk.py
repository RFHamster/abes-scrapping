...
class _ClientBulk:
    ...
    def _execute_command(
        ...
        while self.idx_offset < self.total_ops:
            ...
            if write_concern.acknowledged:
                ...
                if result.get('error'):
                    ...
                    retryable_top_level_error = (
                        ...
                        and error.details.get('code', 0)
                        in _RETRYABLE_ERROR_CODES
                    )
                    retryable_network_error = isinstance(
                        error, ConnectionFailure
                    ) and not isinstance(
                        error, (NotPrimaryError, WaitQueueTimeoutError)
                    )

                    # Synthesize the full bulk result without modifying the
                    # current one because this write operation may be retried.
                    if retryable and (
                        retryable_top_level_error or retryable_network_error
                    ):
                        full = copy.deepcopy(full_result)
                        _merge_command(self.ops, self.idx_offset, full, result)
                        _throw_client_bulk_write_exception(
                            full, self.verbose_results
                        )
                    else:
                        _merge_command(
                            self.ops, self.idx_offset, full_result, result
                        )
                        _throw_client_bulk_write_exception(
                            full_result, self.verbose_results
                        )
                ...
                self._process_results_cursor(
                    full_result, result, conn, session
                )
            ...
            if full_result['error'] or (
                self.ordered and full_result['writeErrors']
            ):
                break
    ...
    def execute_command(
        ...
        if (
            full_result['error']
            or full_result['writeErrors']
            or full_result['writeConcernErrors']
        ):
            _throw_client_bulk_write_exception(
                full_result, self.verbose_results
            )
    ...
    def execute(
        ...
        if not self.write_concern.acknowledged:
            with self.client._conn_for_writes(
                session, operation
            ) as connection:
