...
class MongoClient(common.BaseObject, Generic[_DocumentType]):
    ...
    def _tmp_session(
        self,
        session: Optional[client_session.ClientSession],
        close: bool = True,
        ...
        if session is not None:
            if not isinstance(session, client_session.ClientSession):
                raise ValueError(
                    "'session' argument must be a ClientSession or None."
                )
    ...
    def drop_database(
        ...
        if not isinstance(name, str):
            raise TypeError(
                'name_or_database must be an instance of str or a Database'
            )
...
class _MongoClientErrorHandler:
    ...
    def __init__(
        ...
        if not isinstance(client, MongoClient):
            ...
            if not any(
                cls.__name__ == 'MongoClient' for cls in type(client).__mro__
            ):
                raise TypeError(
                    f'MongoClient required but given {type(client).__name__}'
                )
