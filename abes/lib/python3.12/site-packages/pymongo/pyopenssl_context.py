...
_VERIFY_MAP = {
    ...
    _stdlibssl.CERT_REQUIRED: _SSL.VERIFY_PEER
        ...
        while True:
            try:
                ...
                self.socket_checker.select(
                    self, want_read, want_write, timeout
                )
                if timeout and _time.monotonic() - start > timeout:
                    raise _socket.timeout('timed out') from None
                continue
...
class SSLContext:
    ...
    check_ocsp_endpoint = property(
        __get_check_ocsp_endpoint, __set_check_ocsp_endpoint
    )
