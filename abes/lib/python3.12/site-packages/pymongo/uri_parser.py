        ...
        if not port.isdigit():
            ...
            if all(c.isspace() or c.isdigit() for c in port):
                for c in port:
                    if c.isspace():
                        raise ValueError(
                            f'Port contains whitespace character: {c!r}'
                        )
...
def _handle_security_options(
    ...
    if tlsinsecure is not None:
        for opt in _IMPLICIT_TLSINSECURE_OPTS:
            if opt in options:
                err_msg = (
                    'URI options %s and %s cannot be specified simultaneously.'
                )
                raise InvalidURI(
                    err_msg
                    % (
                        options.cased_key('tlsinsecure'),
                        options.cased_key(opt),
                    )
                )
    ...
    if 'ssl' in options and 'tls' in options:
        ...
        if truth_value(options.get('ssl')) != truth_value(options.get('tls')):
            err_msg = (
                'Can not specify conflicting values for URI options %s and %s.'
            )
            raise InvalidURI(
        ...
        if optname in URI_OPTIONS_DEPRECATION_MAP:
            ...
            if mode == 'renamed':
                ...
                if newoptname in options:
                    warn_msg = (
                        "Deprecated option '%s' ignored in favor of '%s'."
                    )
                    warnings.warn(
                        warn_msg
                        % (
                            options.cased_key(optname),
...
def split_options(
    ...
    if validate:
        options = cast(
            _CaseInsensitiveDictionary, validate_options(options, warn)
        )
...
def parse_uri(
    ...
    if '/' in hosts:
        raise InvalidURI(
            "Any '/' in a unix domain socket must be percent-encoded: %s"
            % host_part
        )
    ...
    if is_srv:
        ...
        if port is not None:
            raise InvalidURI(
                f'{SRV_SCHEME} URIs must not include a port number'
            )
