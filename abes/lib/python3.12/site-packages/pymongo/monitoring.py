...
class ConnectionPoolListener(_EventListener):
    ...
    def connection_check_out_failed(
        self, event: ConnectionCheckOutFailedEvent
    ) -> None:
...
class TopologyListener(_EventListener):
    ...
    def description_changed(
        self, event: TopologyDescriptionChangedEvent
    ) -> None:
...
class ServerListener(_EventListener):
    ...
    def description_changed(
        self, event: ServerDescriptionChangedEvent
    ) -> None:
...
# authentication.
def _is_speculative_authenticate(
...
class CommandStartedEvent(_CommandEvent):
    ...
    def __init__(
        ...
        if cmd_name in _SENSITIVE_COMMANDS or _is_speculative_authenticate(
            cmd_name, command
        ):
...
class PoolCreatedEvent(_PoolEvent):
    ...
    def __repr__(self) -> str:
        return (
            f'{self.__class__.__name__}({self.address!r}, {self.__options!r})'
        )
...
class _ServerEvent:
    ...
    def __init__(
        self, server_address: _Address, topology_id: ObjectId
    ) -> None:
...
class _EventListeners:
    ...
    def publish_server_heartbeat_started(
        self, connection_id: _Address, awaited: bool
    ) -> None:
    ...
    def publish_server_heartbeat_succeeded(
        ...
        event = ServerHeartbeatSucceededEvent(
            duration, reply, connection_id, awaited
        )
        for subscriber in self.__server_heartbeat_listeners:
            try:
                subscriber.succeeded(event)
    ...
    def publish_topology_description_changed(
        ...
        event = TopologyDescriptionChangedEvent(
            previous_description, new_description, topology_id
        )
        for subscriber in self.__topology_listeners:
            try:
                subscriber.description_changed(event)
    ...
    def publish_connection_checked_in(
        self, address: _Address, connection_id: int
    ) -> None:
