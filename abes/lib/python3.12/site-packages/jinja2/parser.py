...
class Parser:
    ...
    def __init__(
        ...
        self.extensions: t.Dict[
            str,
            t.Callable[['Parser'], t.Union[nodes.Node, t.List[nodes.Node]]],
        ] = {}
    ...
    def parse_for(self) -> nodes.For:
        ...
        return nodes.For(
            target, iter, body, else_, test, recursive, lineno=lineno
        )
    ...
    def parse_filter_block(self) -> nodes.FilterBlock:
        ...
        node.body = self.parse_statements(
            ('name:endfilter',), drop_needle=True
        )
        return node
    ...
    def parse_assign_target(
        ...
        if not target.can_assign():
            self.fail(
                f"can't assign to {type(target).__name__.lower()!r}",
                target.lineno,
            )
    ...
    def parse_primary(self) -> nodes.Expr:
        ...
        if token.type == 'name':
            if token.value in ('true', 'false', 'True', 'False'):
                node = nodes.Const(
                    token.value in ('true', 'True'), lineno=token.lineno
                )
    ...
    def parse_call_args(
        ...
        def ensure(expr: bool) -> None:
            if not expr:
                self.fail(
                    'invalid syntax for function call expression', token.lineno
                )
            ...
            if self.stream.current.type == 'mul':
                ...
                if (
                    ...
                    # Parsing an arg
                        ...
                        dyn_args is None and dyn_kwargs is None and not kwargs
                    )
                    args.append(self.parse_expression())
    ...
    def parse_call(self, node: nodes.Expr) -> nodes.Call:
        ...
        return nodes.Call(
            node, args, kwargs, dyn_args, dyn_kwargs, lineno=token.lineno
        )
    ...
    def parse_test(self, node: nodes.Expr) -> nodes.Expr:
        ...
        if self.stream.current.type == 'lparen':
            ...
            'lbracket',
            'lbrace',
        } and not self.stream.current.test_any(
            ...
            while self.stream:
                ...
                if token.type == 'data':
                    if token.value:
                        add_data(
                            nodes.TemplateData(
                                token.value, lineno=token.lineno
                            )
                        )
                    next(self.stream)
                    ...
                    next(self.stream)
                    if (
                        end_tokens is not None
                        and self.stream.current.test_any(*end_tokens)
                    ):
                        return body
