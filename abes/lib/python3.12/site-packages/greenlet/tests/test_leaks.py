...
class TestLeaks(TestCase):
    ...
    def _check_issue251(
        ...
        greenlet.getcurrent()
        greenlets_before = self.count_objects(
            greenlet.greenlet, exact_kind=False
        )
        ...
        greenlets_after = self.count_objects(
            greenlet.greenlet, exact_kind=False
        )
        ...
        if (
            ...
            and greenlet._greenlet.get_clocks_used_doing_optional_cleanup()
            is not None
        ):
            ...
            if manually_collect_background:
                ...
                self.assertEqual(
                    HasFinalizerTracksInstances.EXTANT_INSTANCES, set()
                )
        else:
        ...
        if (
            greenlet._greenlet.get_clocks_used_doing_optional_cleanup()
            is not None
        ):
    ...
    def _check_untracked_memory_thread(self, deallocate_in_thread=True):
        ...
        self.assertLessEqual(
            uss_after, uss_before, 'after attempts %d' % (count,)
        )
