...
class TestGreenlet(TestCase):
    ...
    def _do_test_throw_to_dead_thread_doesnt_crash(
        ...
        self.expect_greenlet_leak = (
            True  # direct us not to wait for it to go away
        )
    ...
    def test_get_stack_with_nested_c_calls(self):
        ...
        def recurse(v):
            if v > 0:
                return v * _test_extension_cpp.test_call(
                    partial(recurse, v - 1)
                )
...
class TestGreenletSetParentErrors(TestCase):
    def test_threaded_reparent(self):
        ...
        self.assertEqual(
            str(exc.exception), 'parent cannot be on a different thread'
        )
    ...
    def test_unexpected_reparenting_thread_running(self):
        ...
        try:
            ...
            with self.assertRaises(greenlet.error) as exc:
                g.switch()
            self.assertIn(
                'Cannot switch to a different thread', str(exc.exception)
            )
    ...
    def test_trivial_cycle_main(self):
        ...
        self.assertEqual(
            str(exc.exception), 'cannot set the parent of a main greenlet'
        )
...
class TestBrokenGreenlets(TestCase):
    ...
    def test_failed_to_switch_into_running(self):
        ...
        with self.assertRaisesRegex(
            SystemError, 'Failed to switch stacks into a running greenlet.'
        ):
            g.switch()
    ...
    def test_reentrant_switch_three_greenlets(self):
        ...
        ex = self.assertScriptRaises(
            'fail_switch_three_greenlets.py', exitcodes=(1,)
        )
