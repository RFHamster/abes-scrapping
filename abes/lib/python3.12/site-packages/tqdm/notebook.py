...
class tqdm_notebook(std_tqdm):
    ...
    def display(
        ...
        if not msg and not close:
            ...
            # remove {bar}
            d['bar_format'] = (
                d['bar_format'] or '{l_bar}<bar/>{r_bar}'
            ).replace('{bar}', '<bar/>')
        ...
        if msg:
            ...
            if '<bar/>' in msg:
                left, right = map(
                    escape, re.split(r'\|?<bar/>\|?', msg, maxsplit=1)
                )
            else:
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Get bar width
        self.ncols = (
            '100%' if self.dynamic_ncols else kwargs.get('ncols', None)
        )
    ...
    def reset(self, total=None):
        ...
        if total is not None:
            pbar.max = total
            if (
                not self.total and self.ncols is None
            ):  # no longer unknown total
