...
class DummyTqdmFile(ObjectWrapper):
    ...
    def __del__(self):
        if self._buf:
            ...
            try:
                tqdm.write(
                    blank.join(self._buf), end=blank, file=self._wrapped
                )
...
def tenumerate(
    iterable, start=0, total=None, tqdm_class=tqdm_auto, **tqdm_kwargs
):
    """
    ...
    try:
        ...
        if isinstance(iterable, np.ndarray):
            return tqdm_class(
                np.ndenumerate(iterable),
                total=total or iterable.size,
                **tqdm_kwargs
            )
