...
class TqdmDefaultWriteLock(object):
    ...
    def __init__(self):
        ...
        cls.create_mp_lock()
        self.locks = [
            lk for lk in [cls.mp_lock, cls.th_lock] if lk is not None
        ]
        if root_lock is not None:
            root_lock.release()
    ...
    def create_th_lock(cls):
        ...
        warn(
            'create_th_lock not needed anymore',
            TqdmDeprecationWarning,
            stacklevel=2,
        )
...
class Bar(object):
    ...
    def __format__(self, format_spec):
        if format_spec:
            ...
            try:
                charset = {'a': self.ASCII, 'u': self.UTF, 'b': self.BLANK}[
                    _type
                ]
            except KeyError:
...
class EMA(object):
    ...
    def __call__(self, x=None):
        ...
        if x is not None:
            ...
            self.calls += 1
        return (
            self.last / (1 - beta**self.calls) if self.calls else self.last
        )
...
class tqdm(Comparable):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            (ncols is None or nrows is None)
            and (file in (sys.stderr, sys.stdout))
        ...
        with self._lock:
            ...
            self.pos = (
                self._get_free_pos(self) if position is None else -position
            )
    ...
    def __contains__(self, item):
        ...
        return (
            contains(item) if contains is not None else item in self.__iter__()
        )
            ...
            if dt >= self.mininterval and cur_t >= self.start_t + self.delay:
                ...
                if self.dynamic_miniters:
                    ...
                    if self.maxinterval and dt >= self.maxinterval:
                        self.miniters = (
                            dn * (self.mininterval or self.maxinterval) / dt
                        )
                    elif self.smoothing:
                        # EMA miniters update
