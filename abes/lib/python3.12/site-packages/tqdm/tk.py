...
class tqdm_tk(std_tqdm):  # pragma: no cover
    ...
    def __init__(self, *args, **kwargs):
        ...
        warn(
            'GUI is experimental/alpha', TqdmExperimentalWarning, stacklevel=2
        )
        ...
        self._tk_window.after(
            0, lambda: self._tk_window.wm_attributes('-topmost', 0)
        )
        ...
        if self._cancel_callback is not None:
            _tk_button = ttk.Button(
                pbar_frame, text='Cancel', command=self.cancel
            )
            _tk_button.pack()
        if grab:
