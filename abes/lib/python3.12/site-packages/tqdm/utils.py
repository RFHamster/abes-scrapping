...
IS_NIX = any(
    CUR_OS.startswith(i) for i in ['aix', 'linux', 'darwin', 'freebsd']
)
    ...
    def __init__(self, wrapped, tqdm_instance):
        ...
        if hasattr(wrapped, 'write'):
            self.wrapper_setattr(
                'write',
                self.disable_on_exception(tqdm_instance, wrapped.write),
            )
        if hasattr(wrapped, 'flush'):
            self.wrapper_setattr(
                'flush',
                self.disable_on_exception(tqdm_instance, wrapped.flush),
            )
...
def _screen_shape_tput(*_):  # pragma: no cover
    ...
    try:
        ...
        return [
            int(check_call(shlex.split('tput ' + i))) - 1
            for i in ('cols', 'lines')
        ]
    except Exception:  # nosec
        pass
