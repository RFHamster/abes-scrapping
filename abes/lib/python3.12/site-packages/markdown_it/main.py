...
class MarkdownIt:
    ...
    def configure(
        ...
        if isinstance(presets, str):
            if presets not in _PRESETS:
                raise KeyError(
                    f"Wrong `markdown-it` preset '{presets}', check name"
                )
    ...
    def enable(
        ...
        if missed and not ignoreInvalid:
            raise ValueError(
                f'MarkdownIt. Failed to enable unknown rule(s): {missed}'
            )
    ...
    def disable(
        ...
        if missed and not ignoreInvalid:
            raise ValueError(
                f'MarkdownIt. Failed to disable unknown rule(s): {missed}'
            )
        return self
    ...
    def parse(self, src: str, env: EnvType | None = None) -> list[Token]:
        ...
        if not isinstance(env, MutableMapping):
            raise TypeError(
                f'Input data should be a MutableMapping, not {type(env)}'
            )
    ...
    def parseInline(self, src: str, env: EnvType | None = None) -> list[Token]:
        ...
        if not isinstance(env, MutableMapping):
            raise TypeError(
                f'Input data should be an MutableMapping, not {type(env)}'
            )
        if not isinstance(src, str):
            raise TypeError(f'Input data should be a string, not {type(src)}')
        state = StateCore(src, self, env)
    ...
    def renderInline(self, src: str, env: EnvType | None = None) -> Any:
        ...
        return self.renderer.render(
            self.parseInline(src, env), self.options, env
        )
