    ...
    try:
        for changes in watch(
            *paths,
            ...
            process.stop(
                sigint_timeout=sigint_timeout, sigkill_timeout=sigkill_timeout
            )
...
def start_process(
    ...
    if target_type == 'function':
        ...
        process = spawn_context.Process(
            target=target_, args=args, kwargs=kwargs
        )
        process.start()
    else:
        if args or kwargs:
        ...
        assert isinstance(
            target, str
        ), 'target must be a string to run as a command'
    ...
    def stop(self, sigint_timeout: int = 5, sigkill_timeout: int = 1) -> None:
        ...
        if self.is_alive():
            ...
            try:
                ...
                logger.warning(
                    'SIGINT timed out after %r seconds', sigint_timeout
                )
                pass
            ...
            logger.warning(
                'process already dead, exit code: %d', self.exitcode
            )
