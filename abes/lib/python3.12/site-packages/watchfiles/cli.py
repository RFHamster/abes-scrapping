...
def cli(*args_: str) -> None:
    ...
    parser.add_argument(
        'target', help='Command or dotted function path to run'
    )
    parser.add_argument(
        'paths',
        nargs='*',
    ...
    if target_type == 'function':
        logger.debug(
            'target_type=function, attempting import of "%s"',
...
def build_filter(
    ...
    if isinstance(watch_filter_cls, type) and issubclass(
        ...
        return (
            watch_filter_cls(ignore_paths=ignore_paths),
            watch_filter_cls.__name__,
        )
