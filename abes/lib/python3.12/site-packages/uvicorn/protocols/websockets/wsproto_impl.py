...
class WSProtocol(asyncio.Protocol):
    ...
    def connection_made(  # type: ignore[override]
        ...
        if self.logger.level <= TRACE_LOG_LEVEL:
            ...
            self.logger.log(
                TRACE_LOG_LEVEL, '%sWebSocket connection made', prefix
            )
    ...
    def connection_lost(self, exc: Exception | None) -> None:
        ...
        if self.logger.level <= TRACE_LOG_LEVEL:
            prefix = '%s:%d - ' % self.client if self.client else ''
            self.logger.log(
                TRACE_LOG_LEVEL, '%sWebSocket connection lost', prefix
            )
    ...
    def handle_connect(self, event: events.Request) -> None:
        ...
        full_raw_path = self.root_path.encode('ascii') + raw_path.encode(
            'ascii'
        )
    ...
    def handle_text(self, event: events.TextMessage) -> None:
        ...
        if event.message_finished:
            self.queue.put_nowait(
                {'type': 'websocket.receive', 'text': self.text}
            )
            self.text = ''
    ...
    def handle_bytes(self, event: events.BytesMessage) -> None:
        ...
        if event.message_finished:
            self.queue.put_nowait(
                {'type': 'websocket.receive', 'bytes': self.bytes}
            )
            self.bytes = b''
            if not self.read_paused:
                self.read_paused = True
