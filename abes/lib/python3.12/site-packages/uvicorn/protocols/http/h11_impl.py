...
STATUS_PHRASES = {
    status_code: _get_status_phrase(status_code)
...
class H11Protocol(asyncio.Protocol):
    ...
    def handle_websocket_upgrade(self, event: h11.Request) -> None:
        if self.logger.level <= TRACE_LOG_LEVEL:  # pragma: full coverage
            prefix = '%s:%d - ' % self.client if self.client else ''
            self.logger.log(
                TRACE_LOG_LEVEL, '%sUpgrading to WebSocket', prefix
            )
    ...
    def on_response_complete(self) -> None:
        ...
        if (
            self.conn.our_state is h11.DONE
            and self.conn.their_state is h11.DONE
        ):
            self.conn.start_next_cycle()
            self.handle_events()
...
class RequestResponseCycle:
    ...
    async def send(self, message: ASGISendEvent) -> None:
        ...
        if not self.response_started:
            ...
            if (
                CLOSE_HEADER in self.scope['headers']
                and CLOSE_HEADER not in headers
            ):
            ...
            response = h11.Response(
                status_code=status, headers=headers, reason=reason
            )
