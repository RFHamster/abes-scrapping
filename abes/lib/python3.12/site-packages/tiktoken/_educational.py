...
class SimpleBytePairEncoding:
    def __init__(
        self, *, pat_str: str, mergeable_ranks: dict[bytes, int]
    ) -> None:
        ...
        self._decoder = {
            token: token_bytes
            for token_bytes, token in mergeable_ranks.items()
        }
    ...
    def train(training_data: str, vocab_size: int, pat_str: str):
        ...
        return SimpleBytePairEncoding(
            pat_str=pat_str, mergeable_ranks=mergeable_ranks
        )
    ...
    while len(ranks) < vocab_size:
        ...
        if visualise:
            ...
            if visualise in ['colour', 'color']:
                ...
                visualise_tokens(
                    [token for word in words[:50] for token in word]
                )
            elif visualise == 'simple':
                print(
                    'Now the first twenty words in our training data look like:'
                )
...
def visualise_tokens(token_values: list[bytes]) -> None:
    background = [
        f'\u001b[48;5;{i}m' for i in [167, 179, 185, 77, 80, 68, 134]
    ]
...
def train_simple_encoding():
    ...
    enc = SimpleBytePairEncoding.train(
        data, vocab_size=600, pat_str=gpt2_pattern
    )
