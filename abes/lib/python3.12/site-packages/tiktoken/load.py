...
def read_file(blobpath: str) -> bytes:
    if not blobpath.startswith('http://') and not blobpath.startswith(
        'https://'
    ):
        try:
            import blobfile
...
def data_gym_to_mergeable_bpe_ranks(
    ...
    encoder_json = json.loads(
        read_file_cached(encoder_json_file, encoder_json_hash)
    )
    encoder_json_loaded = {
        decode_data_gym(k): v for k, v in encoder_json.items()
    }
...
def dump_tiktoken_bpe(
    bpe_ranks: dict[bytes, int], tiktoken_bpe_file: str
) -> None:
    try:
        import blobfile
    except ImportError as e:
