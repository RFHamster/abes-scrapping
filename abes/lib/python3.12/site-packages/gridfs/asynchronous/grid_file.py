    ...
    def __init__(self, database: AsyncDatabase, collection: str = 'fs'):
        ...
        if not database.write_concern.acknowledged:
            raise ConfigurationError(
                'database must use acknowledged write_concern'
            )
    ...
    async def get_version(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            return AsyncGridOut(
                self._collection, file_document=doc, session=session
            )
    ...
    async def exists(
        ...
        **kwargs: Any,
        ...
        if kwargs:
            ...
            f = await self._files.find_one(
                document_or_id, ['_id'], session=session
            )
...
class AsyncGridFSBucket:
    ...
    async def download_to_stream(
        ...
        .. versionchanged:: 3.6
           Added ``session`` parameter.
        """
        async with await self.open_download_stream(
...
class AsyncGridIn:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not isinstance(root_collection, AsyncCollection):
            raise TypeError(
                'root_collection must be an instance of AsyncCollection'
            )
    ...
    async def _create_index(
        ...
        doc = await collection.find_one(
            projection={'_id': 1}, session=self._session
        )
        if doc is None:
            try:
                index_keys = [
                    index_spec['key']
                    async for index_spec in await collection.list_indexes(
        ...
        if (
            ...
            try:
                await self._chunks.insert_many(
                    self._buffered_docs, session=self._session
                )
    ...
    async def _flush(self) -> Any:
        ...
        try:
            ...
            self._file['uploadDate'] = datetime.datetime.now(
                tz=datetime.timezone.utc
            )

            return await self._coll.files.insert_one(
...
class AsyncGridOut(GRIDOUT_BASE_CLASS):  # type: ignore
    ...
    _id: Any = _a_grid_out_property(
        '_id', "The ``'_id'`` value for this file."
    )
    ...
    length: int = _a_grid_out_property(
        'length', 'Length (in bytes) of this file.'
    )
    chunk_size: int = _a_grid_out_property(
        'chunkSize', 'Chunk size for this file.'
    )
    ...
    async def open(self) -> None:
        if not self._file:
            ...
            self._file = await self._files.find_one(
                {'_id': self._file_id}, session=self._session
            )
            if not self._file:
                raise NoFile(
    ...
    def __getattr__(self, name: str) -> Any:
        if _IS_SYNC:
            ...
            raise InvalidOperation(
                'You must call AsyncGridOut.open() before accessing the %s property'
                % name
            )
    ...
    async def _read_size_or_line(
        self, size: int = -1, line: bool = False
    ) -> bytes:
        ...
        while received < size:
            ...
            if line:
                pos = (
                    buf.find(NEWLN, chunk_start, chunk_start + needed)
                    - chunk_start
                )
                if pos >= 0:
...
class _AsyncGridOutChunkIterator:
    ...
    def _create_cursor(self) -> None:
        ...
        _disallow_transactions(self._session)
        self._cursor = self._chunks.find(
            filter, sort=[('n', 1)], session=self._session
        )
