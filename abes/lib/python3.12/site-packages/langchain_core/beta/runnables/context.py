...
def _config_with_context(
    ...
    if any(
        k.startswith(CONTEXT_CONFIG_PREFIX)
        for k in config.get('configurable', {})
    ):
        return config
    ...
    deps_by_key = {
        key: {
            _key_from_id(dep)
            for spec in group
            for dep in (spec[0].dependencies or [])
        }
    ...
    values: Values = {}
    events: defaultdict[
        str, Union[asyncio.Event, threading.Event]
    ] = defaultdict(event_cls)
    context_funcs: dict[str, Callable[[], Any]] = {}
    for key, group in grouped_by_key.items():
        getters = [
            s for s in group if s[0].id.endswith(CONTEXT_CONFIG_SUFFIX_GET)
        ]
        setters = [
            s for s in group if s[0].id.endswith(CONTEXT_CONFIG_SUFFIX_SET)
        ]
        ...
        if getters:
            context_funcs[getters[0][0].id] = partial(
                getter, events[key], values
            )
        ...
        **kwargs: Any,
        ...
        if isinstance(self.key, list):
            return {
                ...
                for key, id_ in zip(self.key, self.ids)
            }
        else:
        ...
        **kwargs: Any,
        ...
        if isinstance(self.key, list):
            values = await asyncio.gather(
                *(configurable[id_]() for id_ in self.ids)
            )
