...
class VectorStore(ABC):
    ...
    async def aadd_documents(
        ...
        return await run_in_executor(
            None, self.add_documents, documents, **kwargs
        )
    ...
    def search(
        ...
        if search_type == 'similarity':
            ...
            docs_and_similarities = (
                self.similarity_search_with_relevance_scores(query, **kwargs)
            )
    ...
    async def asearch(
        ...
        if search_type == 'similarity':
            ...
            docs_and_similarities = (
                await self.asimilarity_search_with_relevance_scores(
                    query, **kwargs
                )
            )
            return [doc for doc, _ in docs_and_similarities]
    ...
    async def asimilarity_search(
        ...
        # asynchronous in the vector store implementations.
        return await run_in_executor(
            None, self.similarity_search, query, k=k, **kwargs
        )
...
class VectorStoreRetriever(BaseRetriever):
    ...
    def validate_search_type(cls, values: dict) -> Any:
        ...
        if search_type == 'similarity_score_threshold':
            score_threshold = values.get('search_kwargs', {}).get(
                'score_threshold'
            )
    ...
    def _get_ls_params(self, **kwargs: Any) -> LangSmithRetrieverParams:
        ...
        ] = self.vectorstore.__class__.__name__
