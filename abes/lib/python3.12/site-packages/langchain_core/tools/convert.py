...
def tool(
    ...
    *args: Any,
    ...
    parse_docstring: bool = False,
    error_on_invalid_docstring: bool = True,
) -> Union[BaseTool, Callable[[Union[Callable, Runnable]], BaseTool],]:
    ...
    def _create_tool_factory(
        ...
        def _tool_factory(dec_func: Union[Callable, Runnable]) -> BaseTool:
            if isinstance(dec_func, Runnable):
                ...
                if (
                    runnable.input_schema.model_json_schema().get('type')
                    != 'object'
                ):
    ...
    if runnable is not None:
        ...
        if callable(name_or_callable) and hasattr(
            name_or_callable, '__name__'
        ):
            # Used as a decorator without parameters
            # @tool
            # def my_tool():
            #    pass
            return _create_tool_factory(name_or_callable.__name__)(
                name_or_callable
            )
...
def _get_schema_from_runnable_and_arg_types(
    ...
    fields = {
        key: (key_type, Field(...)) for key, key_type in arg_types.items()
    }
