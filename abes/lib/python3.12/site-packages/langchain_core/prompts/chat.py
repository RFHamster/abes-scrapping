...
class BaseStringMessagePromptTemplate(BaseMessagePromptTemplate, ABC):
    ...
    def pretty_repr(self, html: bool = False) -> str:
        ...
        title = self.__class__.__name__.replace(
            'MessagePromptTemplate', ' Message'
        )
...
class _StringImageMessagePromptTemplate(BaseMessagePromptTemplate):
    ...
    prompt: Union[
        StringPromptTemplate,
        list[Union[StringPromptTemplate, ImagePromptTemplate]],
    ]
    """Prompt template."""
    additional_kwargs: dict = Field(default_factory=dict)
    """Additional keyword arguments to pass to the prompt template."""
    ...
    def from_template(
        ...
        template: Union[
            str, list[Union[str, _TextTemplateParam, _ImageTemplateParam]]
        ],
        template_format: PromptTemplateFormat = 'f-string',
        *,
        partial_variables: Optional[dict[str, Any]] = None,
    ...
    def from_template_file(
        ...
        **kwargs: Any,
        ...
        return cls.from_template(
            template, input_variables=input_variables, **kwargs
        )
        ...
        if isinstance(self.prompt, StringPromptTemplate):
            ...
            for prompt in self.prompt:
                ...
                if isinstance(prompt, StringPromptTemplate):
                    ...
                    content.append(
                        {'type': 'image_url', 'image_url': formatted}
                    )
            return self._msg_class(
    ...
    async def aformat(self, **kwargs: Any) -> BaseMessage:
        ...
        if isinstance(self.prompt, StringPromptTemplate):
            ...
            for prompt in self.prompt:
                inputs = {var: kwargs[var] for var in prompt.input_variables}
                if isinstance(prompt, StringPromptTemplate):
                    formatted: Union[str, ImageURL] = await prompt.aformat(
                        **inputs
                    )
                    content.append({'type': 'text', 'text': formatted})
                elif isinstance(prompt, ImagePromptTemplate):
    ...
    def pretty_repr(self, html: bool = False) -> str:
        ...
        prompt_reprs = '\n\n'.join(
            prompt.pretty_repr(html=html) for prompt in prompts
        )
        return f'{title}\n\n{prompt_reprs}'
...
class ChatPromptTemplate(BaseChatPromptTemplate):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        """
        _messages = [
            _convert_to_message(message, template_format)
        ...
        cast(type[ChatPromptTemplate], super()).__init__(
    ...
    def validate_input_variables(cls, values: dict) -> Any:
        ...
        for message in messages:
            if isinstance(
                message, (BaseMessagePromptTemplate, BaseChatPromptTemplate)
            ):
            ...
            if isinstance(message, MessagesPlaceholder):
                ...
                if (
                    message.optional
                    and message.variable_name
                    not in values['partial_variables']
                ):
    ...
    if message_type in ('human', 'user'):
        ...
        if isinstance(template, str):
            ...
            message = MessagesPlaceholder(
                variable_name=var_name, optional=True
            )
            ...
            message = MessagesPlaceholder(
                variable_name=var_name, optional=is_optional
            )
        else:
            msg = (
