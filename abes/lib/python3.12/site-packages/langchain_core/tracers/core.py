...
class _TracerCore(ABC):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._schema_format = (
            _schema_format  # For internal use only API will change.
        )
    ...
    def _start_trace(self, run: Run) -> Union[None, Coroutine[Any, Any, None]]:  # type: ignore[return]
        current_dotted_order = run.start_time.strftime(
            '%Y%m%dT%H%M%S%fZ'
        ) + str(run.id)
    ...
    def _create_chat_model_run(
        ...
        **kwargs: Any,
        ...
        return Run(
            ...
            inputs={
                'messages': [
                    [dumpd(msg) for msg in batch] for batch in messages
                ]
            },
            extra=kwargs,
    ...
    def _complete_retrieval_run(
        ...
        **kwargs: Any,
        ...
        retrieval_run.events.append(
            {'name': 'end', 'time': retrieval_run.end_time}
        )
    ...
    def _errored_retrieval_run(
        ...
        **kwargs: Any,
        ...
        retrieval_run.end_time = datetime.now(timezone.utc)
        retrieval_run.events.append(
            {'name': 'error', 'time': retrieval_run.end_time}
        )
        return retrieval_run
