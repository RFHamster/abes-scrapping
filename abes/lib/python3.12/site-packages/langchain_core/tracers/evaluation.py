    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if max_concurrency is None:
            ...
            weakref.finalize(
                self,
                lambda: cast(ThreadPoolExecutor, self.executor).shutdown(
                    wait=True
                ),
            )
        else:
        ...
        self.logged_eval_results: dict[
            tuple[str, str], list[EvaluationResult]
        ] = {}
        self.lock = threading.Lock()
        global _TRACERS
        _TRACERS.add(self)

    def _evaluate_in_project(
        self, run: Run, evaluator: langsmith.RunEvaluator
    ) -> None:
        ...
        with self.lock:
            for res in eval_results:
                ...
                self.logged_eval_results.setdefault(
                    (run_id, example_id), []
                ).append(res)
