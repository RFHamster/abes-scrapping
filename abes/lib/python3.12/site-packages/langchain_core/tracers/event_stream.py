...
def _assign_name(
    name: Optional[str], serialized: Optional[dict[str, Any]]
) -> str:
...
class _AstreamEventsCallbackHandler(
    AsyncCallbackHandler, _StreamingCallbackHandler
):
    ...
    async def tap_output_aiter(
        ...
        if tap is sentinel:
            ...
            self._send(
                {**event, 'data': {'chunk': first}}, run_info['run_type']
            )
            yield cast(T, first)
    ...
    async def on_llm_end(
        ...
        generations: Union[
            list[list[GenerationChunk]], list[list[ChatGenerationChunk]]
        ]
        output: Union[dict, BaseMessage] = {}

        if run_info['run_type'] == 'chat_model':
            generations = cast(
                list[list[ChatGenerationChunk]], response.generations
            )
            ...
            generations = cast(
                list[list[GenerationChunk]], response.generations
            )
            output = {
                'generations': [
                    [
        ...
        **kwargs,
        ...
        for path in paths:
            ...
            if log_entry['end_time'] is None:
                event_type = (
                    'stream' if log_entry['streamed_output'] else 'start'
                )
            else:
                event_type = 'end'
    ...
    **kwargs: Any,
    ...
    async def consume_astream() -> None:
        try:
            ...
            async with aclosing(
                runnable.astream(input, config, **kwargs)
            ) as stream:
