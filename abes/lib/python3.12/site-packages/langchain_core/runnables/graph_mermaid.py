    ...
    *,
    ...
    if with_styles:
        ...
        for key, node in nodes.items():
            ...
            node_label = format_dict.get(
                key, format_dict[default_class_label]
            ).format(_escape_node_label(key), label)
    ...
    def add_subgraph(edges: list[Edge], prefix: str) -> None:
        ...
        for nested_prefix in edge_groups:
            if (
                not nested_prefix.startswith(prefix + ':')
                or nested_prefix == prefix
            ):
                continue
    ...
    if with_styles:
        mermaid_graph += _generate_mermaid_graph_styles(
            node_styles or NodeStyles()
        )
...
def _render_mermaid_using_api(
    ...
    mermaid_syntax_encoded = base64.b64encode(
