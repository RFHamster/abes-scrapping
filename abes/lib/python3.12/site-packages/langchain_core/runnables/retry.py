...
if TYPE_CHECKING:
    ...
    T = TypeVar(
        'T', CallbackManagerForChainRun, AsyncCallbackManagerForChainRun
    )
U = TypeVar('U')
...
class RunnableRetry(RunnableBindingBase[Input, Output]):
    ...
    def _kwargs_retrying(self) -> dict[str, Any]:
        ...
        if self.retry_exception_types:
            kwargs['retry'] = retry_if_exception_type(
                self.retry_exception_types
            )
    ...
    def _patch_config_list(
        ...
        return [
            ...
            for c, rm in zip(config, run_manager)
        ]
        ...
        for attempt in self._sync_retrying(reraise=True):
            with attempt:
                result = super().invoke(
                    input,
                    self._patch_config(
                        config, run_manager, attempt.retry_state
                    ),
                    **kwargs,
                )
            if (
