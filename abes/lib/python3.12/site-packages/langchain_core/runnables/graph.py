...
class Graph:
    ...
    def add_node(
        ...
        *,
        ...
        node = Node(
            id=id, data=data, metadata=metadata, name=node_data_str(id, data)
        )
    ...
    def extend(
        ...
        self.nodes.update(
            {
                prefixed(k): v.copy(id=prefixed(k))
        ...
        self.edges.extend(
            [
                edge.copy(
                    source=prefixed(edge.source), target=prefixed(edge.target)
                )
...
def _first_node(graph: Graph, exclude: Sequence[str] = ()) -> Optional[Node]:
    ...
    When drawing the graph, this node would be the origin."""
    targets = {
        edge.target for edge in graph.edges if edge.source not in exclude
    }
    found: list[Node] = []
...
def _last_node(graph: Graph, exclude: Sequence[str] = ()) -> Optional[Node]:
    ...
    When drawing the graph, this node would be the destination."""
    sources = {
        edge.source for edge in graph.edges if edge.target not in exclude
    }
    found: list[Node] = []
    for node in graph.nodes.values():
