        ...
        if dx == 0 and dy == 0:
            ...
            for x in range(x0, x1 + 1):
                y = (
                    y0
                    if dx == 0
                    else y0 + int(round((x - x0) * dy / float(dx)))
                )
                self.point(x, y, char)
        elif y0 < y1:
            for y in range(y0, y1 + 1):
                x = (
                    x0
                    if dy == 0
                    else x0 + int(round((y - y0) * dx / float(dy)))
                )
                self.point(x, y, char)
        else:
            for y in range(y1, y0 + 1):
                x = (
                    x0
                    if dy == 0
                    else x1 + int(round((y - y1) * dx / float(dy)))
                )
                self.point(x, y, char)
...
def _build_sugiyama_layout(
    ...
    edges_ = [
        Edge(vertices_[s], vertices_[e], data=cond) for s, e, _, cond in edges
    ]
...
def draw_ascii(vertices: Mapping[str, str], edges: Sequence[LangEdge]) -> str:
    ...
    for edge in sug.g.sE:
        ...
        for index in range(1, len(edge.view._pts)):
            ...
            canvas.line(
                start_x, start_y, end_x, end_y, '.' if edge.data else '*'
            )
