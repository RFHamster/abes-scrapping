...
class RunnableConfigurableFields(DynamicRunnable[Input, Output]):
    ...
    def _prepare(
        ...
        specs_by_id = {
            spec.id: (key, spec) for key, spec in self.fields.items()
        }
        configurable_fields = {
            specs_by_id[k][0]: v
            for k, v in config.get('configurable', {}).items()
            if k in specs_by_id
            and isinstance(specs_by_id[k][1], ConfigurableField)
        }
        configurable_single_options = {
            k: v.options[
                (config.get('configurable', {}).get(v.id) or v.default)
            ]
...
class RunnableConfigurableAlternatives(DynamicRunnable[Input, Output]):
    ...
    def config_specs(self) -> list[ConfigurableFieldSpec]:
        with _enums_for_spec_lock:
            if which_enum := _enums_for_spec.get(self.which):
                ...
                which_enum = StrEnum(  # type: ignore[call-overload]
                    ...
                    (
                        (v, v)
                        for v in list(self.alternatives.keys())
                        + [self.default_key]
                    ),
                )
        ...
        return get_unique_config_specs(
            ...
            + (
                [
                    prefix_config_spec(
                        s, f'{self.which.id}=={self.default_key}'
                    )
    ...
    def _prepare(
        ...
        config = ensure_config(config)
        which = config.get('configurable', {}).get(
            self.which.id, self.default_key
        )
