    ...
    **RunnableParallel** invokes runnables concurrently, providing the same input
    ...
    def get_graph(self, config: Optional[RunnableConfig] = None) -> Graph:
        ...
        try:
            ...
            input_node = graph.add_node(
                create_model_v2(self.get_name('Input'))
            )
        ...
        try:
            ...
            output_node = graph.add_node(
                create_model_v2(self.get_name('Output'))
            )
    ...
    def __or__(
        ...
        other: Union[
            ...
            Mapping[
                str, Union[Runnable[Any, Other], Callable[[Any], Other], Any]
            ],
        ],
    ...
    def __ror__(
        ...
        other: Union[
            ...
            Mapping[
                str, Union[Runnable[Other, Any], Callable[[Other], Any], Any]
            ],
        ],
    ...
    async def ainvoke(
        ...
        **kwargs: Any,
        ...
        return await run_in_executor(
            config, self.invoke, input, config, **kwargs
        )
    ...
    async def _acall_with_config(
        ...
        func: Union[
            ...
            Callable[
                [Input, AsyncCallbackManagerForChainRun], Awaitable[Output]
            ],
            Callable[
    ...
    def _batch_with_config(
        ...
        **kwargs: Optional[Any],
        ...
        callback_managers = [
            get_callback_manager_for_config(c) for c in configs
        ]
        run_managers = [
    ...
    async def _abatch_with_config(
        ...
        **kwargs: Optional[Any],
        ...
        configs = get_config_list(config, len(input))
        callback_managers = [
            get_async_callback_manager_for_config(c) for c in configs
        ]
        run_managers: list[
            AsyncCallbackManagerForChainRun
    ...
    def _transform_stream_with_config(
        ...
        transformer: Union[
            ...
            Callable[
                [Iterator[Input], CallbackManagerForChainRun], Iterator[Output]
            ],
            Callable[
                [
                    Iterator[Input],
    ...
    if len(steps) == 1:
        ...
        if not issubclass(prev_output_schema, RootModel):
            ...
            if isinstance(last.keys, list):
                ...
                return create_model_v2(  # type: ignore[call-overload]
                    'RunnableSequenceOutput',
                    root=(field.annotation, field.default),
                )
    ...
    **RunnableSequence** is the most important composition operator in LangChain
    ...
    def config_specs(self) -> list[ConfigurableFieldSpec]:
        ...
        specs_by_pos = groupby(
            [
                tup
                for tup in all_specs
                if tup[0].id.startswith(CONTEXT_CONFIG_PREFIX)
            ],
            lambda x: x[1],
        )
                ...
                for stepidx, step in enumerate(self.steps):
                    ...
                    inputs = step.batch(
                        ...
                        [
                            ...
                            patch_config(
                                config,
                                callbacks=rm.get_child(
                                    f'seq:step:{stepidx+1}'
                                ),
                            )
                            for i, (rm, config) in enumerate(
                                zip(run_managers, configs)
                            )
                            if i not in failed_inputs_map
                        ],
                    ...
                    inputs = [
                        inp for inp in inputs if not isinstance(inp, Exception)
                    ]
                ...
                for i, step in enumerate(self.steps):
                    inputs = step.batch(
                        ...
                        [
                            ...
                            patch_config(
                                config,
                                callbacks=rm.get_child(f'seq:step:{i+1}'),
                            )
    ...
    async def abatch(
        ...
        **kwargs: Optional[Any],
        ...
        # finish the root runs
            ...
            await asyncio.gather(
                *(rm.on_chain_error(e) for rm in run_managers)
            )
            if return_exceptions:
                return cast(list[Output], [e for _ in inputs])
            else:
    ...
    def transform(
        ...
        **kwargs: Optional[Any],
        ...
        yield from self._transform_stream_with_config(
            ...
            self._transform,
            patch_config(
                config, run_name=(config or {}).get('run_name') or self.name
            ),
            **kwargs,
        )
...
class RunnableParallel(RunnableSerializable[Input, dict[str, Any]]):
        ...
        steps__: Optional[
            Mapping[
                ...
                Union[
                    ...
                    Mapping[
                        str,
                        Union[Runnable[Input, Any], Callable[[Input], Any]],
                    ],
                ],
            ]
        ] = None,
    ...
    def get_name(
        ...
        name = (
            name
            or self.name
            or f"RunnableParallel<{','.join(self.steps__.keys())}>"
        )
        ...
        if all(
            ...
            return create_model_v2(  # type: ignore[call-overload]
                ...
                field_definitions={
                    k: (v.annotation, v.default)
                    for step in self.steps__.values()
                    for k, v in step.get_input_schema(
                        config
                    ).model_fields.items()
                    if k != '__root__'
                },
    ...
    def get_output_schema(
        ...
        return create_model_v2(
            self.get_name('Output'), field_definitions=fields
        )
        ...
        async def _ainvoke_step(
            ...
            if asyncio_accepts_context():
                ...
                return await asyncio.create_task(
                    step.ainvoke(input, child_config)
                )
            ...
            named_generators = [
                (
                    ...
                    step.transform(
                        ...
                        patch_config(
                            config,
                            callbacks=run_manager.get_child(f'map:key:{name}'),
                        ),
                    ),
                )
    ...
    async def _atransform(
        ...
        # Start the first iteration of each generator
        tasks = {
            asyncio.create_task(get_next_chunk(generator)): (
                step_name,
                generator,
            )
            for step_name, generator in named_generators
...
class RunnableGenerator(Runnable[Input, Output]):
    ...
    def __eq__(self, other: Any) -> bool:
        if isinstance(other, RunnableGenerator):
            ...
            elif hasattr(self, '_atransform') and hasattr(
                other, '_atransform'
            ):
...
class RunnableLambda(Runnable[Input, Output]):
    ...
    def get_input_schema(
        ...
        if isinstance(func, itemgetter):
            ...
            items = (
                str(func).replace('operator.itemgetter(', '')[:-1].split(', ')
            )
            if all(
                item[0] == "'" and item[-1] == "'" and len(item) > 2
                for item in items
            ):
                fields = {item[1:-1]: (Any, ...) for item in items}
                # It's a dict, lol
                return create_model_v2(
                    self.get_name('Input'), field_definitions=fields
                )
            else:
                module = getattr(func, '__module__', None)
                return create_model_v2(
        ...
        try:
            ...
            if sig.return_annotation != inspect.Signature.empty:
                ...
                if getattr(sig.return_annotation, '__origin__', None) in (
                    ...
                    return getattr(sig.return_annotation, '__args__', (Any,))[
                        0
                    ]
        ...
        **kwargs: Any,
        ...
        if isinstance(output, Runnable):
            ...
            if recursion_limit <= 0:
                msg = f'Recursion limit reached when invoking {self} with input {input}.'
    ...
    async def atransform(
        ...
        **kwargs: Optional[Any],
        ...
        async for output in self._atransform_stream_with_config(
            ...
            self._atransform,
            self._config(
                config, self.afunc if hasattr(self, 'afunc') else self.func
            ),
            **kwargs,
        ):
            yield output
...
class RunnableBindingBase(RunnableSerializable[Input, Output]):
    ...
    def _merge_configs(
        self, *configs: Optional[RunnableConfig]
    ) -> RunnableConfig:
        config = merge_configs(self.config, *configs)
        return merge_configs(
            config, *(f(config) for f in self.config_factories)
        )
    ...
    def with_config(
        ...
        **kwargs: Any,
        ...
        return self.__class__(
            ...
            kwargs=self.kwargs,
            config=cast(
                RunnableConfig, {**self.config, **(config or {}), **kwargs}
            ),
            custom_input_type=self.custom_input_type,
            custom_output_type=self.custom_output_type,
        )
...
class _RunnableCallableAsync(Protocol[Input, Output]):
    def __call__(
        self, __in: Input, *, config: RunnableConfig
    ) -> Awaitable[Output]:
        ...
