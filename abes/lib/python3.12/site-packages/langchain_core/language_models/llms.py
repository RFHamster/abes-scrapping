...
class BaseLLM(BaseLanguageModel[str], ABC):
    ...
    def _get_ls_params(
        ...
        **kwargs: Any,
        ...
        ls_params = LangSmithParams(
            ls_provider=default_provider, ls_model_type='llm'
        )
        if stop:
        ...
        # temperature
        if 'temperature' in kwargs and isinstance(
        ...
        if max_concurrency is None:
            ...
            return [
                ...
                for output in self.batch(
                    batch,
                    config=config[
                        i * max_concurrency : (i + 1) * max_concurrency
                    ],
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if (
            ...
            callback_managers = [
                ...
                for callback, tag, meta in zip(
                    callbacks, tags_list, metadata_list
                )
            ]
        else:
        ...
        if (
            self.cache is None and get_llm_cache() is None
        ) or self.cache is False:
            run_managers = [
        ...
        return LLMResult(
            generations=generations, llm_output=llm_output, run=run_info
        )

    @staticmethod
    def _get_run_ids_list(
        run_id: Optional[Union[uuid.UUID, list[Optional[uuid.UUID]]]],
        prompts: list,
    ) -> list:
        if run_id is None:
