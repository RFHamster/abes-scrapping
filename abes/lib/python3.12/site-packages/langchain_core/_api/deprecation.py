...
def deprecated(
    ...
    *,
    ...
    _validate_deprecation_params(
        pending, removal, alternative, alternative_import
    )
    ...
    def deprecate(
        ...
        *,
        ...
        if isinstance(obj, type):
            ...
            def finalize(wrapper: Callable[..., Any], new_doc: str) -> T:
                ...
                def warn_if_direct_instance(
                    ...
                    if (
                        not warned
                        and type(self) is obj
                        and not is_caller_internal()
                    ):
                        warned = True
                        emit_warning()
        ...
        if (
            _alternative
            and _alternative.split('.')[-1].lower()
            == _alternative.split('.')[-1]
        ):
        ...
        package = (
            _package or _name.split('.')[0].replace('_', '-')
            if '.' in _name
            else None
        )
...
def warn_deprecated(
    ...
    *,
    ...
    if not pending:
        if not removal:
            removal = (
                f'in {removal}' if removal else 'within ?? minor releases'
            )
            msg = (
    ...
    warning_cls = (
        ...
        if pending
        else LangChainDeprecationWarning
    )
