...
def _convert_typed_dict_to_openai_function(
    typed_dict: type,
) -> FunctionDescription:
    visited: dict = {}
    ...
    if type_ in visited:
        ...
        for arg, arg_type in annotations_.items():
            if get_origin(arg_type) is Annotated:
                ...
                field_kwargs = dict(
                    zip(('default', 'description'), annotated_args[1:])
                )
                if (
                    field_desc := field_kwargs.get('description')
                ) and not isinstance(field_desc, str):
                    msg = (
...
def convert_to_openai_function(
    ...
    *,
    ...
    # an Amazon Bedrock Converse format tool
        ...
        oai_function = cast(
            dict, convert_pydantic_to_openai_function(function)
        )
        ...
        oai_function = cast(
            dict, convert_python_function_to_openai_function(function)
        )
    else:
        msg = (
    ...
    if docstring:
        ...
        if error_on_invalid_docstring:
            filtered_annotations = {
                arg
                for arg in args
                if arg not in ('run_manager', 'callbacks', 'return')
            }
            if filtered_annotations and (
                len(docstring_blocks) < 2
                or not docstring_blocks[1].startswith('Args:')
            ):
