...
def merge_dicts(
    left: dict[str, Any], *others: dict[str, Any]
) -> dict[str, Any]:
    ...
    for right in others:
        for right_k, right_v in right.items():
            if (
                right_k not in merged
                or right_v is not None
                and merged[right_k] is None
            ):
...
def merge_lists(
    left: Optional[list], *others: Optional[list]
) -> Optional[list]:
