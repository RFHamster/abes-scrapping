    ...
    if isinstance(destination, VectorStore):
        ...
        for method in methods:
            if not hasattr(destination, method):
                msg = f'Vectorstore {destination} does not have required method {method}'
                raise ValueError(msg)
    ...
    for doc_batch in _batch(batch_size, doc_iterator):
        ...
        if cleanup == 'incremental':
            ...
            indexed_source_ids = cast(
                Sequence[str],
                [source_id_assigner(doc) for doc in docs_to_index],
            )
...
async def aindex(
    docs_source: Union[
        BaseLoader, Iterable[Document], AsyncIterator[Document]
    ],
    record_manager: RecordManager,
    vector_store: Union[VectorStore, DocumentIndex],
    *,
