...
class AIMessage(BaseMessage):
    ...
    def _backwards_compat_tool_calls(cls, values: dict) -> Any:
        ...
        if check_additional_kwargs and (
            raw_tool_calls := values.get('additional_kwargs', {}).get(
                'tool_calls'
            )
        ):
            try:
                if issubclass(cls, AIMessageChunk):  # type: ignore
                    ...
                    (
                        parsed_tool_calls,
                        parsed_invalid_tool_calls,
                    ) = default_tool_parser(raw_tool_calls)
    ...
    def pretty_repr(self, html: bool = False) -> str:
        ...
        def _format_tool_args(
            tc: Union[ToolCall, InvalidToolCall]
        ) -> list[str]:
            lines = [
...
def add_ai_message_chunks(
    ...
    # Token usage
    if left.usage_metadata or any(
        o.usage_metadata is not None for o in others
    ):
