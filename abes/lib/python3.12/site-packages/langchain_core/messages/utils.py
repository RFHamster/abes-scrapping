...
from langchain_core.messages.function import (
    FunctionMessage,
...
def merge_message_runs(
    ...
    **NOTE**: ToolMessages are not merged, as each has a distinct tool call id that
    ...
    for msg in messages:
        ...
        if not last:
            merged.append(curr)
        elif isinstance(curr, ToolMessage) or not isinstance(
            curr, last.__class__
        ):
...
def trim_messages(
    ...
    *,
    ...
    text_splitter: Optional[
        Union[Callable[[str], list[str]], TextSplitter]
    ] = None,
) -> list[BaseMessage]:
    ...
    **Note** The examples below show how to configure `trim_messages` to achieve
    ...
    if hasattr(token_counter, 'get_num_tokens_from_messages'):
        ...
        if (
            list(inspect.signature(token_counter).parameters.values())[
                0
            ].annotation
            is BaseMessage
        ):
    ...
    try:
        ...
        text_splitter_fn: Optional[Callable] = cast(
            Optional[Callable], text_splitter
        )
    else:
    ...
    *,
    ...
    for i, message in enumerate(messages):
        ...
        if isinstance(message, AIMessage) and message.tool_calls:
            oai_msg['tool_calls'] = _convert_to_openai_tool_calls(
                message.tool_calls
            )
            ...
            if text_format == 'string' and all(
                ...
                for j, block in enumerate(message.content):
                    ...
                    if isinstance(block, str):
                        ...
                        if missing := [
                            k for k in ('image_url',) if k not in block
                        ]:
                            err = (
                                f'Unrecognized content block at '
        ...
        if not included_partial:
            ...
            if text:
                ...
                for _ in range(num_splits - 1):
                    ...
                    if (
                        token_counter(messages[:idx] + [excluded])
                        <= max_tokens
                    ):
                        if partial_strategy == 'last':
...
def _last_max_tokens(
    ...
    *,
    ...
    reversed_ = (
        messages[:1] + messages[1:][::-1] if swapped_system else messages[::-1]
    )
...
def _msg_to_chunk(message: BaseMessage) -> BaseMessageChunk:
    if message.__class__ in _MSG_CHUNK_MAP:
        return _MSG_CHUNK_MAP[message.__class__](
...
def _is_message_type(
    message: BaseMessage,
    type_: Union[
        str, type[BaseMessage], Sequence[Union[str, type[BaseMessage]]]
    ],
) -> bool:
