...
FieldPlainInfoSerializerFunction = Callable[
    [Any, Any, FieldSerializationInfo], Any
]
...
def filter_dict_schema(
    *, include: IncExDict | None = None, exclude: IncExDict | None = None
    ...
    return _dict_not_none(
        type='include-exclude-dict', include=include, exclude=exclude
    )
...
NoInfoWrapValidatorFunction = Callable[
    [Any, ValidatorFunctionWrapHandler], Any
]
...
class TypedDictField(TypedDict, total=False):
    ...
    schema: Required[CoreSchema]
    required: bool
    validation_alias: Union[
        str, List[Union[str, int]], List[List[Union[str, int]]]
    ]
...
def definitions_schema(
    ...
    return DefinitionsSchema(
        type='definitions', schema=schema, definitions=definitions
    )
...
def field_plain_validator_function(
    ...
    return with_info_plain_validator_function(
        function, field_name=field_name, **kwargs
    )
...
def __getattr__(attr_name: str) -> object:
    ...
    if new_attr is None:
        raise AttributeError(
            f"module 'pydantic_core' has no attribute '{attr_name}'"
        )
    else:
        import warnings

        msg = (
            f'`{attr_name}` is deprecated, use `{new_attr.__name__}` instead.'
        )
