...
class DatetimeMS:
    ...
    def __init__(self, value: Union[int, datetime.datetime]):
        ...
        if isinstance(value, int):
            ...
            raise TypeError(
                f'{type(value)} is not a valid type for DatetimeMS'
            )
    ...
    def as_datetime(
        ...
        """
        return cast(
            datetime.datetime, _millis_to_datetime(self._value, codec_options)
        )
...
def _datetime_to_millis(dtm: datetime.datetime) -> int:
    ...
    return int(
        calendar.timegm(dtm.timetuple()) * 1000 + dtm.microsecond // 1000
    )
...
def _millis_to_datetime(
    ...
    if (
        ...
        if opts.datetime_conversion == DatetimeConversion.DATETIME_CLAMP:
            millis = max(
                _min_datetime_ms(tz), min(millis, _max_datetime_ms(tz))
            )
        ...
        raise ValueError(
            'datetime_conversion must be an element of DatetimeConversion'
        )
