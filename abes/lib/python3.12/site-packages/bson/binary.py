    ...
    **(BETA)** Subtype 9 provides a space-efficient representation of 1-dimensional vector data.
    ...
    def as_vector(self) -> BinaryVector:
        ...
        if self.subtype != VECTOR_SUBTYPE:
            raise ValueError(
                f'Cannot decode subtype {self.subtype} as a vector.'
            )
        ...
        if dtype == BinaryVectorDtype.INT8:
            ...
            dtype_format = 'B'
            format_string = f'<{n_values}{dtype_format}'
            unpacked_uint8s = list(
                struct.unpack_from(format_string, self, position)
            )
            return BinaryVector(unpacked_uint8s, dtype, padding)

        else:
            raise NotImplementedError(
                'Binary Vector dtype %s not yet supported' % dtype.name
            )
