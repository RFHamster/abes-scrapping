...
def _json_convert(
    obj: Any, json_options: JSONOptions = DEFAULT_JSON_OPTIONS
) -> Any:
...
def _parse_canonical_binary(
    ...
    if not isinstance(subtype, str) or len(subtype) > 2:
        raise TypeError(
            f'$binary subType must be a string at most 2 characters: {doc}'
        )
    if len(binary) != 2:
        raise TypeError(
...
def _parse_canonical_datetime(
    ...
    if isinstance(dtm, str):
        ...
        if json_options.tz_aware:
            ...
            if (
                json_options.datetime_conversion
    ...
    return _millis_to_datetime(
        int(dtm), cast('CodecOptions[Any]', json_options)
    )
...
def _parse_canonical_dbpointer(doc: Any, dummy0: Any) -> Any:
    ...
    if isinstance(dbref, DBRef):
        ...
        if not isinstance(dbref.id, ObjectId):
            raise TypeError(
                f'Bad $dbPointer, $id must be an ObjectId: {dbref_doc}'
            )
        if len(dbref_doc) != 2:
            raise TypeError(
                f'Bad $dbPointer, extra field(s) in DBRef: {dbref_doc}'
            )
        return dbref
    else:
...
def _encode_binary(
    data: bytes, subtype: int, json_options: JSONOptions
) -> Any:
