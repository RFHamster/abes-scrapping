...
def _get_c_string(
    ...
    return (
        _utf_8_decode(
            view[position:end], opts.unicode_decode_error_handler, True
        )[0],
        end + 1,
    )
...
def _get_object_size(
    data: Any, position: int, obj_end: int
) -> Tuple[int, int]:
...
def _get_code_w_scope(
    ...
    scope, position = _get_object(
        data, view, position, code_end, opts, element_name
    )
...
def _bson_to_dict(
    ...
    try:
        ...
        return cast(
            '_DocumentType', _elements_to_dict(data, view, 4, end, opts)
        )
    except InvalidBSON:
        raise
    except Exception:
...
def _encode_float(
    name: bytes, value: float, dummy0: Any, dummy1: Any
) -> bytes:
    """Encode a float."""
...
def _encode_bytes(
    name: bytes, value: bytes, dummy0: Any, dummy1: Any
) -> bytes:
...
def _encode_list(
    ...
    data = b''.join(
        [
            _name_value_to_bson(next(lname), item, check_keys, opts)
            for item in value
        ]
    )
...
def _encode_binary(
    name: bytes, value: Binary, dummy0: Any, dummy1: Any
) -> bytes:
...
def _encode_objectid(
    name: bytes, value: ObjectId, dummy: Any, dummy1: Any
) -> bytes:
...
def _encode_regex(
    name: bytes, value: Regex[Any], dummy0: Any, dummy1: Any
) -> bytes:
...
def _encode_timestamp(
    name: bytes, value: Any, dummy0: Any, dummy1: Any
) -> bytes:
...
def _encode_minkey(
    name: bytes, dummy0: Any, dummy1: Any, dummy2: Any
) -> bytes:
...
def decode(
    ...
    return cast(
        'Union[dict[str, Any], _DocumentType]', _bson_to_dict(data, opts)
    )
    ...
    try:
        while position < end:
            ...
            if use_raw:
                ...
                docs.append(
                    _elements_to_dict(data, view, position + 4, obj_end, opts)
                )
            position += obj_size
        return docs
    except InvalidBSON:
...
def _decode_all_selective(
    ...
    internal_codec_options: CodecOptions[
        RawBSONDocument
    ] = codec_options.with_options(
