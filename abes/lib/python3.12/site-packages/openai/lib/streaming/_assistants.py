...
from ...types.beta.threads.runs import (
    RunStep,
    ToolCall,
    RunStepDelta,
    ToolCallDelta,
...
class AssistantEventHandler:
    ...
    def _emit_sse_event(self, event: AssistantStreamEvent) -> None:
        ...
        for content_delta in new_content:
            ...
            block = self.__current_message_snapshot.content[
                content_delta.index
            ]
            if block.type == 'text':
        ...
        if (
            ...
            if message_delta.content is not None:
                for content_delta in message_delta.content:
                    ...
                    if (
                        content_delta.index
                        != self._current_message_content_index
                    ):
                        if self._current_message_content is not None:
                            if self._current_message_content.type == 'text':
                                ...
                                self._current_message_content.type
                                == 'image_file'
                            ):
...
AssistantEventHandlerT = TypeVar(
    'AssistantEventHandlerT', bound=AssistantEventHandler
)
...
class AsyncAssistantEventHandler:
    ...
    async def on_message_delta(
        self, delta: MessageDelta, snapshot: Message
    ) -> None:
        """Callback that is fired whenever a message delta is returned from the API
            ...
            if (
                ...
                for tool_call_delta in run_step_delta.step_details.tool_calls:
                    ...
                    if tool_call_delta.index != self._current_tool_call_index:
                        ...
                        await self.on_tool_call_created(
                            self._current_tool_call
                        )
