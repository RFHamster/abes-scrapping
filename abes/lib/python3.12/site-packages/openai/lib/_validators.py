...
def necessary_column_validator(
    df: pd.DataFrame, necessary_column: str
) -> Remediation:
    """
...
def additional_column_validator(
...
def completions_space_start_validator(df: pd.DataFrame) -> Remediation:
    ...
    if (
        df.completion.str[:1].nunique() != 1
        or df.completion.values[0][0] != ' '
    ):
    ...
    if os.path.isfile(fname):
        try:
            if fname.lower().endswith('.csv') or fname.lower().endswith(
                ...
                necessary_msg = (
                    'Your format `XLSX` will be converted to `JSONL`'
                )
                ...
                necessary_msg = (
                    'Your format `TXT` will be converted to `JSONL`'
                )
                ...
                if '.' in fname:
                    ...
                    error_msg += (
                        f' Your file `{fname}` is missing a file extension.'
                    )
...
def apply_necessary_remediation(
    df: OptionalDataFrameT, remediation: Remediation
) -> OptionalDataFrameT:
    """
...
def write_out_file(
    ...
    common_completion_suffix_new_line_handled = (
        common_completion_suffix.replace('\n', '\\n')
    )
    ...
    if not any_remediations and not split:
        ...
        if split:
            assert (
                len(fnames) == 2
                and 'train' in fnames[0]
                and 'valid' in fnames[1]
            )
        ...
        files_string = (
            ('s' if split else '') + ' to `' + ('` and `'.join(fnames))
        )
