...
def get_input_tool_by_name(
    *, input_tools: list[ChatCompletionToolParam], name: str
    ...
    return next(
        (t for t in input_tools if t.get('function', {}).get('name') == name),
        None,
    )
...
def parse_function_tool_arguments(
    *,
    input_tools: list[ChatCompletionToolParam],
    function: Function | ParsedFunction,
) -> object:
    input_tool = get_input_tool_by_name(
        input_tools=input_tools, name=function.name
    )
    if not input_tool:
        return None
...
def is_response_format_param(
    response_format: object,
) -> TypeGuard[ResponseFormatParam]:
...
def _parse_content(
    ...
    if is_basemodel_type(response_format):
        return cast(
            ResponseFormatT, model_parse_json(response_format, content)
        )
...
def type_to_response_format_param(
    ...
    json_schema_type: type[pydantic.BaseModel] | pydantic.TypeAdapter[
        Any
    ] | None = None
