...
class BaseModel(pydantic.BaseModel):
    if PYDANTIC_V2:
        model_config: ClassVar[ConfigDict] = ConfigDict(
            extra='allow',
            defer_build=coerce_boolean(
                os.environ.get('DEFER_PYDANTIC_BUILD', 'true')
            ),
        )
    else:
        ...
        def __repr_args__(self) -> ReprArgs:
            ...
            return [
                ...
                if arg[0] not in {'_request_id', '__exclude_fields__'}
            ]
...
def is_basemodel_type(
    type_: type,
) -> TypeGuard[type[BaseModel] | type[GenericModel]]:
...
def build(
    ...
    **kwargs: P.kwargs,
    ...
    return cast(
        _BaseModelT, construct_type(type_=base_model_cls, value=kwargs)
    )
...
def construct_type(*, value: object, type_: object) -> object:
    ...
    if is_union(origin):
        try:
            return validate_type(
                type_=cast('type[object]', type_), value=value
            )
        except Exception:
            pass
    ...
    if not is_literal_type(type_) and (
        ...
        if is_list(value):
            return [
                cast(Any, type_).construct(**entry)
                if is_mapping(entry)
                else entry
                for entry in value
            ]
    ...
    if origin == list:
        ...
        return [
            construct_type(value=entry, type_=inner_type) for entry in value
        ]
    ...
    *, union: type, meta_annotations: tuple[Any, ...]
    ...
    for variant in get_args(union):
        ...
        if is_basemodel_type(variant):
            if PYDANTIC_V2:
                field = _extract_field_schema_pv2(
                    variant, discriminator_field_name
                )
                if not field:
                    continue
                ...
                if field_schema['type'] == 'literal':
                    for entry in cast('LiteralSchema', field_schema)[
                        'expected'
                    ]:
                ...
                field_info = cast(
                    'dict[str, FieldInfo]', variant.__fields__
                ).get(
                ...
                if field_info.annotation and is_literal_type(
