...
class BasePage(GenericModel, Generic[_T]):
    ...
    def _params_from_url(self, url: URL) -> httpx.QueryParams:
        ...
        return httpx.QueryParams(cast(Any, self._options.params)).merge(
            url.params
        )
...
class BaseClient(Generic[_HttpxClientT, _DefaultStreamT]):
    ...
    def _build_headers(
        ...
        if 'x-stainless-retry-count' not in (
            header.lower() for header in custom_headers
        ):
    ...
    def _build_request(
        ...
        *,
        ...
        if log.isEnabledFor(logging.DEBUG):
            log.debug(
                'Request options: %s', model_dump(options, exclude_unset=True)
            )
        ...
        if '_' in prepared_url.host:
            ...
            kwargs['extensions'] = {
                'sni_hostname': prepared_url.host.replace('_', '-')
            }
        ...
        *,
        ...
        try:
            if inspect.isclass(cast_to) and issubclass(
                cast_to, ModelBuilderProtocol
            ):
                return cast(
                    ResponseT, cast_to.build(response=response, data=data)
                )

            if self._strict_response_validation:
                return cast(
                    ResponseT, validate_type(type_=cast_to, value=data)
                )
    ...
    def _should_retry(self, response: httpx.Response) -> bool:
        ...
        if should_retry_header == 'false':
            log.debug(
                'Not retrying as header `x-should-retry` is set to `false`'
            )
            return False
...
class SyncAPIClient(BaseClient[httpx.Client, Stream[Any]]):
    ...
    def request(
        ...
        *,
        ...
        if remaining_retries is not None:
            retries_taken = (
                options.get_max_retries(self.max_retries) - remaining_retries
            )
        else:
            retries_taken = 0
    ...
    def _request(
        ...
        *,
        ...
        remaining_retries = (
            options.get_max_retries(self.max_retries) - retries_taken
        )
        ...
        try:
            response = self._client.send(
                request,
                stream=stream
                or self._should_stream_response_body(request=request),
                **kwargs,
            )
    ...
    def _retry_request(
        ...
        *,
        ...
        timeout = self._calculate_retry_timeout(
            remaining_retries, options, response_headers
        )
...
class AsyncAPIClient(BaseClient[httpx.AsyncClient, AsyncStream[Any]]):
    ...
    def _request_api_list(
        ...
        return AsyncPaginator(
            client=self, options=options, page_cls=page, model=model
        )
    ...
    async def get(
        ...
        *,
        ...
        return await self.request(
            cast_to, opts, stream=stream, stream_cls=stream_cls
        )
