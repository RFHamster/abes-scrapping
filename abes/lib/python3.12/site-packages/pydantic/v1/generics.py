...
class GenericModel(BaseModel):
    ...
    def __class_getitem__(
        ...
        if cls is GenericModel and any(
            isinstance(param, TypeVar) for param in params
        ):
            raise TypeError(
        ...
        if (
            all_identical(typevars_map.keys(), typevars_map.values())
            and typevars_map
        ):
        ...
        created_model = cast(
            ...
            create_model(
                ...
                __base__=(cls,)
                + tuple(cls.__parameterized_bases__(typevars_map)),
                __config__=None,
        ...
        if (
            ...
            reference_module_globals = sys.modules[
                created_model.__module__
            ].__dict__
        ...
        new_params = tuple(
            {
                param: None
                for param in iter_contained_typevars(typevars_map.values())
            }
        ...
        _prepare_model_fields(
            created_model, fields, instance_type_hints, typevars_map
        )
...
def replace_types(type_: Any, type_map: Mapping[Any, Any]) -> Any:
    ...
    if origin_type is Annotated:
        ...
        return Annotated[
            replace_types(annotated_type, type_map), tuple(annotations)
        ]
    ...
    if type_args:
        resolved_type_args = tuple(
            replace_types(arg, type_map) for arg in type_args
        )
        ...
        if (
            sys.version_info >= (3, 10) and origin_type is types.UnionType
        ):  # noqa: E721
    ...
    if (
        ...
        resolved_type_args = tuple(
            replace_types(t, type_map) for t in type_args
        )
        if all_identical(type_args, resolved_type_args):
            return type_
