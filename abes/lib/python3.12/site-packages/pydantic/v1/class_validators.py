...
def validator(
    *fields: str,
    ...
    if whole is not None:
        ...
        assert (
            each_item is False
        ), '"each_item" and "whole" conflict, remove "whole"'
        each_item = not whole
    ...
    *,
    ...
    if _func:
        ...
        setattr(
            ...
            Validator(
                func=f_cls.__func__, pre=pre, skip_on_failure=skip_on_failure
            ),
        )
        return f_cls
...
class ValidatorGroup:
    ...
    def get_validators(self, name: str) -> Optional[Dict[str, Validator]]:
        ...
        if validators:
            return {
                getattr(
                    v.func, '__name__', f'<No __name__: id:{id(v.func)}>'
                ): v
                for v in validators
            }
        else:
...
def make_generic_validator(validator: AnyCallable) -> 'ValidatorCallable':
    ...
    if first_arg == 'self':
        ...
        return wraps(validator)(
            _generic_validator_cls(validator, sig, set(args[1:]))
        )
    else:
        # assume the first argument was value which has already been removed
        return wraps(validator)(
            _generic_validator_basic(validator, sig, set(args))
        )
...
def gather_all_validators(type_: 'ModelOrDc') -> Dict[str, 'AnyClassMethod']:
    ...
    return {
        ...
        if hasattr(v, VALIDATOR_CONFIG_KEY)
        or hasattr(v, ROOT_VALIDATOR_CONFIG_KEY)
    }
