...
class FieldInfo(Representation):
    ...
    def __repr_args__(self) -> 'ReprArgs':
        ...
        return [
            (a, v)
            for a, v in attrs
            if v != field_defaults_to_hide.get(a, None)
        ]
    ...
    def update_from_config(self, from_config: Dict[str, Any]) -> None:
        ...
        for attr_name, value in from_config.items():
            try:
                ...
                if current_value is self.__field_constraints__.get(
                    attr_name, None
                ):
...
def Field(
    ...
    *,
    ...
    exclude: Optional[
        Union['AbstractSetIntStr', 'MappingIntStrAny', Any]
    ] = None,
    include: Optional[
        Union['AbstractSetIntStr', 'MappingIntStrAny', Any]
    ] = None,
...
class ModelField(Representation):
    ...
    def infer(
        ...
        *,
        ...
        field_info, value = cls._get_field_info(
            name, annotation, value, config
        )
        required: 'BoolUndefined' = Undefined
    ...
    def set_config(self, config: Type['BaseConfig']) -> None:
        ...
        if new_alias and new_alias_priority >= (
            self.field_info.alias_priority or 0
        ):
    ...
    def prepare(self) -> None:
        ...
        if (
            self.type_.__class__ is ForwardRef
            or self.type_.__class__ is DeferredType
        ):
    ...
    def _type_analysis(self) -> None:  # noqa: C901 (ignore complexity)
        ...
        if origin is None or origin is CollectionsHashable:
            ...
            if len(types_) == 1:
                ...
                self.sub_fields = [
                    self._create_sub_type(
                        t, f'{self.name}_{display_as_type(t)}'
                    )
                    for t in types_
                ]
            ...
            if not args:  # plain tuple
                ...
                self.shape = SHAPE_TUPLE_ELLIPSIS
                self.sub_fields = [
                    self._create_sub_type(args[0], f'{self.name}_0')
                ]
        ...
        # check for Iterable and CollectionsIterable, as it could receive one even when declared with the other
            ...
            self.sub_fields = [
                self._create_sub_type(self.type_, f'{self.name}_type')
            ]
    ...
    def _create_sub_type(
        ...
        field_info, _ = self._get_field_info(
            name, type_, None, self.model_config
        )
    ...
    def populate_validators(self) -> None:
        ...
        """
        self.validate_always = getattr(
            self.type_, 'validate_always', False
        ) or any(v.always for v in self.class_validators.values())
        ...
        if not self.sub_fields or self.shape == SHAPE_GENERIC:
            ...
            v_funcs = (
                ...
                *[
                    v.func
                    for v in class_validators_
                    if v.each_item and not v.pre
                ],
            )
        ...
        if self.field_info and self.field_info.const:
            self.post_validators.append(
                make_generic_validator(constant_validator)
            )
        ...
        if class_validators_:
            ...
            self.post_validators += prep_validators(
                v.func
                for v in class_validators_
                if not v.each_item and not v.pre
            )
        ...
        *,
        ...
        if self.pre_validators:
            v, errors = self._apply_validators(
                v, values, loc, cls, self.pre_validators
            )
            if errors:
                return v, errors
        ...
        if self.shape == SHAPE_SINGLETON:
            ...
            v, errors = self._apply_validators(
                v, values, loc, cls, self.validators
            )
        else:
    ...
    def _validate_singleton(
        ...
        if self.sub_fields:
            ...
            if self.model_config.smart_union and is_union(
                get_origin(self.type_)
            ):
                ...
                for field in self.sub_fields:
                    ...
                    try:
                        ...
                        # compound type
                        if lenient_isinstance(
                            v, get_origin(field.outer_type_)
                        ):
                            value, error = field.validate(
                                v, values, loc=loc, cls=cls
                            )
                            if not error:
                                return value, None
    ...
    def _validate_discriminated_union(
        ...
        try:
            ...
            return v, ErrorWrapper(
                MissingDiscriminator(discriminator_key=self.discriminator_key),
                loc,
            )
        except TypeError:
            try:
        ...
        try:
            ...
            return sub_field.validate(
                v,
                values,
                loc=(*loc, display_as_type(sub_field.type_)),
                cls=cls,
            )
    ...
    def is_complex(self) -> bool:
        ...
        return (
            ...
            or lenient_issubclass(
                self.type_, (BaseModel, list, set, frozenset, dict)
            )
        )
    ...
    def _type_display(self) -> PyObjectStr:
        ...
        if self.allow_none and (
            self.shape != SHAPE_SINGLETON or not self.sub_fields
        ):
            t = f'Optional[{t}]'
...
def is_finalvar_with_default_val(type_: Type[Any], val: Any) -> bool:
    return (
        ...
        and not isinstance(val, FieldInfo)
    )
