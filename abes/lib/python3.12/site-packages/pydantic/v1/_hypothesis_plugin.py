...
try:
    ...
    st.register_type_strategy(
        ...
        st.builds(
            ...
            st.from_regex(
                '[A-Za-z0-9_]+( [A-Za-z0-9_]+){0,5}', fullmatch=True
            ),
...
_color_regexes = (
    ...
    .replace(
        pydantic.color._r_alpha, r'(?:(0(?:\.\d+)?|1(?:\.0+)?|\.\d+|\d{1,2}%))'
    )
    .replace(
...
def add_luhn_digit(card_number: str) -> str:
    ...
    for digit in '0123456789':
        with contextlib.suppress(Exception):
            pydantic.PaymentCardNumber.validate_luhn_check_digit(
                card_number + digit
            )
...
# Secrets
st.register_type_strategy(
    pydantic.SecretBytes, st.binary().map(pydantic.SecretBytes)
)
st.register_type_strategy(
    pydantic.SecretStr, st.text().map(pydantic.SecretStr)
)
...
def resolve_json(cls):  # type: ignore[no-untyped-def]
    try:
        inner = (
            st.none()
            if cls.inner_type is None
            else st.from_type(cls.inner_type)
        )
...
def resolve_condecimal(cls):  # type: ignore[no-untyped-def]
    ...
    if cls.lt is not None:
        ...
        max_value = cls.lt
    s = st.decimals(
        min_value, max_value, allow_nan=False, places=cls.decimal_places
    )
    if cls.lt is not None:
