...
from pydantic.v1.config import (
    BaseConfig,
    Extra,
    inherit_config,
    prepare_config,
...
class ModelMetaclass(ABCMeta):
    ...
    def __new__(mcs, name, bases, namespace, **kwargs):  # noqa C901
        ...
        for base in reversed(bases):
            if (
                ...
                validators = inherit_validators(
                    base.__validators__, validators
                )
        ...
        validators = inherit_validators(
            extract_validators(namespace), validators
        )
        ...
        if (namespace.get('__module__'), namespace.get('__qualname__')) != (
            ...
            for ann_name, ann_type in annotations.items():
                if is_classvar(ann_type):
                    ...
                    ann_name not in namespace
                    and config.underscore_attrs_are_private
                ):
        ...
        if config.json_encoders:
            json_encoder = partial(
                custom_pydantic_encoder, config.json_encoders
            )
        else:
        ...
        exclude_from_namespace = (
            fields | private_attributes.keys() | {'__slots__'}
        )
        new_namespace = {
            '__config__': config,
            '__fields__': fields,
            ...
            **{
                ...
                if n not in exclude_from_namespace
            },
        }
...
class BaseModel(Representation, metaclass=ModelMetaclass):
    ...
    def __setattr__(self, name, value):  # noqa: C901 (ignore complexity)
        ...
        if (
            self.__config__.extra is not Extra.allow
    ...
    def _enforce_dict_if_root(cls, obj: Any) -> Any:
        if cls.__custom_root_type__ and (
            ...
            and not (
                ...
                and obj.__fields__.keys() == {ROOT_KEY}
            )
    ...
    def parse_obj(cls: Type['Model'], obj: Any) -> 'Model':
        ...
        if not isinstance(obj, dict):
            try:
                ...
                raise ValidationError(
                    [ErrorWrapper(exc, loc=ROOT_KEY)], cls
                ) from e
        return cls(**obj)
    ...
    def _get_value(
        ...
        elif isinstance(v, Enum) and getattr(
            cls.Config, 'use_enum_values', False
        ):
            return v.value
    ...
    def __repr_args__(self) -> 'ReprArgs':
        return [
            ...
            and (
                k not in self.__fields__ or self.__fields__[k].field_info.repr
            )
        ]
    ...
    for validator in model.__pre_root_validators__:
        try:
            ...
            return (
                {},
                set(),
                ValidationError([ErrorWrapper(exc, loc=ROOT_KEY)], cls_),
            )
