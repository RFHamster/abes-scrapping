...
def truncate(v: Union[str], *, max_len: int = 80) -> str:
    ...
    """
    warnings.warn(
        '`truncate` is no-longer used by pydantic and is deprecated',
        DeprecationWarning,
    )
...
def validate_field_name(
    bases: List[Type['BaseModel']], field_name: str
) -> None:
    """
...
def almost_equal_floats(
    value_1: float, value_2: float, *, delta: float = 1e-8
) -> bool:
    """
...
def get_model(
    ...
    if not issubclass(model_cls, BaseModel):
        raise TypeError(
            'Unsupported type, must be either BaseModel or dataclass'
        )
    return model_cls
...
class ValueItems(Representation):
    ...
    def merge(cls, base: Any, override: Any, intersect: bool = False) -> Any:
        ...
        for k in merge_keys:
            merged_item = cls.merge(
                base.get(k), override.get(k), intersect=intersect
            )
...
def _get_union_alias_and_all_values(
    ...
    return (
        get_unique_discriminator_alias(all_aliases, discriminator_key),
        all_values,
    )
