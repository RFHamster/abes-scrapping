...
_str_types_attrs: Tuple[
    Tuple[str, Union[type, Tuple[type, ...]], str], ...
] = (
...
def get_field_schema_validations(field: ModelField) -> Dict[str, Any]:
    ...
    if lenient_issubclass(field.type_, numeric_types) and not issubclass(
        field.type_, bool
    ):
...
def get_flat_models_from_model(
    model: Type['BaseModel'], known_models: Optional[TypeModelSet] = None
) -> TypeModelSet:
    """
...
def get_flat_models_from_field(
    ...
    if lenient_issubclass(
        getattr(field_type, '__pydantic_model__', None), BaseModel
    ):
    ...
    if field.sub_fields and not lenient_issubclass(field_type, BaseModel):
        ...
        lenient_issubclass(field_type, BaseModel)
        and field_type not in known_models
    ):
...
def get_flat_models_from_fields(
    ...
    flat_models: TypeModelSet = set()
    for field in fields:
        flat_models |= get_flat_models_from_field(
    ...
    *,
    ...
    if len(sub_fields) == 1:
        ...
        if field_has_discriminator:
            ...
            discriminator_models_refs: Dict[
            ...
            for (
                discriminator_value,
                sub_field,
            ) in field.sub_fields_mapping.items():
                ...
                if is_union(get_origin(sub_field.type_)):
                    ...
                    discriminator_models_refs[
                        discriminator_value
                    ] = discriminator_model_ref['$ref']
...
def add_field_type_to_schema(field_type: Any, schema_: Dict[str, Any]) -> None:
    ...
    for type_, t_schema in field_class_to_schema:
        ...
        if (
            lenient_issubclass(field_type, type_)
            or field_type is type_ is Pattern
        ):
    ...
    *,
    ...
    if is_literal_type(field_type):
        values = tuple(
            ...
            for x in all_literal_values(field_type)
        )
        ...
        f_schema, schema_overrides = get_field_info_schema(
            field, schema_overrides
        )
        f_schema.update(
            get_schema_ref(
                enum_name, ref_prefix, ref_template, schema_overrides
            )
        )
...
def get_annotation_with_constraints(
    ...
    def go(type_: Any) -> Type[Any]:
        ...
        if isinstance(type_, type):
            ...
            elif issubclass(type_, str) and not issubclass(
                type_, (EmailStr, AnyUrl)
            ):
        ...
        if attrs:
            ...
            kwargs = {
                ...
                for attr_name, attr in (
                    (attr_name, getattr(field_info, attr_name))
