...
BUILTINS_NAME = (
    'builtins' if MYPY_VERSION_TUPLE >= (0, 930) else '__builtins__'
)
...
class PydanticPlugin(Plugin):
    ...
    def get_base_class_hook(
        ...
        if sym and isinstance(sym.node, TypeInfo):  # pragma: no branch
            ...
            if any(
                base.fullname == BASEMODEL_FULLNAME for base in sym.node.mro
            ):
    ...
    def _pydantic_model_metaclass_marker_callback(
        self, ctx: ClassDefContext
    ) -> None:
        ...
        if toml_config is not None:
            ...
            for key in self.__slots__:
                setting = plugin_config.getboolean(
                    CONFIGFILE_KEY, key, fallback=False
                )
                setattr(self, key, setting)
...
def from_attributes_callback(ctx: MethodContext) -> Type:
    ...
    if isinstance(ctx_type, CallableType) and isinstance(
        ctx_type.ret_type, Instance
    ):
        model_type = ctx_type.ret_type  # called on the class
    elif isinstance(ctx_type, Instance):
        model_type = (
            ctx_type  # called on an instance (unusual, but still valid)
        )
    ...
    if pydantic_metadata is None:
        return ctx.default_return_type
    if not any(
        base.fullname == BASEMODEL_FULLNAME for base in model_type.type.mro
    ):
        # not a Pydantic v2 model
        return ctx.default_return_type
    from_attributes = pydantic_metadata.get('config', {}).get(
        'from_attributes'
    )
...
class PydanticModelField:
    ...
    def to_argument(
        ...
        variable = self.to_var(
            current_info, api, use_alias, force_typevars_invariant
        )
        ...
        return Argument(
            ...
            if is_root_model_root
                ...
                ARG_NAMED_OPT
                if force_optional or self.has_default
                else ARG_NAMED
            ),
        )
    ...
    def expand_type(
        ...
        if (
            ...
            and getattr(self.info, 'self_type', None) is not None
        ):
    ...
    def to_var(
        ...
        return Var(
            name, self.expand_type(current_info, api, force_typevars_invariant)
        )
    ...
    def expand_typevar_from_subtype(
        ...
        if self.type is not None:
            with state.strict_optional_set(api.options.strict_optional):
                self.type = map_type_from_supertype(
                    self.type, sub_type, self.info
                )
...
class PydanticModelTransformer:
    ...
    def transform(self) -> bool:
        ...
        * stores the fields, config, and if the class is settings in the mypy metadata for access by subclasses
        ...
        fields, class_vars = self.collect_fields_and_class_vars(
            config, is_root_model
        )
        ...
        self.add_model_construct_method(
            fields, config, is_settings, is_root_model
        )
        ...
        info.metadata[METADATA_KEY] = {
            ...
            'class_vars': {
                class_var.name: class_var.serialize()
    ...
    def collect_config(
        self,
    ) -> ModelConfigData:  # noqa: C901 (ignore complexity)
        ...
        for stmt in cls.defs.body:
            ...
            if isinstance(stmt, AssignmentStmt):
                ...
                ):  # calls to `dict` or `ConfigDict`
                    for arg_name, arg in zip(
                        stmt.rvalue.arg_names, stmt.rvalue.args
                    ):
                        if arg_name is None:
                            continue
                        config.update(
                            self.get_config_update(
                                arg_name, arg, lax_extra=True
                            )
                        )
                ...
                for substmt in stmt.defs.body:
                    ...
                    if not isinstance(lhs, NameExpr):
                        continue
                    config.update(
                        self.get_config_update(lhs.name, substmt.rvalue)
                    )
    ...
    def collect_fields_and_class_vars(
        ...
        list[PydanticModelField] | None, list[PydanticModelClassVar] | None
    ]:
        ...
        for info in reversed(
            ...
            for name, data in info.metadata[METADATA_KEY]['fields'].items():
                ...
                if (
                    sym_node
                    and sym_node.node
                    and not isinstance(sym_node.node, Var)
                ):
                    self._api.fail(
            ...
            # Collect ClassVars
            for name, data in info.metadata[METADATA_KEY][
                'class_vars'
            ].items():
                found_class_vars[name] = PydanticModelClassVar.deserialize(
                    data
                )
    ...
    def collect_field_or_class_var_from_stmt(  # noqa C901
        ...
        if (
            not _fields.is_valid_field_name(lhs.name)
            or lhs.name == 'model_config'
        ):
            return None
        ...
        if sym.type is None and node.is_final and node.is_inferred:
            ...
            # argument for Final[...].
            typ = self._api.analyze_simple_literal_type(
                stmt.rvalue, is_final=True
            )
            if typ:
                node.type = typ
            else:
    ...
    def _infer_dataclass_attr_init_type(
        ...
        if setter:
            if isinstance(setter.node, FuncDef):
                ...
                if isinstance(
                    setter_type, CallableType
                ) and setter_type.arg_kinds == [
                    ARG_POS,
                    ARG_POS,
                    ARG_POS,
                ]:
                ...
                self._api.fail(
                    f'Unsupported "__set__" in "{t.type.name}"', context
                )
    ...
    def add_initializer(
        ...
        if is_settings:
            ...
            if '__init__' in base_settings_node.names:
                base_settings_init_node = base_settings_node.names[
                    '__init__'
                ].node
                if (
    ...
    def add_model_construct_method(
        ...
        fields_set_argument = Argument(
            Var('_fields_set', optional_set_str),
            optional_set_str,
            None,
            ARG_OPT,
        )
        ...
        for field in fields:
            ...
            if sym_node is not None:
                ...
                if isinstance(var, Var):
                    ...
                    isinstance(var, PlaceholderNode)
            ...
            for arg, name in zip(expr.args, expr.arg_names):
                ...
                if name == 'default_factory':
                    return not (
                        ...
                        and arg.fullname == 'builtins.None'
                    )
            return False
    ...
    def should_init_forbid_extra(
        ...
        *unless* a required dynamic alias is present (since then we can't determine a valid signature).
        ...
        if not config.populate_by_name:
            if self.is_dynamic_alias_present(
...
ERROR_ORM = ErrorCode(
    'pydantic-orm', 'Invalid from_attributes call', 'Pydantic'
)
ERROR_CONFIG = ErrorCode('pydantic-config', 'Invalid config value', 'Pydantic')
ERROR_ALIAS = ErrorCode(
    'pydantic-alias', 'Dynamic alias disallowed', 'Pydantic'
)
ERROR_UNEXPECTED = ErrorCode(
    'pydantic-unexpected', 'Unexpected behavior', 'Pydantic'
)
ERROR_UNTYPED = ErrorCode(
    'pydantic-field', 'Untyped field disallowed', 'Pydantic'
)
ERROR_FIELD_DEFAULTS = ErrorCode(
    'pydantic-field', 'Invalid Field defaults', 'Pydantic'
)
...
def error_from_attributes(
    ...
    api.fail(
        f'"{model_name}" does not have from_attributes=True',
        context,
        code=ERROR_ORM,
    )
