...
def _iter(
    ...
    if include is not None:
        include = _utils.ValueItems.merge(
            {k: True for k in self.model_fields}, include, intersect=True
        )
    ...
    value_exclude = (
        _utils.ValueItems(self, exclude) if exclude is not None else None
    )
    value_include = (
        _utils.ValueItems(self, include) if include is not None else None
    )

    if self.__pydantic_extra__ is None:
        items = self.__dict__.items()
    else:
        items = list(self.__dict__.items()) + list(
