...
FieldDecoratorInfoType = TypeVar(
    'FieldDecoratorInfoType', bound=FieldDecoratorInfo
)
...
def filter_field_decorator_info_by_field(
    validator_functions: Iterable[Decorator[FieldDecoratorInfoType]],
    field: str,
...
def modify_model_json_schema(
    ...
    *,
    ...
    docstring = (
        None
        if cls is BaseModel
        or is_builtin_dataclass(cls)
        or is_pydantic_dataclass(cls)
        else cls.__doc__
    )
...
class GenerateSchema:
    ...
    def _enum_schema(self, enum_type: type[Enum]) -> CoreSchema:
        ...
        description = (
            None
            if not enum_type.__doc__
            else inspect.cleandoc(enum_type.__doc__)
        )
        if (
        ...
        if issubclass(enum_type, int):
            sub_type = 'int'
            value_ser_type: core_schema.SerSchema = (
                core_schema.simple_ser_schema('int')
            )
    ...
    def _add_js_function(
        ...
        pydantic_js_functions = metadata.setdefault(
    ...
    def generate_schema(
        ...
        if metadata_js_function is not None:
            metadata_schema = resolve_original_schema(
                schema, self.defs.definitions
            )
            if metadata_schema:
    ...
    def _model_schema(self, cls: type[BaseModel]) -> core_schema.CoreSchema:
        ...
        with self.defs.get_schema_or_ref(cls) as (model_ref, maybe_schema):
            ...
            metadata = build_metadata_dict(
                js_functions=[
                    partial(modify_model_json_schema, cls=cls, title=title)
                ]
            )
            ...
            with self._config_wrapper_stack.push(
                config_wrapper
            ), self._types_namespace_stack.push(cls):
                    ...
                    for candidate_cls in cls.__mro__[:-1]:
                        ...
                        if extras_annotation is not None:
                            if isinstance(extras_annotation, str):
                                extras_annotation = (
                                    _typing_extra.eval_type_backport(
                ...
                if cls.__pydantic_root_model__:
                    ...
                    model_schema = core_schema.model_schema(
                        ...
                        inner_schema,
                        custom_init=getattr(
                            cls, '__pydantic_custom_init__', None
                        ),
                        root_model=True,
    ...
    def _generate_schema_from_property(
        ...
        if (
            ...
            (existing_schema := getattr(obj, '__pydantic_core_schema__', None))
            is not None
    ...
    def _resolve_forward_ref(self, obj: Any) -> Any:
        ...
        try:
            obj = _typing_extra.eval_type_backport(
                obj, globalns=self._types_namespace
            )
    ...
    def _match_generic_type(
        self, obj: Any, origin: Any
    ) -> CoreSchema:  # noqa: C901
    ...
    def _common_field_schema(  # C901
        ...
        if has_instance_in_type(field_info.annotation, (ForwardRef, str)):
            ...
            evaluated = _typing_extra.eval_type_lenient(
                field_info.annotation, types_namespace
            )
            if (
                evaluated is not field_info.annotation
                and not has_instance_in_type(evaluated, PydanticRecursiveRef)
            ):
        ...
        for decorator in filter_field_decorator_info_by_field(
            ...
            validators_from_decorators.append(
                _mode_to_validator[decorator.info.mode]._from_decorator(
                    decorator
                )
            )
        ...
        metadata = build_metadata_dict(
            js_annotation_functions=[
                get_json_schema_update_func(
                    json_schema_updates, json_schema_extra
                )
            ]
        )
    ...
    def _union_schema(self, union_type: Any) -> core_schema.CoreSchema:
        ...
        if len(choices) == 1:
            ...
            for choice in choices:
                tag = choice.get('metadata', {}).get(
                    _core_utils.TAGGED_UNION_TAG_KEY
                )
                if tag is not None:
        ...
        with self.defs.get_schema_or_ref(obj) as (ref, maybe_schema):
            ...
            with self._types_namespace_stack.push(origin):
                annotation = _typing_extra.eval_type_lenient(
                    annotation, self._types_namespace
                )
    ...
    def _literal_schema(self, literal_type: Any) -> CoreSchema:
        ...
        assert (
            expected
        ), f'literal "expected" cannot be empty, obj={literal_type}'
    ...
    def _typed_dict_schema(
        ...
        with self.model_type_stack.push(
            typed_dict_cls
            ...
            with self._config_wrapper_stack.push(
                config
            ), self._types_namespace_stack.push(typed_dict_cls):
                ...
                required_keys: frozenset[
                    str
                ] = typed_dict_cls.__required_keys__
                ...
                for field_name, annotation in get_cls_type_hints_lenient(
                    typed_dict_cls, self._types_namespace
                ).items():
                    annotation = replace_types(annotation, typevars_map)
                ...
                title = self._get_model_title_from_config(
                    typed_dict_cls, ConfigWrapper(config)
                )
                metadata = build_metadata_dict(
                    js_functions=[
                        partial(
    ...
    def _namedtuple_schema(
        ...
        with self.model_type_stack.push(
            ...
            with self._types_namespace_stack.push(namedtuple_cls):
                annotations: dict[str, Any] = get_cls_type_hints_lenient(
                    namedtuple_cls, self._types_namespace
                )
                if not annotations:
                ...
                arguments_schema = core_schema.arguments_schema(
                    ...
                    metadata=build_metadata_dict(
                        js_prefer_positional_arguments=True
                    ),
                )
    ...
    def _tuple_schema(self, tuple_type: Any) -> core_schema.CoreSchema:
        ...
        if typevars_map and params:
            params = tuple(
                replace_types(param, typevars_map) for param in params
            )
    ...
    def _subclass_schema(self, type_: Any) -> core_schema.CoreSchema:
        ...
        if type_param == Any:
            ...
            if type_param.__bound__:
                if _typing_extra.origin_is_union(
                    get_origin(type_param.__bound__)
                ):
    ...
    def _pattern_schema(self, pattern_type: Any) -> core_schema.CoreSchema:
        ...
        if param is str:
            return core_schema.no_info_plain_validator_function(
                _validators.pattern_str_validator,
                serialization=ser,
                metadata=metadata,
            )
        elif param is bytes:
    ...
    def _dataclass_schema(
        self,
        dataclass: type[StandardDataclass],
        origin: type[StandardDataclass] | None,
    ) -> core_schema.CoreSchema:
            ...
            with ExitStack() as dataclass_bases_stack:
                ...
                for dataclass_base in dataclass.__mro__:
                    if dataclasses.is_dataclass(dataclass_base):
                        dataclass_bases_stack.enter_context(
                            self._types_namespace_stack.push(dataclass_base)
                        )
                ...
                for dataclass_base in reversed(dataclass.__mro__):
                    if dataclasses.is_dataclass(dataclass_base):
                        config = getattr(
                            dataclass_base, '__pydantic_config__', None
                        )
                        dataclass_bases_stack.enter_context(
                ...
                title = self._get_model_title_from_config(
                    dataclass, ConfigWrapper(config)
                )
                metadata = build_metadata_dict(
                    js_functions=[
                        partial(
    ...
    def _callable_schema(
        ...
        type_hints = _typing_extra.get_function_type_hints(
            function, types_namespace=self._types_namespace
        )
    ...
    def _apply_annotations(
        ...
        transform_inner_schema: Callable[
        ...
        def inner_handler(obj: Any) -> CoreSchema:
            from_property = self._generate_schema_from_property(
                obj, source_type
            )
            if from_property is None:
                ...
                schema = from_property
            metadata_js_function = _extract_get_pydantic_json_schema(
                obj, schema
            )
            if metadata_js_function is not None:
                metadata_schema = resolve_original_schema(
                ...
                if metadata_schema is not None:
                    self._add_js_function(
                        metadata_schema, metadata_js_function
                    )
    ...
    def _apply_single_annotation_json_schema(
        ...
        if isinstance(metadata, FieldInfo):
            ...
            if metadata.examples:
                json_schema_update['examples'] = to_jsonable_python(
                    metadata.examples
                )
            ...
            if json_schema_update or json_schema_extra:
                CoreMetadataHandler(schema).metadata.setdefault(
                    ...
                    get_json_schema_update_func(
                        json_schema_update, json_schema_extra
                    )
                )
        return schema
    ...
    def _apply_model_serializers(
        ...
        if serializers:
            ...
            info_arg = inspect_model_serializer(
...
_VALIDATOR_F_MATCH: Mapping[
    ...
    Callable[
        [Callable[..., Any], core_schema.CoreSchema, str | None],
        core_schema.CoreSchema,
    ],
] = {
    (
        'before',
        'no-info',
    ): lambda f, schema, _: core_schema.no_info_before_validator_function(
        f, schema
    ),
    (
        'after',
        'no-info',
    ): lambda f, schema, _: core_schema.no_info_after_validator_function(
        f, schema
    ),
    (
        'plain',
    ...
    (
        'wrap',
        'no-info',
    ): lambda f, schema, _: core_schema.no_info_wrap_validator_function(
        f, schema
    ),
    (
        'before',
...
def _extract_get_pydantic_json_schema(
    ...
    if hasattr(tp, '__modify_schema__'):
        ...
        has_custom_v2_modify_js_func = (
            ...
            not in (
                js_modify_function,
                getattr(js_modify_function, '__func__', None),
            )
        )
