...
def set_discriminator_in_metadata(
    schema: CoreSchema, discriminator: Any
) -> None:
...
def apply_discriminators(
    ...
    def inner(
        s: core_schema.CoreSchema, recurse: _core_utils.Recurse
    ) -> core_schema.CoreSchema:
        ...
        discriminator = metadata.pop(
            CORE_SCHEMA_METADATA_DISCRIMINATOR_PLACEHOLDER_KEY, None
        )
...
def apply_discriminator(
    ...
    return _ApplyInferredDiscriminator(discriminator, definitions or {}).apply(
        schema
    )
...
class _ApplyInferredDiscriminator:
    ...
    * Handling various edge cases such as 'definitions', 'default', 'nullable' schemas, and more
    ...
    def _handle_choice(self, choice: core_schema.CoreSchema) -> None:
        ...
        * Updating the _tagged_union_choices mapping that will ultimately be used to build the TaggedUnionSchema.
        ...
        if choice['type'] == 'none':
            ...
            if choice[
                'type'
            ] == 'tagged-union' and self._is_discriminator_shared(choice):
            ...
            self._set_unique_choice_for_values(
                choice, inferred_discriminator_values
            )

    def _is_discriminator_shared(
        self, choice: core_schema.TaggedUnionSchema
    ) -> bool:
    ...
    def _infer_discriminator_values_for_typed_dict_choice(
        self,
        choice: core_schema.TypedDictSchema,
        ...
        source = (
            ...
            else f'TypedDict {source_name!r}'
        )
    ...
    def _infer_discriminator_values_for_model_choice(
        self,
        choice: core_schema.ModelFieldsSchema,
        source_name: str | None = None,
    ) -> list[str | int]:
        source = (
            'ModelFields' if source_name is None else f'Model {source_name!r}'
        )
        field = choice['fields'].get(self.discriminator)
        if field is None:
            raise PydanticUserError(
    ...
    def _infer_discriminator_values_for_dataclass_choice(
        self,
        choice: core_schema.DataclassArgsSchema,
        source_name: str | None = None,
    ) -> list[str | int]:
        source = (
            'DataclassArgs'
            if source_name is None
            else f'Dataclass {source_name!r}'
        )
    ...
    def _infer_discriminator_values_for_inner_schema(
        ...
        if schema['type'] == 'literal':
            ...
            for choice in schema['choices']:
                ...
                choice_values = (
                    self._infer_discriminator_values_for_inner_schema(
    ...
    def _set_unique_choice_for_values(
        ...
        for discriminator_value in values:
            if discriminator_value in self._tagged_union_choices:
                ...
                existing_choice = self._tagged_union_choices[
                    discriminator_value
                ]
