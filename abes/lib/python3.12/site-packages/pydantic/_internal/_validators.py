    ...
    * the substring of `dotted_path` after the colon is not an attribute of the module (e.g., 'collections:abc123')
    ...
    try:
        ...
        if '.' in module_path:
            ...
            try:
                return _import_string_logic(
                    f'{maybe_module_path}:{maybe_attribute}'
                )
            except ImportError:
                pass
...
def pattern_either_validator(input_value: Any, /) -> typing.Pattern[Any]:
    if isinstance(input_value, typing.Pattern):
        ...
        raise PydanticCustomError(
            'pattern_type', 'Input should be a valid pattern'
        )
...
def ip_v4_address_validator(input_value: Any, /) -> IPv4Address:
    ...
    try:
        ...
        raise PydanticCustomError(
            'ip_v4_address', 'Input is not a valid IPv4 address'
        )
...
def ip_v6_address_validator(input_value: Any, /) -> IPv6Address:
    ...
    try:
        ...
        raise PydanticCustomError(
            'ip_v6_address', 'Input is not a valid IPv6 address'
        )
...
def ip_v4_network_validator(input_value: Any, /) -> IPv4Network:
    ...
    try:
        ...
        raise PydanticCustomError(
            'ip_v4_network', 'Input is not a valid IPv4 network'
        )
        ...
        try:
            if not predicate(x, constraint_value):
                raise PydanticKnownError(
                    error_type,
                    context_gen(constraint_value, x)
                    if context_gen
                    else {constraint_id: constraint_value},
                )
        except TypeError:
            raise TypeError(
                f"Unable to apply constraint '{constraint_id}' to supplied value {x}"
            )
        return x
...
_CONSTRAINT_TO_VALIDATOR_LOOKUP: dict[str, Callable] = {
    'gt': create_constraint_validator(
        'gt', lambda x, gt: x > gt, 'greater_than'
    ),
    'ge': create_constraint_validator(
        'ge', lambda x, ge: x >= ge, 'greater_than_equal'
    ),
    'lt': create_constraint_validator('lt', lambda x, lt: x < lt, 'less_than'),
    'le': create_constraint_validator(
        'le', lambda x, le: x <= le, 'less_than_equal'
    ),
    'multiple_of': create_constraint_validator(
        'multiple_of',
        lambda x, multiple_of: x % multiple_of == 0,
        'multiple_of',
    ),
