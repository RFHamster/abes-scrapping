...
if typing.TYPE_CHECKING:
    ...
    StdSchemaFunction = Callable[
        [GenerateSchema, type[Any]], core_schema.CoreSchema
    ]
...
def path_schema_prepare_pydantic_annotations(
    ...
    ) = _known_annotated_metadata.collect_known_metadata(annotations)
    ...
    construct_path = (
        pathlib.PurePath
        if orig_source_type is os.PathLike
        else orig_source_type
    )
    constrained_schema = (
...
class DequeValidator:
    ...
    def __get_pydantic_core_schema__(
        ...
        coerce_instance_wrap = partial(
            ...
            partial(
                deque_validator, maxlen=self.metadata.get('max_length', None)
            ),
        )
...
def defaultdict_validator(
    ...
    if isinstance(input_value, collections.defaultdict):
        ...
        return collections.defaultdict(
            default_default_factory, handler(input_value)
        )
...
def get_defaultdict_default_default_factory(
    ...
    def infer_default() -> Callable[[], Any]:
        ...
        values_type_origin = (
            get_origin(values_source_type) or values_source_type
        )
...
def mapping_like_prepare_pydantic_annotations(
    ...
    if not args:
        ...
        if len(args) != 1:
            raise ValueError(
                'Expected Counter to have exactly 1 generic parameter'
            )
        args = (args[0], int)  # keys are always an int
    elif len(args) != 2:
        raise ValueError(
            'Expected mapping to have exactly 2 generic parameters'
        )
