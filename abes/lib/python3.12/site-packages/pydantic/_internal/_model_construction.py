        ...
        **kwargs: Any,
        ...
        if bases:
            ...
            if __pydantic_generic_metadata__:
                ...
                parameters = (
                    getattr(cls, '__parameters__', None) or parent_parameters
                )
                if (
                    parameters
                    ...
                    if (
                        ...
                        combined_parameters = (
                            parent_parameters + missing_parameters
                        )
                        parameters_str = ', '.join(
...
def inspect_namespace(  # noqa C901
    ...
    * check for items which look like fields but are not (e.g. have no annotation) and warn.
    ...
    for var_name, value in list(namespace.items()):
        ...
        elif isinstance(value, FieldInfo) and not is_valid_field_name(
            var_name
        ):
            suggested_name = (
    ...
    for ann_name, ann_type in raw_annotations.items():
        if (
            ...
            if is_annotated(ann_type):
                ...
                private_attr = next(
                    (v for v in metadata if isinstance(v, ModelPrivateAttr)),
                    None,
                )
...
def make_hash_func(cls: type[BaseModel]) -> Any:
    getter = (
        operator.itemgetter(*cls.model_fields.keys())
        if cls.model_fields
        else lambda _: 0
    )
...
def complete_model_class(
    ...
    *,
    ...
    if (
        config_wrapper.defer_build
        and 'model' in config_wrapper.experimental_defer_build_mode
    ):
...
def set_deprecated_descriptors(cls: type[BaseModel]) -> None:
    ...
    for field, computed_field_info in cls.model_computed_fields.items():
        if (
            ...
            desc = _DeprecatedFieldDescriptor(
                msg, computed_field_info.wrapped_property
            )
            desc.__set_name__(cls, field)
            setattr(cls, field, desc)
...
class _DeprecatedFieldDescriptor:
    ...
    def __init__(
        self, msg: str, wrapped_property: property | None = None
    ) -> None:
        self.msg = msg
...
class _PydanticWeakRef:
    ...
    def __reduce__(
        self,
    ) -> tuple[Callable, tuple[weakref.ReferenceType | None]]:
        return _PydanticWeakRef, (self(),)


def build_lenient_weakvaluedict(
    d: dict[str, Any] | None
) -> dict[str, Any] | None:
