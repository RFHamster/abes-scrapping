...
from .plugin._schema_validator import (
...
class TypeAdapter(Generic[T]):
    ...
    **Note:** By default, `TypeAdapter` does not respect the
    ...
    def _init_core_attrs(self, rebuild_mocks: bool) -> None:
        try:
            ...
            self._core_schema = _get_schema(
                self._type, config_wrapper, parent_depth=self._parent_depth
            )
        ...
        if rebuild_mocks and isinstance(
    ...
    def core_schema(self) -> CoreSchema:
        ...
        if self._core_schema is None or isinstance(
            self._core_schema, _mock_val_ser.MockCoreSchema
        ):
            self._init_core_attrs(
                rebuild_mocks=True
            )  # Do not expose MockCoreSchema from public function
        assert self._core_schema is not None and not isinstance(
            self._core_schema, _mock_val_ser.MockCoreSchema
        )
