    ...
    def __get_pydantic_core_schema__(
        ...
        if isinstance(self, source):
            ...
            _check_annotated_type(
                schema['type'], 'uuid', self.__class__.__name__
            )
    ...
    def validate_file(path: Path, _: core_schema.ValidationInfo) -> Path:
        if path.is_file():
            ...
            raise PydanticCustomError(
                'path_not_file', 'Path does not point to a file'
            )
...
class ByteSize(int):
    ...
    @classmethod
    def _validate(
        cls, input_value: Any, /, _: core_schema.ValidationInfo
    ) -> ByteSize:
        try:
    ...
    def human_readable(
        self, decimal: bool = False, separator: str = ''
    ) -> str:
...
class EncodedBytes:
    ...
    def __get_pydantic_json_schema__(
        ...
        field_schema = handler(core_schema)
        field_schema.update(
            type='string', format=self.encoder.get_json_format()
        )
        return field_schema
...
class EncodedStr(EncodedBytes):
    ...
    def __get_pydantic_core_schema__(
        ...
        return core_schema.with_info_after_validator_function(
            ...
            schema=super(EncodedStr, self).__get_pydantic_core_schema__(
    ...
    def encode_str(self, value: str) -> str:
        ...
        """
        return (
            super(EncodedStr, self).encode(value=value.encode()).decode()
        )  # noqa: UP008
...
class GetPydanticSchema:
    ...
    get_pydantic_core_schema: Callable[
        [Any, GetCoreSchemaHandler], CoreSchema
    ] | None = None
    get_pydantic_json_schema: Callable[
    ...
    if not TYPE_CHECKING:
        ...
        def __getattr__(self, item: str) -> Any:
            ...
            if (
                item == '__get_pydantic_core_schema__'
                and self.get_pydantic_core_schema
            ):
                return self.get_pydantic_core_schema
            elif (
                item == '__get_pydantic_json_schema__'
                and self.get_pydantic_json_schema
            ):
