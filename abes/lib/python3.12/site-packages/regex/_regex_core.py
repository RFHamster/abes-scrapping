...
class RegexFlag(enum.IntFlag):
    ...
    E = (
        ENHANCEMATCH
    ) = 0x8000   # Attempt to improve the fit after finding the first
    # fuzzy match.
...
GLOBAL_FLAGS = (
    _ALL_VERSIONS | BESTMATCH | DEBUG | ENHANCEMATCH | POSIX | REVERSE
)
SCOPED_FLAGS = (
    FULLCASE
    | IGNORECASE
...
def _shrink_cache(
    cache_dict, args_dict, locale_sensitive, max_length, divisor=5
):
            ...
            if ch in ')|':
                ...
                if counts:
                    ...
                    if constraints:
                        ...
                        sequence.append(
                            Character(ord(ch), case_flags=case_flags)
                        )
            else:
                # A literal.
...
def parse_limited_quantifier(source):
    ...
    if max_count is not None and min_count > max_count:
        raise error(
            'min repeat greater than max repeat', source.string, saved_pos
        )
...
def parse_extension(source, info):
    ...
    if ch == '=':
        ...
        if info.is_open_group(name):
            raise error(
                'cannot refer to an open group', source.string, saved_pos
            )
    ...
    if ch == '?':
        ...
        if ch == '<':
            ...
            if ch in ('=', '!'):
                ...
                return parse_lookaround_conditional(
                    source, info, True, ch == '='
                )

        source.pos = saved_pos
        raise error(
            'expected lookaround conditional', source.string, source.pos
        )
...
def parse_lookaround_conditional(source, info, behind, positive):
    ...
    return LookAroundConditional(
        behind, positive, subpattern, yes_branch, no_branch
    )
...
def parse_name(source, allow_numeric=False, allow_group_0=False):
    ...
    if name.isdigit():
        ...
        if not allow_numeric or int(name) < min_group:
            raise error(
                'bad character in group name', source.string, source.pos
            )
    else:
    ...
    if ch in HEX_ESCAPES:
        ...
        return Atomic(
            Branch(
                [
                    String([0x0D, 0x0A]),
                    SetUnion(info, [Character(c) for c in charset]),
                ]
            )
        )
...
def parse_octal_escape(source, info, digits, in_set):
    ...
    try:
        ...
        if digits[0] in OCT_DIGITS:
            raise error(
                'incomplete escape \\%s' % ''.join(digits),
                source.string,
                source.pos,
            )
        else:
            raise error(
                'bad escape \\%s' % digits[0], source.string, source.pos
            )
...
def parse_hex_escape(source, info, esc, expected_len, in_set, type):
    ...
    # Bad hex escape.
    raise error(
        'bad hex escape \\%s%s' % (esc, ''.join(digits)),
        source.string,
        saved_pos,
    )
    ...
    if source.match('{'):
        ...
        if source.match('}'):
            try:
                ...
                raise error(
                    'undefined character name', source.string, source.pos
                )
...
def parse_set_member(source, info):
    ...
    if (
        not isinstance(start, Character)
        or not start.positive
        or not source.match('-')
    ):
...
class ZeroWidthBase(RegexBase):
    ...
    def dump(self, indent, reverse):
        print(
            '{}{} {}'.format(
                INDENT * indent, self._op_name, POS_TEXT[self.positive]
            )
        )
...
class Atomic(RegexBase):
    ...
    def __eq__(self, other):
        return (
            type(self) is type(other) and self.subpattern == other.subpattern
        )
        ...
        for b in branches:
            if Branch._is_simple_character(b):
                ...
                Branch._flush_char_prefix(
                    info, reverse, prefixed, order, new_branches
                )
...
class Character(RegexBase):
    ...
    def __init__(
        self, value, positive=True, case_flags=NOCASE, zerowidth=False
    ):
        RegexBase.__init__(self)
        self.value = value
        self.positive = bool(positive)
        ...
        if (
            self.positive
            and (self.case_flags & FULLIGNORECASE) == FULLIGNORECASE
        ):
...
class Conditional(RegexBase):
    ...
    def get_firstset(self, reverse):
        return self.yes_item.get_firstset(reverse) | self.no_item.get_firstset(
            reverse
        )
...
class GreedyRepeat(RegexBase):
    ...
    def dump(self, indent, reverse):
        ...
        print(
            '{}{} {} {}'.format(
                INDENT * indent, self._op_name, self.min_count, limit
            )
        )
...
class PossessiveRepeat(GreedyRepeat):
    ...
    def _compile(self, reverse, fuzzy):
        ...
        return (
            [(OP.ATOMIC,), tuple(repeat)] + subpattern + [(OP.END,), (OP.END,)]
        )
...
class Range(RegexBase):
    ...
    def __init__(
        self, lower, upper, positive=True, case_flags=NOCASE, zerowidth=False
    ):
        RegexBase.__init__(self)
        self.lower = lower
        self.upper = upper
    ...
    def optimise(self, info, reverse, in_set=False):
        ...
        for ch in expanding_chars:
            if self.lower <= ord(ch) <= self.upper:
                ...
                items.append(
                    String(
                        [ord(c) for c in folded], case_flags=self.case_flags
                    )
                )
    ...
    def _compile(self, reverse, fuzzy):
        ...
        return [
            (
                self._opcode[self.case_flags, reverse],
                flags,
                self.lower,
                self.upper,
            )
...
class Sequence(RegexBase):
        ...
        for s in self.items:
            if type(s) is Character and s.positive and not s.zerowidth:
                if s.case_flags != case_flags:
                    ...
                    if s.case_flags or is_cased_i(info, s.value):
                        Sequence._flush_characters(
                            info, characters, case_flags, items
                        )
                ...
                if s.case_flags != case_flags:
                    ...
                    if s.case_flags or any(
                        is_cased_i(info, c) for c in characters
                    ):
                        Sequence._flush_characters(
                            info, characters, case_flags, items
                        )
    ...
    def _flush_characters(info, characters, case_flags, items):
        ...
        if (case_flags & FULLIGNORECASE) == FULLIGNORECASE:
            ...
            for item in literals:
                ...
                if len(chars) == 1:
                    items.append(
                        Character(chars[0], case_flags=item.case_flags)
                    )
                else:
    ...
    def _fix_full_casefold(characters):
        ...
        for start, end in Sequence._merge_chunks(chunks):
            if pos < start:
                literals.append(
                    Literal(characters[pos:start], case_flags=IGNORECASE)
                )

            literals.append(
                Literal(characters[start:end], case_flags=FULLIGNORECASE)
            )
            pos = end
...
class SetBase(RegexBase):
    def __init__(
        self, info, items, positive=True, case_flags=NOCASE, zerowidth=False
    ):
        RegexBase.__init__(self)
        self.info = info
    ...
    def optimise(self, info, reverse, in_set=False):
        ...
        if len(items) == 1:
            return (
                items[0]
                .with_flags(
                    case_flags=self.case_flags, zerowidth=self.zerowidth
                )
                .optimise(info, reverse, in_set)
            )

        self.items = tuple(
            m.optimise(info, reverse, in_set=True) for m in items
        )
...
class String(RegexBase):
    ...
    def get_firstset(self, reverse):
        if reverse:
            ...
            pos = 0
        return set(
            [Character(self.characters[pos], case_flags=self.case_flags)]
        )
...
class StringSet(Branch):
    def __init__(self, info, name, case_flags=NOCASE):
        ...
        for string in items:
            ...
            choices.append(
                [Character(c, case_flags=case_flags) for c in string]
            )
    ...
    def __init__(self, lexicon, flags=0):
        ...
        for phrase, action in lexicon:
            ...
            if not source.at_end():
                raise error(
                    'unbalanced parenthesis', source.string, source.pos
                )
        ...
        req_offset, req_chars, req_flags = _get_required_string(
            parsed, info.flags
        )
...
for prop_name, (prop_id, values) in PROPERTIES.items():
    ...
    for val_name, val_id in values.items():
        prop_values[val_id] = max(
            prop_values.get(val_id, ''), val_name, key=len
        )
