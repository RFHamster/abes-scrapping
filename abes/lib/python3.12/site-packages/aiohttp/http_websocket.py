...
_WS_EXT_RE_SPLIT: Final[Pattern[str]] = re.compile(
    r'permessage-deflate([^,]+)?'
)


def ws_ext_parse(
    extstr: Optional[str], isserver: bool = False
) -> Tuple[int, bool]:
    if not extstr:
        return 0, False
...
def ws_ext_gen(
    ...
    if compress < 9 or compress > 15:
        raise ValueError(
            'Compress wbits must between 9 and 15, '
...
class WebSocketReader:
        ...
        for fin, opcode, payload, compressed in self.parse_frame(data):
            if opcode in MESSAGE_TYPES_WITH_CONTENT:
                ...
                if not fin:
                    ...
                    if (
                        self._max_msg_size
                        and len(self._partial) >= self._max_msg_size
                    ):
                ...
                if (
                    self._max_msg_size
                    and len(assembled_payload) >= self._max_msg_size
                ):
                    raise WebSocketError(
                        WSCloseCode.MESSAGE_TOO_BIG,
                        'Message size {} exceeds limit {}'.format(
                ...
                if compressed:
                    ...
                    payload_merged = self._decompressobj.decompress_sync(
                        assembled_payload + _WS_DEFLATE_TRAILING,
                        self._max_msg_size,
                    )
                ...
                if len(payload) >= 2:
                    ...
                    if (
                        close_code < 3000
                        and close_code not in ALLOWED_CLOSE_CODES
                    ):
                        raise WebSocketError(
                            WSCloseCode.PROTOCOL_ERROR,
    ...
    def parse_frame(
        ...
        frames: List[
            Tuple[bool, Optional[int], bytearray, Optional[bool]]
        ] = []
        if self._tail:
...
class WebSocketWriter:
    ...
    async def close(
        self, code: int = 1000, message: Union[bytes, str] = b''
    ) -> None:
