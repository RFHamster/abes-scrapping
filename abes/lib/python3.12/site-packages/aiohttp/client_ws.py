...
class ClientWebSocketResponse:
    ...
    def _reset_heartbeat(self) -> None:
        ...
        when = calculate_timeout_when(
            now, self._heartbeat, timeout_ceil_threshold
        )
    ...
    def _send_heartbeat(self) -> None:
        ...
        when = calculate_timeout_when(
            now, self._pong_heartbeat, timeout_ceil_threshold
        )
        self._cancel_pong_response_cb()
        ...
        if sys.version_info >= (3, 12):
            ...
            ping_task = asyncio.Task(
                self._writer.ping(), loop=loop, eager_start=True
            )
        else:
    ...
    async def send_bytes(
        self, data: bytes, compress: Optional[int] = None
    ) -> None:
    ...
    async def receive_str(self, *, timeout: Optional[float] = None) -> str:
        ...
        if msg.type is not WSMsgType.TEXT:
            raise TypeError(
                f'Received message {msg.type}:{msg.data!r} is not str'
            )
    ...
    async def receive_bytes(self, *, timeout: Optional[float] = None) -> bytes:
        ...
        if msg.type is not WSMsgType.BINARY:
            raise TypeError(
                f'Received message {msg.type}:{msg.data!r} is not bytes'
            )
