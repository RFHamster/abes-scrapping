...
class Connection:
    ...
    def __del__(self, _warnings: Any = warnings) -> None:
        if self._protocol is not None:
            kwargs = {'source': self}
            _warnings.warn(
                f'Unclosed connection {self!r}', ResourceWarning, **kwargs
            )
    ...
    def loop(self) -> asyncio.AbstractEventLoop:
        warnings.warn(
            'connector.loop property is deprecated',
            DeprecationWarning,
            stacklevel=2,
        )
        return self._loop
...
class BaseConnector:
        ...
        *,
        ...
        if force_close:
            if (
                ...
                and keepalive_timeout is not sentinel
            ):
                raise ValueError(
        ...
        self._conns: Dict[
            ConnectionKey, List[Tuple[ResponseHandler, float]]
        ] = {}
        self._limit = limit
        ...
        self._waiters: DefaultDict[
            ConnectionKey, deque[asyncio.Future[None]]
        ] = defaultdict(deque)
    ...
    def __del__(self, _warnings: Any = warnings) -> None:
        ...
        kwargs = {'source': self}
        _warnings.warn(
            f'Unclosed connector {self!r}', ResourceWarning, **kwargs
        )
        context = {
            'connector': self,
            'connections': conns,
...
class TCPConnector(BaseConnector):
    ...
    def __init__(
        ...
        *,
        ...
        self._ssl = _merge_ssl_params(
            ssl, verify_ssl, ssl_context, fingerprint
        )
        if resolver is None:
        ...
        self._family = family
        self._local_addr_infos = aiohappyeyeballs.addr_to_addr_infos(
            local_addr
        )
        self._happy_eyeballs_delay = happy_eyeballs_delay
        self._interleave = interleave
        self._resolve_host_tasks: Set[
            'asyncio.Task[List[ResolveResult]]'
        ] = set()
    ...
    async def _resolve_host(
        ...
        #
        coro = self._resolve_host_with_throttle(
            key, host, port, futures, traces
        )
        loop = asyncio.get_running_loop()
        if sys.version_info >= (3, 12):
            # Optimization for Python 3.12, try to send immediately
            resolved_host_task = asyncio.Task(
                coro, loop=loop, eager_start=True
            )
        else:
        ...
        if not resolved_host_task.done():
            ...
            resolved_host_task.add_done_callback(
                self._resolve_host_tasks.discard
            )
        ...
        try:
            ...
            def drop_exception(
                fut: 'asyncio.Future[List[ResolveResult]]',
            ) -> None:
    ...
    async def _resolve_host_with_throttle(
        ...
        try:
            ...
            addrs = await self._resolver.resolve(
    ...
    async def _create_direct_connection(
        ...
        *,
        ...
        while addr_infos:
            ...
            server_hostname = (
                (req.server_hostname or host).rstrip('.')
                if sslcontext
                else None
            )
            ...
            try:
                ...
                try:
                    ...
                    if not runtime_has_start_tls:
                        rawsock = transport.get_extra_info(
                            'socket', default=None
                        )
                        if rawsock is None:
                            raise RuntimeError(
