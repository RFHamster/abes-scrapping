...
DEFAULT_TIMEOUT: Final[ClientTimeout] = ClientTimeout(
    total=5 * 60, sock_connect=30
)
...
class ClientSession:
        ...
        if timeout is sentinel or timeout is None:
            ...
            if conn_timeout is not None:
                self._timeout = attr.evolve(
                    self._timeout, connect=conn_timeout
                )
                warnings.warn(
            ...
            with timer:
                ...
                while True:
                    ...
                    try:
                        ...
                        if exc.errno is None and isinstance(
                    ...
                    if (
                        ...
                        if (
                            resp.status == 303
                            and resp.method != hdrs.METH_HEAD
                        ) or (
        ...
        *,
        ...
        try:
            ...
            match = base64.b64encode(
                hashlib.sha1(sec_key + WS_KEY).digest()
            ).decode()
            if r_key != match:
                raise WSServerHandshakeError(
                    resp.request_info,
            ...
            if receive_timeout is None:
                ...
                conn_proto.read_timeout = max(
                    receive_timeout, conn_proto.read_timeout
                )
    ...
    def loop(self) -> asyncio.AbstractEventLoop:
        """Session's loop."""
        warnings.warn(
            'client.loop property is deprecated',
            DeprecationWarning,
            stacklevel=2,
        )
        return self._loop
...
class _BaseRequestContextManager(
    Coroutine[Any, Any, _RetType], Generic[_RetType]
):

    __slots__ = ('_coro', '_resp')

    def __init__(
        self, coro: Coroutine['asyncio.Future[Any]', None, _RetType]
    ) -> None:
