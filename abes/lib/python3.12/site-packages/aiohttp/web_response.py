...
REASON_PHRASES = {
    http_status.value: http_status.phrase for http_status in HTTPStatus
}
...
class StreamResponse(BaseClass, HeadersMixin):
    ...
    def enable_chunked_encoding(
        ...
        if hdrs.CONTENT_LENGTH in self._headers:
            raise RuntimeError(
                "You can't enable chunked encoding when "
    ...
    def enable_compression(
        ...
        if isinstance(force, bool):
            ...
            warnings.warn(
                'Using boolean for force is deprecated #3318',
                DeprecationWarning,
            )
    ...
    def content_length(self, value: Optional[int]) -> None:
        if value is not None:
            ...
            if self._chunked:
                raise RuntimeError(
                    "You can't set content length when "
    ...
    def etag(self, value: Optional[Union[ETag, str]]) -> None:
        if value is None:
            ...
            hdr_value = (
                f'W/"{value.value}"' if value.is_weak else f'"{value.value}"'
            )
    ...
    async def prepare(
        ...
        self._must_be_empty_body = must_be_empty_body(
    ...
    async def _prepare_headers(self) -> None:
        ...
        if self._must_be_empty_body:
            if (
                hdrs.CONTENT_LENGTH in headers
                and should_remove_content_length(request.method, self.status)
            ):
                del headers[hdrs.CONTENT_LENGTH]
    ...
    async def _write_headers(self) -> None:
        ...
        version = request.version
        status_line = (
            f'HTTP/{version[0]}.{version[1]} {self._status} {self._reason}'
        )
    ...
    async def drain(self) -> None:
        ...
        assert (
            self._payload_writer is not None
...
class Response(StreamResponse):
    ...
    async def _do_start_compression(self, coding: ContentCoding) -> None:
        ...
        if coding != ContentCoding.identity:
            ...
            if (
                self._zlib_executor_size is None
                and len(self._body) > 1024 * 1024
            ):
                warnings.warn(
            ...
            self._headers[hdrs.CONTENT_ENCODING] = coding.value
            self._headers[hdrs.CONTENT_LENGTH] = str(
                len(self._compressed_body)
            )
