...
_YARL_SUPPORTS_HOST_SUBCOMPONENT = tuple(
...
class ClientRequest:
    ...
    def update_auto_headers(
        self, skip_auto_headers: Optional[Iterable[str]]
    ) -> None:
    ...
    def update_auth(
        self, auth: Optional[BasicAuth], trust_env: bool = False
    ) -> None:
        """Set basic auth."""
        if auth is None:
            auth = self.auth
    ...
    def __del__(self, _warnings: Any = warnings) -> None:
        ...
        if self._connection is not None:
            ...
            if self._loop.get_debug():
                kwargs = {'source': self}
                _warnings.warn(
                    f'Unclosed response {self!r}', ResourceWarning, **kwargs
                )
                context = {
    ...
    def _release_connection(self) -> None:
        if self._connection is not None:
            if self.__writer is None:
                ...
                self.__writer.add_done_callback(
                    lambda f: self._release_connection()
                )
