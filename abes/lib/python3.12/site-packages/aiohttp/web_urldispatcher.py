...
YARL_VERSION: Final[Tuple[int, ...]] = tuple(
    map(int, yarl_version.split('.')[:2])
)
    ...
    async def resolve(self, request: Request) -> _Resolve:
        ...
        for route_obj in self._routes:
            ...
            if route_method == request.method or route_method == hdrs.METH_ANY:
                return (
                    UrlMappingMatchInfo(match_dict, route_obj),
                    allowed_methods,
                )
        else:
            return None, allowed_methods
...
class StaticResource(PrefixResource):
    ...
    async def resolve(self, request: Request) -> _Resolve:
        ...
        match_dict = {
            'filename': _unquote_path_safe(path[len(self._prefix) + 1 :])
        }
        return (
...
class ResourcesView(
    Sized, Iterable[AbstractResource], Container[AbstractResource]
):
    ...
    def register_resource(self, resource: AbstractResource) -> None:
        ...
        if name is not None:
            ...
            if name in self._named_resources:
                raise ValueError(
                    'Duplicate {!r}, '
                    'already handled by {!r}'.format(
                        name, self._named_resources[name]
                    )
                )
    ...
    def add_route(
        ...
        *,
        ...
        return resource.add_route(
            method, handler, expect_handler=expect_handler
        )
