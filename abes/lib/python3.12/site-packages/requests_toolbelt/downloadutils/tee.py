...
def _tee(response, callback, chunksize, decode_content):
    for chunk in response.raw.stream(
        amt=chunksize, decode_content=decode_content
    ):
        callback(chunk)
        yield chunk
...
def tee(
    response, fileobject, chunksize=_DEFAULT_CHUNKSIZE, decode_content=None
):
    ...
    if not (
        'b' in getattr(fileobject, 'mode', '')
        or isinstance(fileobject, io.BytesIO)
    ):
        raise TypeError(
...
def tee_to_file(
    response, filename, chunksize=_DEFAULT_CHUNKSIZE, decode_content=None
):
    """Stream the response both to the generator and a file.
...
def tee_to_bytearray(
    ...
    if not isinstance(bytearr, bytearray):
        raise TypeError(
            'tee_to_bytearray() expects bytearr to be a ' 'bytearray'
        )
