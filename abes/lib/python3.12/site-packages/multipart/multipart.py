...
class File:
    ...
    def on_data(self, data: bytes) -> int:
        ...
        if bwritten != len(data):
            self.logger.warning(
                'bwritten != len(data) (%d != %d)', bwritten, len(data)
            )
            return bwritten
...
class BaseParser:
    ...
    def callback(
        ...
        if data is not None:
            ...
            self.logger.debug(
                'Calling %s with data[%d:%d]', on_name, start, end
            )
            func(data, start, end)
        else:
...
class OctetStreamParser(BaseParser):
    ...
    def __init__(
        ...
        if not isinstance(max_size, Number) or max_size < 1:
            raise ValueError(
                'max_size must be a positive number, not %r' % max_size
            )
                    ...
                    if found_sep:
                        ...
                        if strict_parsing:
                            e = QuerystringParseError(
                                'Skipping duplicate ampersand/semicolon at %d'
                                % i
                            )
                            e.offset = i
                            raise e
            ...
            if state == MultipartState.START:
                ...
                if index == len(boundary) - 2:
                    ...
                    if c != boundary[index + 2]:
                        msg = (
                            'Expected boundary character %r, got %r at index %d'
                            % (boundary[index + 2], c, index + 2)
                        )
                        self.logger.warning(msg)
                        e = MultipartParseError(msg)
                        e.offset = i
                ...
                if c != LF:
                    msg = (
                        'Did not find LF character at end of header '
                        '(found %r)' % (c,)
                    )
                    self.logger.warning(msg)
                    e = MultipartParseError(msg)
                ...
                if c not in (CR, LF):
                    self.logger.warning(
                        "Consuming a byte '0x%x' in the end state", c
                    )  # pragma: no cover
...
class FormParser:
    ...
    def __init__(
        ...
        parser: OctetStreamParser | MultipartParser | QuerystringParser | None = (
            None
        )
        ...
        if content_type == 'application/octet-stream':
            ...
            def on_headers_finished() -> None:
                ...
                if transfer_encoding in (b'binary', b'8bit', b'7bit'):
                    ...
                    self.logger.warning(
                        'Unknown Content-Transfer-Encoding: %r',
                        transfer_encoding,
                    )
