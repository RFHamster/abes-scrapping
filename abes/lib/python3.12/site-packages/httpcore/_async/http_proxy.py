...
def merge_headers(
    ...
    override_headers = (
        [] if override_headers is None else list(override_headers)
    )
        ...
        async with self._connect_lock:
            if not self._connected:
                ...
                if (
                    ...
                    or connect_response.status > 299
                ):
                ...
                kwargs = {
                    'ssl_context': ssl_context,
                    'server_hostname': self._remote_origin.host.decode(
                        'ascii'
                    ),
                    'timeout': timeout,
                }
                async with Trace(
                    'start_tls', logger, request, kwargs
                ) as trace:
