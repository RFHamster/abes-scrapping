...
class AsyncHTTP11Connection(AsyncConnectionInterface):
    ...
    async def _receive_event(
        ...
        while True:
            with map_exceptions(
                {h11.RemoteProtocolError: RemoteProtocolError}
            ):
            ...
            if event is h11.NEED_DATA:
                ...
                if (
                    data == b''
                    and self._h11_state.their_state == h11.SEND_RESPONSE
                ):
    ...
    def has_expired(self) -> bool:
        now = time.monotonic()
        keepalive_expired = (
            self._expire_at is not None and now > self._expire_at
        )
...
class HTTP11ConnectionByteStream:
    def __init__(
        self, connection: AsyncHTTP11Connection, request: Request
    ) -> None:
...
class AsyncHTTP11UpgradeStream(AsyncNetworkStream):
    def __init__(
        self, stream: AsyncNetworkStream, leading_data: bytes
    ) -> None:
        self._stream = stream
        self._leading_data = leading_data

    async def read(
        self, max_bytes: int, timeout: Optional[float] = None
    ) -> bytes:
    ...
    async def write(
        self, buffer: bytes, timeout: Optional[float] = None
    ) -> None:
