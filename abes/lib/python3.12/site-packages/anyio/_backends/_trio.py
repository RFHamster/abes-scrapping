            ...
            while True:
                try:
                    (
                        ...
                        flags,
                        addr,
                    ) = await self._trio_socket.recvmsg(
...
class ConnectedUDPSocket(
    _TrioSocketMixin[IPSockAddrType], abc.ConnectedUDPSocket
):
    def __init__(self, trio_socket: TrioSocketType) -> None:
        super().__init__(trio_socket)
        self._receive_guard = ResourceGuard('reading from')
...
class Lock(BaseLock):
    ...
    def statistics(self) -> LockStatistics:
        orig_statistics = self.__original.statistics()
        owner = (
            TrioTaskInfo(orig_statistics.owner)
            if orig_statistics.owner
            else None
        )
...
class TrioBackend(AsyncBackend):
    ...
    async def open_process(
        ...
        **kwargs: Any,
        ...
        stdin_stream = (
            SendStreamWrapper(process.stdin) if process.stdin else None
        )
        stdout_stream = (
            ReceiveStreamWrapper(process.stdout) if process.stdout else None
        )
        stderr_stream = (
            ReceiveStreamWrapper(process.stderr) if process.stderr else None
        )
