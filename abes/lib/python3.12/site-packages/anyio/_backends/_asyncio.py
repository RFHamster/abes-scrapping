...
def _task_started(task: asyncio.Task) -> bool:
    ...
    try:
        return getcoroutinestate(task.get_coro()) in (
            CORO_RUNNING,
            CORO_SUSPENDED,
        )
        ...
        raise Exception(
            f'Cannot determine if task {task} has started or not'
        ) from None
    ...
    def _deliver_cancellation(self, origin: CancelScope) -> bool:
        ...
        for scope in self._child_scopes:
            if not scope._shield and not scope.cancel_called:
                should_retry = (
                    scope._deliver_cancellation(origin) or should_retry
                )
    ...
    def _spawn(
        ...
        def task_done(_task: asyncio.Task) -> None:
            ...
            if exc is not None:
                ...
                if (
                    task_status_future is not None
                    and task_status_future.cancelled()
                ):
                    return
                ...
                task_status_future is not None
                and not task_status_future.done()
            ):
        ...
        if not iscoroutine(coro):
            prefix = (
                f'{func.__module__}.' if hasattr(func, '__module__') else ''
            )
            raise TypeError(
    ...
    async def receive_fds(
        ...
        for cmsg_level, cmsg_type, cmsg_data in ancdata:
            if (
                ...
                or cmsg_type != socket.SCM_RIGHTS
            ):
                raise RuntimeError(
            ...
            while True:
                try:
                    ...
                    self._loop.add_reader(
                        self.__raw_socket, f.set_result, None
                    )
                    f.add_done_callback(
...
class ConnectedUNIXDatagramSocket(
    _RawSocketMixin, abc.ConnectedUNIXDatagramSocket
):
    async def receive(self) -> bytes:
        loop = get_running_loop()
        await AsyncIOBackend.checkpoint()
...
class Lock(BaseLock):
    ...
    def statistics(self) -> LockStatistics:
        task_info = (
            AsyncIOTaskInfo(self._owner_task) if self._owner_task else None
        )
...
class AsyncIOBackend(AsyncBackend):
    ...
    @classmethod
    def setup_process_pool_exit_at_shutdown(
