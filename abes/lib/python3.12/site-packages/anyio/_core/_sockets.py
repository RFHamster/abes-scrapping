...
async def connect_tcp(
    ...
    *,
    ...
    async def try_connect(remote_host: str, event: Event) -> None:
        ...
        try:
            stream = await asynclib.connect_tcp(
                remote_host, remote_port, local_address
            )
...
def getnameinfo(
    sockaddr: IPSockAddrType, flags: int = 0
    ...
    if path_str is not None:
        try:
            await to_thread.run_sync(
                raw_socket.bind, path_str, abandon_on_cancel=True
            )
            if mode is not None:
                await to_thread.run_sync(
                    chmod, path_str, mode, abandon_on_cancel=True
                )
        except BaseException:
            raw_socket.close()
            raise
