...
async def _gen_check_output(
    ...
    try:
        (stdout, stderr) = await asyncio.wait_for(
            process.communicate(stdin), timeout
        )
...
async def black_run(
    ...
    if not repo_path:
        ...
        results.failed_projects[project_name] = CalledProcessError(
            69,
            [],
            f'{project_name} has no repo_path: {repo_path}'.encode(),
            b'',
        )
        return
...
async def git_checkout_or_rebase(
    ...
    *,
    ...
    try:
        ...
        LOG.error(
            f"Unable to git clone / pull {project_config['git_clone_url']}: {e}"
        )
        return None
...
def handle_PermissionError(
    ...
    if func in (os.rmdir, os.unlink) and excvalue.errno == errno.EACCES:
        LOG.debug(
            f'Setting {path} writable, readable, and executable by everyone... '
        )
        os.chmod(
...
async def project_runner(
    ...
    while True:
        ...
        if (
            ...
            LOG.debug(
                f"Skipping {project_name} as it's not enabled for {py_version}"
            )
            continue
        ...
        if not long_checkouts and project_config['long_checkout']:
            ...
            LOG.debug(
                f"Skipping {project_name} as it's configured as a long checkout"
            )
            continue
        ...
        if not stdin_project:
            repo_path = await git_checkout_or_rebase(
                work_path, project_config, rebase
            )
            if not repo_path:
                continue
        await black_run(
            project_name, repo_path, project_config, results, no_diff
        )
...
async def process_queue(
    ...
    config, queue = await load_projects_queue(
        Path(config_file), projects_to_run
    )
