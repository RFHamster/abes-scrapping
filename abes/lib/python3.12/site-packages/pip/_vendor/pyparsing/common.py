...
class pyparsing_common:
    ...
    integer = (
        Word(nums).set_name('integer').set_parse_action(convert_to_integer)
    )
    ...
    _short_ipv6_address.add_condition(
        lambda t: sum(1 for tt in t if pyparsing_common._ipv6_part.matches(tt))
        < 8
    )
    _mixed_ipv6_address = ('::ffff:' + ipv4_address).set_name(
        'mixed IPv6 address'
    )
