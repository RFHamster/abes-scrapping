    ...
    def __getattr__(self, name: str) -> Any:
        try:
            ...
            raise AttributeError(
                f"'EvalError' object has no attribute '{name}'"
            )
...
def _get_prompt(inputs: Dict[str, Any]) -> str:
    ...
    if 'prompt' in inputs:
        ...
        if isinstance(prompt_, str):
            prompts = [prompt_]
        elif isinstance(prompt_, list) and all(
            isinstance(i, str) for i in prompt_
        ):
            prompts = prompt_
        else:
            raise InputFormatError(
...
def _setup_evaluation(
    ...
    if evaluation:
        if isinstance(llm_or_chain_factory, BaseLanguageModel):
            ...
            run_outputs = (
                chain.output_keys if isinstance(chain, Chain) else None
            )
...
def _load_run_evaluators(
    ...
    if config.evaluators or (
        ...
        and any(
            [isinstance(e, StringEvaluator) for e in config.custom_evaluators]
        )
    ):
    ...
    if input_mapper is not None:
        ...
        try:
            ...
            llm_output = llm.invoke(
                **llm_inputs,
                config=RunnableConfig(
                    callbacks=callbacks, metadata=metadata or {}
                ),
            )
    return llm_output
...
def _prepare_eval_run(
    ...
    examples = list(
        client.list_examples(dataset_id=dataset.id, as_of=dataset_version)
    )
    if not examples:
...
class _DatasetRunContainer:
    ...
    batch_evaluators: Optional[
        List[smith_eval_config.BATCH_EVALUATOR_LIKE]
    ] = None
    ...
    def _merge_test_outputs(
        ...
        for example, output in zip(self.examples, batch_results):
            row_result = cast(
                _RowResult, all_eval_results.get(str(example.id), {})
            )
    ...
    def prepare(
        ...
        return cls(
            ...
            examples=examples,
            configs=configs,
            batch_evaluators=evaluation.batch_evaluators
            if evaluation
            else None,
        )
...
async def arun_on_dataset(
    ...
    **kwargs: Any,
    ...
    if input_mapper:
        warn_deprecated(
            '0.0.305', message=_INPUT_MAPPER_DEP_WARNING, pending=True
        )
...
def run_on_dataset(
    ...
    **kwargs: Any,
    ...
    if concurrency_level == 0:
        ...
        with runnable_config.get_executor_for_config(
            container.configs[0]
        ) as executor:
            batch_results = list(
                executor.map(
