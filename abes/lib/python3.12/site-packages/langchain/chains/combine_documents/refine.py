...
class RefineDocumentsChain(BaseCombineDocumentsChain):
    ...
    async def acombine_docs(
        ...
        """
        inputs = self._construct_initial_inputs(docs, **kwargs)
        res = await self.initial_llm_chain.apredict(
        ...
        for doc in docs[1:]:
            base_inputs = self._construct_refine_inputs(doc, res)
            inputs = {**base_inputs, **kwargs}
            res = await self.refine_llm_chain.apredict(
                callbacks=callbacks, **inputs
            )
            refine_steps.append(res)
        return self._construct_result(refine_steps, res)
    ...
    def _construct_result(
        self, refine_steps: List[str], res: str
    ) -> Tuple[str, dict]:
        if self.return_intermediate_steps:
    ...
    def _construct_refine_inputs(
        self, doc: Document, res: str
    ) -> Dict[str, Any]:
        return {
            self.document_variable_name: format_document(
