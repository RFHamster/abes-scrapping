...
def _create_openai_json_runnable(
    ...
    *,
    ...
    """"""
    if isinstance(output_schema, type) and is_basemodel_subclass(
        output_schema
    ):
        output_parser = output_parser or PydanticOutputParser(
            pydantic_object=output_schema,  # type: ignore
        )
        schema_as_dict = convert_to_openai_function(output_schema)[
            'parameters'
        ]
    else:
    ...
    if prompt:
        if 'output_schema' in prompt.input_variables:
            prompt = prompt.partial(
                output_schema=json.dumps(schema_as_dict, indent=2)
            )
