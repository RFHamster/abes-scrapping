...
from langchain.chains.elasticsearch_database.prompts import (
    ANSWER_PROMPT,
    DSL_PROMPT,
)
...
class ElasticsearchDatabaseChain(Chain):
    ...
    def _list_indices(self) -> List[str]:
        all_indices = [
            index['index']
            for index in self.database.cat.indices(format='json')
        ]
        ...
        if self.ignore_indices:
            all_indices = [
                i for i in all_indices if i not in self.ignore_indices
            ]
