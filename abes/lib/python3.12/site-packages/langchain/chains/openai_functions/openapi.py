...
def _openapi_params_to_json_schema(
    params: List[Parameter], spec: OpenAPISpec
) -> dict:
    properties = {}
    required = []
    for p in params:
        ...
        for method in spec.get_methods_for_path(path):
            ...
            if request_body and request_body.content:
                ...
                if len(media_types) == 1:
                    ...
                    key = (
                        'json' if media_type == 'application/json' else 'data'
                    )
                    request_args[key] = schema_dict
                elif len(media_types) > 1:
                    request_args['data'] = {
                        'anyOf': list(media_types.values())
                    }
...
class SimpleRequestChain(Chain):
    ...
    def _call(
        ...
        _text = (
            f'Calling endpoint {_pretty_name} with arguments:\n' + _pretty_args
        )
