...
URL_BASE = (
    'https://raw.githubusercontent.com/hwchase17/langchain-hub/master/chains/'
)
...
def _load_stuff_documents_chain(
    config: dict, **kwargs: Any
) -> StuffDocumentsChain:
    if 'llm_chain' in config:
        llm_chain_config = config.pop('llm_chain')
...
def _load_llm_checker_chain(config: dict, **kwargs: Any) -> LLMCheckerChain:
    ...
    if 'list_assertions_prompt' in config:
        ...
        list_assertions_prompt = load_prompt(
            config.pop('list_assertions_prompt_path')
        )
    ...
    if 'revised_answer_prompt' in config:
        ...
        revised_answer_prompt = load_prompt(
            config.pop('revised_answer_prompt_path')
        )
...
def _load_refine_documents_chain(
    config: dict, **kwargs: Any
) -> RefineDocumentsChain:
    if 'initial_llm_chain' in config:
        initial_llm_chain_config = config.pop('initial_llm_chain')
        initial_llm_chain = load_chain_from_config(
            initial_llm_chain_config, **kwargs
        )
    elif 'initial_llm_chain_path' in config:
        initial_llm_chain = load_chain(
            config.pop('initial_llm_chain_path'), **kwargs
        )
    else:
        raise ValueError(
    ...
    if 'refine_llm_chain' in config:
        ...
        refine_llm_chain = load_chain_from_config(
            refine_llm_chain_config, **kwargs
        )
    elif 'refine_llm_chain_path' in config:
        refine_llm_chain = load_chain(
            config.pop('refine_llm_chain_path'), **kwargs
        )
    else:
        raise ValueError(
...
def _load_qa_with_sources_chain(
    config: dict, **kwargs: Any
) -> QAWithSourcesChain:
    if 'combine_documents_chain' in config:
...
def _load_api_chain(config: dict, **kwargs: Any) -> APIChain:
    if 'api_request_chain' in config:
        ...
        api_request_chain = load_chain_from_config(
            api_request_chain_config, **kwargs
        )
    ...
    if 'api_answer_chain' in config:
        ...
        api_answer_chain = load_chain_from_config(
            api_answer_chain_config, **kwargs
        )
    elif 'api_answer_chain_path' in config:
        api_answer_chain = load_chain(
            config.pop('api_answer_chain_path'), **kwargs
        )
    else:
        raise ValueError(
