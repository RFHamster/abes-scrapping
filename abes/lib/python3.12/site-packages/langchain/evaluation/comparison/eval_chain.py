...
class PairwiseStringEvalChain(PairwiseStringEvaluator, LLMEvalChain, LLMChain):
    ...
    def from_llm(
        ...
        **kwargs: Any,
        ...
        if not hasattr(llm, 'model_name') or not llm.model_name.startswith(
            'gpt-4'
        ):
            logger.warning(
...
Performance may be significantly worse with other models.'
        ...
        criteria_str = (
            CRITERIA_INSTRUCTIONS + criteria_str if criteria_str else ''
        )
        return cls(
            llm=llm, prompt=prompt_.partial(criteria=criteria_str), **kwargs
        )
    ...
    def _evaluate_string_pairs(
        ...
        **kwargs: Any,
        ...
        input_ = self._prepare_input(
            prediction, prediction_b, input, reference
        )
        result = self(
            inputs=input_,
            callbacks=callbacks,
