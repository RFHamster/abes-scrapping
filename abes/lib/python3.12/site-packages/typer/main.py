...
def get_install_completion_arguments() -> Tuple[
    click.Parameter, click.Parameter
]:
...
def get_command(typer_instance: Typer) -> click.Command:
    ...
    if (
        ...
        if not single_command.context_settings and not isinstance(
            ...
            single_command.context_settings = (
                typer_instance.info.context_settings
            )
    ...
    if typer_info.typer_instance:
        ...
        if typer_info.typer_instance.info.callback:
            return get_command_name(
                typer_info.typer_instance.info.callback.__name__
            )
    return None
...
def get_group_from_info(
    ...
    *,
    ...
    assert issubclass(
        cls, TyperGroup
    ), f'{cls} should be a subclass of {TyperGroup}'
    group = cls(
...
def get_params_convertors_ctx_param_name_from_function(
    ...
    List[Union[click.Argument, click.Option]], Dict[str, Any], Optional[str]
]:
    params = []
    convertors = {}
...
def get_click_type(
    *, annotation: Any, parameter_info: ParameterInfo
    ...
    if parameter_info.click_type is not None:
        ...
        if parameter_info.min is not None or parameter_info.max is not None:
            ...
            return click.IntRange(
                min=min_, max=max_, clamp=parameter_info.clamp
            )
        else:
            return click.INT
    ...
    raise RuntimeError(
        f'Type not yet supported: {annotation}'
    )  # pragma: no cover
...
def get_click_param(
    ...
    raise AssertionError(
        'A click.Parameter should be returned'
    )  # pragma: no cover
...
def get_param_completion(
    ...
    def wrapper(
        ctx: click.Context, args: List[str], incomplete: Optional[str]
    ) -> Any:
        use_params: Dict[str, Any] = {}
        if ctx_name:
            use_params[ctx_name] = ctx
