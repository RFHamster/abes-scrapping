...
if apply_over_axes.__doc__ is not None:
    apply_over_axes.__doc__ = (
        np.apply_over_axes.__doc__[
            : np.apply_over_axes.__doc__.find('Notes')
        ].rstrip()
        + """
...
def average(
    ...
    if weights is None:
        ...
        if a.shape != wgt.shape:
            ...
            wgt = np.broadcast_to(
                wgt, (a.ndim - 1) * (1,) + wgt.shape, subok=True
            )
...
def unique(ar1, return_index=False, return_inverse=False):
    ...
    """
    output = np.unique(
        ar1, return_index=return_index, return_inverse=return_inverse
    )
...
def _ezclump(mask):
    ...
    if mask[0]:
        ...
        r = [slice(0, idx[0])]
        r.extend(
            (slice(left, right) for left, right in zip(idx[1:-1:2], idx[2::2]))
        )
    else:
        if len(idx) == 0:
            return []
