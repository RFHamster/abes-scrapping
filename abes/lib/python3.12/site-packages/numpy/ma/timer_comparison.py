...
class ModuleTester:
    ...
    def test_1(self):
        ...
        xf = np.where(m1, 1.0e20, x)
        xm.set_fill_value(1.0e20)
    ...
    def test_4(self):
        ...
        assert self.allequal(
            np.transpose(y, (2, 0, 1)), self.transpose(x, (2, 0, 1))
        )
        assert self.allequal(
            np.take(y, (2, 0, 1), 1), self.take(x, (2, 0, 1), 1)
        )
        assert self.allequal(
    ...
    def test_7(self):
        """Tests ufunc"""
        d = (
            self.array([1.0, 0, -1, pi / 2] * 2, mask=[0, 1] + [0] * 6),
    ...
    def test_99(self):
        ...
        self.assert_array_equal(
            2.0, self.average(ott, weights=[1.0, 1.0, 2.0, 1.0])
        )
        result, wts = self.average(
            ott, weights=[1.0, 1.0, 2.0, 1.0], returned=1
        )
        ...
        self.assert_array_equal(
            self.average(y, axis=0), np.arange(6) * 3.0 / 2.0
        )
        self.assert_array_equal(
            self.average(y, axis=1),
            [self.average(x, axis=0), self.average(x, axis=0) * 2.0],
        )
        self.assert_array_equal(self.average(y, None, weights=w2), 20.0 / 6.0)
        self.assert_array_equal(
            self.average(y, axis=0, weights=w2),
        ...
        m5 = [0, 1, 1, 1, 1, 1]
        self.assert_array_equal(
            self.average(self.masked_array(x, m1), axis=0), 2.5
        )
        self.assert_array_equal(
            self.average(self.masked_array(x, m2), axis=0), 2.5
        )
        self.assert_array_equal(
            self.average(self.masked_array(x, m5), axis=0), 0.0
        )
        self.assert_array_equal(
        ...
        self.assert_array_equal(
            self.average(z, axis=0, weights=w2),
            [0.0, 1.0, 99.0, 99.0, 4.0, 10.0],
        )
