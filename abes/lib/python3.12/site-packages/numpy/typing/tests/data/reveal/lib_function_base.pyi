...
assert_type(
    np.add_newdoc('__main__', 'blabla', doc=('meth', 'test doc')), None
)
assert_type(
    np.add_newdoc('__main__', 'blabla', doc=[('meth', 'test doc')]), None
)
...
assert_type(
    np.average(AR_f8, returned=True), tuple[np.floating[Any], np.floating[Any]]
)
assert_type(
...
assert_type(
    np.asarray_chkfinite(AR_f8, dtype=np.float64), npt.NDArray[np.float64]
)
...
assert_type(
    np.copy(CHAR_AR_U, 'K', subok=True), np.chararray[Any, np.dtype[np.str_]]
)
assert_type(
    np.copy(CHAR_AR_U, subok=True), np.chararray[Any, np.dtype[np.str_]]
)
...
assert_type(
    np.cov(AR_f8, AR_c16, ddof=1), npt.NDArray[np.complexfloating[Any, Any]]
)
assert_type(
    np.cov(AR_f8, aweights=AR_f8, dtype=np.float32), npt.NDArray[np.float32]
)
...
assert_type(
    np.corrcoef(AR_f8, AR_c16), npt.NDArray[np.complexfloating[Any, Any]]
)
...
assert_type(
    np.median(AR_c16, overwrite_input=True), np.complexfloating[Any, Any]
)
...
assert_type(
    np.percentile(AR_c16, [50]), npt.NDArray[np.complexfloating[Any, Any]]
)
assert_type(np.percentile(AR_m, [50]), npt.NDArray[np.timedelta64])
assert_type(
    np.percentile(AR_M, [50], method='nearest'), npt.NDArray[np.datetime64]
)
...
assert_type(
    np.quantile(AR_c16, [0.5]), npt.NDArray[np.complexfloating[Any, Any]]
)
assert_type(np.quantile(AR_m, [0.5]), npt.NDArray[np.timedelta64])
assert_type(
    np.quantile(AR_M, [0.5], method='nearest'), npt.NDArray[np.datetime64]
)
...
assert_type(
    np.meshgrid(AR_f8, AR_i8, AR_c16, indexing='ij'), list[npt.NDArray[Any]]
)
...
assert_type(
    np.insert(AR_LIKE_f8, [0, 4, 9], [0.5, 9.2, 7], axis=0), npt.NDArray[Any]
)
