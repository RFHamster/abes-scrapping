...
assert_type(
    AR_S.center([2, 3, 4], b'a'), np.chararray[Any, np.dtype[np.bytes_]]
)
...
assert_type(
    ...
    np.chararray[Any, np.dtype[np.bytes_]],
)
...
assert_type(
    AR_S.partition([b'a', b'b', b'c']), np.chararray[Any, np.dtype[np.bytes_]]
)
assert_type(AR_U.rpartition('\n'), np.chararray[Any, np.dtype[np.str_]])
assert_type(
    AR_S.rpartition([b'a', b'b', b'c']), np.chararray[Any, np.dtype[np.bytes_]]
)

assert_type(AR_U.replace('_', '-'), np.chararray[Any, np.dtype[np.str_]])
assert_type(
    AR_S.replace([b'_', b''], [b'a', b'b']),
    np.chararray[Any, np.dtype[np.bytes_]],
)
...
assert_type(
    AR_S.splitlines(keepends=[True, True, False]), npt.NDArray[np.object_]
)
