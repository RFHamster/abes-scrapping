...
assert_type(
    np.ndenumerate(AR_LIKE_f).iter, np.flatiter[npt.NDArray[np.float64]]
)
...
assert_type(
    next(np.ndenumerate(AR_LIKE_f)), tuple[tuple[int, ...], np.float64]
)
...
assert_type(
    np.unravel_index([22, 41, 37], (7, 6)), tuple[npt.NDArray[np.intp], ...]
)
assert_type(
...
assert_type(
    np.ravel_multi_index(AR_LIKE_i, (4, 4), mode=('clip', 'wrap')), np.intp
)
...
assert_type(
    ...
    tuple[Literal[0], slice, ellipsis, list[int]],
)
