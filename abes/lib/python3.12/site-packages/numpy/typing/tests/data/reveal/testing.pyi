...
assert_type(
    np.testing.KnownFailureException(), np.testing.KnownFailureException
)
...
assert_type(
    np.testing.build_err_msg(
        np.arange(9).reshape(3, 3), 'test', verbose=False
    ),
    str,
)
...
assert_type(
    np.testing.assert_equal([1, 2, 3], [1, 2, 3], err_msg='fail'), None
)
assert_type(np.testing.assert_equal(1, 1.0, verbose=True), None)

assert_type(
    np.testing.print_assert_equal('Test XYZ of func xyz', [0, 1], [0, 1]), None
)

assert_type(np.testing.assert_almost_equal(1.0, 1.1), None)
assert_type(
    np.testing.assert_almost_equal([1, 2, 3], [1, 2, 3], err_msg='fail'), None
)
...
assert_type(
    np.testing.assert_array_compare(func2, AR_i8, AR_f8, err_msg='test'), None
)
assert_type(
    np.testing.assert_array_compare(func2, AR_i8, AR_f8, verbose=True), None
)
assert_type(
    np.testing.assert_array_compare(func2, AR_i8, AR_f8, header='header'), None
)
assert_type(
...
assert_type(
    np.testing.assert_array_compare(func2, AR_i8, AR_f8, equal_nan=False), None
)
assert_type(
    np.testing.assert_array_compare(func2, AR_i8, AR_f8, equal_inf=True), None
)
...
assert_type(
    np.testing.assert_array_almost_equal(AR_i8, AR_f8, err_msg='test'), None
)
assert_type(
    np.testing.assert_array_almost_equal(AR_i8, AR_f8, verbose=True), None
)
assert_type(
    np.testing.assert_array_almost_equal(AR_i8, AR_f8, decimal=1), None
)
...
assert_type(
    np.testing.assert_raises_regex(RuntimeWarning, b'test', func3, 5), None
)
assert_type(
    np.testing.assert_raises_regex(
        RuntimeWarning, re.compile(b'test'), func3, 5
    ),
    None,
)
...
assert_type(
    np.testing.decorate_methods(Test, decorate, re.compile('test')), None
)

assert_type(np.testing.measure('for i in range(1000): np.sqrt(i**2)'), float)
assert_type(
    np.testing.measure(b'for i in range(1000): np.sqrt(i**2)', times=5), float
)
...
assert_type(
    np.testing.assert_array_almost_equal_nulp(AR_i8, AR_f8, nulp=2), None
)

assert_type(
    np.testing.assert_array_max_ulp(AR_i8, AR_f8, maxulp=2), npt.NDArray[Any]
)
assert_type(
...
assert_type(
    np.testing.assert_no_warnings(), contextlib._GeneratorContextManager[None]
)
...
assert_type(
    np.testing.tempdir('test_dir'), contextlib._GeneratorContextManager[str]
)
assert_type(
    np.testing.tempdir(prefix=b'test'),
    contextlib._GeneratorContextManager[bytes],
)
assert_type(
...
assert_type(
    np.testing.assert_no_gc_cycles(), contextlib._GeneratorContextManager[None]
)
