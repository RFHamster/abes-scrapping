...
assert_type(
    np.busday_offset('2011-01', '2011-02', roll='forward'), np.datetime64
)
assert_type(
    np.busday_offset(['2011-01'], '2011-02', roll='forward'),
...
assert_type(
    np.compare_chararrays('a', 'b', '!=', rstrip=False), npt.NDArray[np.bool_]
)
assert_type(
    np.compare_chararrays(b'a', b'a', '==', True), npt.NDArray[np.bool_]
)
...
assert_type(
    np.nested_iters(
        [AR_i8, AR_i8], [[0], [1]], op_flags=[['readonly', 'readonly']]
    ),
    tuple[np.nditer, ...],
)
assert_type(
