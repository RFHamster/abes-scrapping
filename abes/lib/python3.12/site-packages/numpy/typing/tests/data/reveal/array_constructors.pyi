...
assert_type(
    np.fromstring('1 1 1', dtype=np.int64, sep=' '), npt.NDArray[np.int64]
)
assert_type(
    np.fromstring(b'1 1 1', dtype=np.int64, sep=' '), npt.NDArray[np.int64]
)
...
assert_type(
    np.fromfile('test.txt', dtype=np.int64, sep=' '), npt.NDArray[np.int64]
)
...
with open('test.txt') as f:
    ...
    assert_type(
        np.fromfile(Path('test.txt'), sep=' '), npt.NDArray[np.float64]
    )
...
assert_type(
    ...
    tuple[npt.NDArray[np.floating[Any]], np.floating[Any]],
)
assert_type(
    np.linspace(0j, 10, retstep=True),
    tuple[
        npt.NDArray[np.complexfloating[Any, Any]], np.complexfloating[Any, Any]
    ],
)
assert_type(
    np.linspace(0, 10, retstep=True, dtype=np.int64),
    tuple[npt.NDArray[np.int64], np.int64],
)
assert_type(
    np.linspace(0j, 10, retstep=True, dtype=int), tuple[npt.NDArray[Any], Any]
)
...
assert_type(
    np.indices([1, 2, 3], sparse=True), tuple[npt.NDArray[np.int_], ...]
)
