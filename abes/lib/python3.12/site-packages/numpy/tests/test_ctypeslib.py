...
try:
    ...
    if cdll is None:
        cdll = load_library(
            '_multiarray_umath', np.core._multiarray_umath.__file__
        )
...
@pytest.mark.skipif(
    ctypes is None, reason='ctypes not available in this python'
)
...
class TestNdpointer:
    def test_dtype(self):
        ...
        assert_raises(
            TypeError, p.from_param, np.array([1], dt.newbyteorder('swap'))
        )
        dtnames = ['x', 'y']
...
class TestNdpointerCFunc:
    ...
    def test_return(self, dt):
        ...
        assert_equal(
            arr2.__array_interface__['data'], arr.__array_interface__['data']
        )
