...
class TestEnvPrivation:
    ...
    UNAVAILABLE_FEAT = (
        None if len(unavailable_feats) == 0 else unavailable_feats[0]
    )
...
@pytest.mark.skipif(
    not is_linux or not is_power, reason='Only for Linux and Power'
)
...
@pytest.mark.skipif(
    not is_linux or not is_zarch, reason='Only for Linux and IBM Z'
)
...
@pytest.mark.skipif(
    not is_linux or not is_arm, reason='Only for Linux and ARM'
)
