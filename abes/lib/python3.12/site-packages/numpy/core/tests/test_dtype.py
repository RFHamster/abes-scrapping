...
class TestBuiltin:
    @pytest.mark.parametrize(
        't', [int, float, complex, np.int32, str, object, np.compat.unicode]
    )
    def test_run(self, t):
    ...
    def test_field_order_equality(self):
        x = np.dtype(
            {'names': ['A', 'B'], 'formats': ['i4', 'f4'], 'offsets': [0, 4]}
        )
        y = np.dtype(
            {'names': ['B', 'A'], 'formats': ['i4', 'f4'], 'offsets': [4, 0]}
        )
...
class TestRecord:
    ...
    def test_not_lists(self):
        ...
        """
        assert_raises(
            TypeError, np.dtype, dict(names={'A', 'B'}, formats=['f8', 'i4'])
        )
        assert_raises(
            TypeError, np.dtype, dict(names=['A', 'B'], formats={'f8', 'i4'})
        )
    ...
    def test_nonint_offsets(self):
        ...
        def make_dtype(off):
            return np.dtype(
                {'names': ['A'], 'formats': ['i4'], 'offsets': [off]}
            )
    ...
    def test_multifield_index(self, align_flag):
        ...
        assert_equal(
            dt_sub,
            np.dtype(
                {'names': [], 'formats': [], 'offsets': [], 'itemsize': 96}
            ),
        )
        assert_equal(dt_sub.isalignedstruct, align_flag)
    ...
    def test_fieldless_views(self):
        ...
        assert_equal(
            np.frombuffer(b'', dtype=[], count=2), np.empty(2, dtype=[])
        )
        ...
        assert_equal(
            np.zeros(2, dtype=[]) == np.zeros(2, dtype=[]),
            np.ones(2, dtype=bool),
        )

        assert_equal(
            np.zeros((1, 2), dtype=[]) == a, np.ones((1, 2), dtype=bool)
        )
...
class TestSubarray:
    ...
    def test_shape_equal(self):
        ...
        assert_dtype_equal(
            np.dtype(('<f4', (3, 2))), np.dtype(('<f4', (3, 2)))
        )
    ...
    def test_shape_simple(self):
        ...
        assert_dtype_not_equal(
            np.dtype(('f8', (1,))), np.dtype(('f8', (1, 1)))
        )
        assert_dtype_not_equal(
            np.dtype(('f4', (3, 2))), np.dtype(('f4', (2, 3)))
        )
    ...
    def test_shape_monster(self):
        ...
        assert_dtype_not_equal(
            np.dtype(
                (
                    [('a', [('a', 'i4', 6)], (2, 1)), ('b', 'f8', (1, 3))],
                    (2, 2),
                )
            ),
            np.dtype(
                (
                    [('a', [('a', 'u4', 6)], (2, 1)), ('b', 'f8', (1, 3))],
                    (2, 2),
                )
            ),
        )
...
class TestMonsterType:
    ...
    def test1(self):
        ...
        a = np.dtype(
            [('yo', int), ('ye', simple1), ('yi', np.dtype((int, (3, 2))))]
        )
        b = np.dtype(
            [('yo', int), ('ye', simple1), ('yi', np.dtype((int, (3, 2))))]
        )
        ...
        c = np.dtype(
            [('yo', int), ('ye', simple1), ('yi', np.dtype((a, (3, 2))))]
        )
        d = np.dtype(
            [('yo', int), ('ye', simple1), ('yi', np.dtype((a, (3, 2))))]
        )
...
class TestPromotion:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                np.longdouble(np.nextafter(1.7e308, 0.0)),
                np.complex128,
                np.clongdouble,
            ),
            ...
            # Base of the double value to sidestep any rounding issues:
            (
                np.clongdouble(np.nextafter(1.7e308, 0.0) * 1j),
                np.complex128,
                np.clongdouble,
            ),
            # Additionally use "nextafter" so the cast can't round down:
    ...
    @pytest.mark.parametrize(
        ['other', 'expected'], [(1, rational), (1.0, np.float64)]
    )
    @np._no_nep50_warning()
    def test_float_int_pyscalar_promote_rational(
        ...
        if not weak_promotion and type(other) == float:
            ...
            # pass.
            with pytest.raises(
                TypeError, match=r'.* do not have a common DType'
            ):
...
class TestDTypeClasses:
    ...
    @pytest.mark.parametrize(
        'name', ['Half', 'Float', 'Double', 'CFloat', 'CDouble']
    )
