...
class TestRegression:
    ...
    def test_object_array_shape(self):
        ...
        assert_equal(
            np.array([[3, 4], [5, 6], None], dtype=object).shape, (3,)
        )
    ...
    def test_ndmin_order(self):
        ...
        assert_(
            np.array(np.ones((2, 2), order='F'), ndmin=3).flags.f_contiguous
        )
        assert_(
            np.array(np.ones((2, 2), order='C'), ndmin=3).flags.c_contiguous
        )
    ...
    def test_noncommutative_reduce_accumulate(self):
        ...
        assert_array_equal(
            np.divide.accumulate(todivide), np.array([2.0, 4.0, 16.0])
        )
    ...
    def test_multidim_byteswap(self):
        ...
        assert_array_equal(
            r.byteswap(), np.array([(256, (0, 256, 512))], r.dtype)
        )
    ...
    def test_frompyfunc_endian(self):
        ...
        assert_almost_equal(
            uradians(big_endian).astype(float),
    ...
    def test_void_scalar_constructor(self):
        ...
        for proto in range(2, pickle.HIGHEST_PROTOCOL + 1):
            assert_(
                pickle.loads(pickle.dumps(test_string, protocol=proto))
                == test_string
            )
            assert_(
                pickle.loads(pickle.dumps(test_record, protocol=proto))
                == test_record
            )
    ...
    def test_byteswap_complex_scalar(self):
        ...
        for dtype in [np.dtype('<' + t) for t in np.typecodes['Complex']]:
            ...
            if x.dtype.byteorder == z.dtype.byteorder:
                ...
                assert_equal(
                    x, np.frombuffer(y.tobytes(), dtype=dtype.newbyteorder())
                )
            else:
                # big-endian machine
    ...
    def test_complex64_alignment(self):
        ...
        data_back = np.ndarray(
            arr2.shape, arr2.dtype, buffer=data_str, order='F'
        )
    ...
    def test_leak_in_structured_dtype_comparison(self):
        # gh-6250
        recordtype = np.dtype(
            [('a', np.float64), ('b', np.int32), ('d', (str, 5))]
        )
    ...
    def test_invalid_structured_dtypes(self):
        ...
        assert_raises(
            ValueError, np.dtype, ('i8', [('name', [('name', 'O')])])
        )
        ...
        assert_raises(
            ValueError, np.dtype, ('i', {'name': ('i', 0, 'title', 'oops')})
        )
        assert_raises(
    ...
    @pytest.mark.skipif(
        sys.maxsize < 2**31 + 1, reason='overflows 32-bit python'
    )
