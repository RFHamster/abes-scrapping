...
def _check_assignment(srcidx, dstidx):
    ...
    assert_(
        np.all(arr == cpy), 'assigning arr[%s] = arr[%s]' % (dstidx, srcidx)
    )
...
def test_diophantine_fuzz():
    ...
    for ndim in range(10):
        ...
        while min(feasible_count, infeasible_count) < min_count:
            ...
            A = tuple(
                int(rng.randint(1, A_max + 1, dtype=np.intp))
                for j in range(ndim)
            )
            U = tuple(
                int(rng.randint(0, U_max + 2, dtype=np.intp))
                for j in range(ndim)
            )
...
def test_may_share_memory_manual():
    ...
    for x in xs:
        ...
        check_may_share_memory_exact(
            x.ravel()[6:6], xp.reshape(13, 21, 23, 11)[:, ::7]
        )

        # Test itemsize is dealt with
        check_may_share_memory_exact(x[:, ::7], xp.reshape(13, 21, 23, 11))
        check_may_share_memory_exact(
            x[:, ::7], xp.reshape(13, 21, 23, 11)[:, 3::3]
        )
        check_may_share_memory_exact(
            x.ravel()[6:7], xp.reshape(13, 21, 23, 11)[:, ::7]
        )
...
def test_internal_overlap_fuzz():
    ...
    while min(overlap, no_overlap) < min_count:
        ...
        strides = tuple(
            rng.randint(-1000, 1000, dtype=np.intp) for j in range(ndim)
        )
...
class TestUFunc:
    ...
    def test_binary_ufunc_reduceat_fuzz(self):
        ...
        self.check_unary_fuzz(
            do_reduceat, get_out_axis_size, dtype=np.int16, count=500
        )
