...
class TestEinsum:
    def test_einsum_errors(self):
        for do_opt in [True, False]:
            ...
            assert_raises(
                TypeError, np.einsum, '', 0, out='test', optimize=do_opt
            )

            # order parameter must be a valid order
            assert_raises(
                ValueError, np.einsum, '', 0, order='W', optimize=do_opt
            )

            # casting parameter must be a valid casting
            assert_raises(
                ValueError, np.einsum, '', 0, casting='blah', optimize=do_opt
            )
            ...
            assert_raises(
                TypeError, np.einsum, '', 0, bad_arg=0, optimize=do_opt
            )
            ...
            assert_raises(
                ValueError, np.einsum, ',', 0, [0], [0], optimize=do_opt
            )
            ...
            assert_raises(ValueError, np.einsum, '...i', 0, optimize=do_opt)
            assert_raises(
                ValueError, np.einsum, 'i...j', [0, 0], optimize=do_opt
            )
            assert_raises(ValueError, np.einsum, 'i...', 0, optimize=do_opt)
            assert_raises(
                ValueError, np.einsum, 'ij...', [0, 0], optimize=do_opt
            )

            # invalid ellipsis
            assert_raises(
                ValueError, np.einsum, 'i..', [0, 0], optimize=do_opt
            )
            assert_raises(
                ValueError, np.einsum, '.i...', [0, 0], optimize=do_opt
            )
            assert_raises(
                ValueError, np.einsum, 'j->..j', [0, 0], optimize=do_opt
            )
            assert_raises(
                ValueError, np.einsum, 'j->.j...', [0, 0], optimize=do_opt
            )
            ...
            assert_raises(
                ValueError, np.einsum, '...j$', [0, 0], optimize=do_opt
            )
            assert_raises(
                ValueError, np.einsum, 'i->&', [0, 0], optimize=do_opt
            )
            ...
            assert_raises(
                ValueError, np.einsum, 'i->ij', [0, 0], optimize=do_opt
            )
    ...
    def test_einsum_object_errors(self):
        ...
        b = np.array(
            [DestructoBox(i, 55) for i in range(1, 4)], dtype='object'
        )
    ...
    def test_einsum_views(self):
        ...
        for do_opt in [True, False]:
            ...
            assert_(b.base is a)
            assert_equal(
                b, [[x[i, i] for i in range(3)] for x in a.transpose(2, 0, 1)]
            )
            ...
            assert_(b.base is a)
            assert_equal(
                b, [[x[i, i] for i in range(3)] for x in a.transpose(1, 0, 2)]
            )
    ...
    def check_einsum_sums(self, dtype, do_opt=False):
        ...
        for n in range(1, 17):
            ...
            assert_equal(np.einsum('...i->...', a, optimize=do_opt), b)
            assert_equal(
                np.einsum(a, [Ellipsis, 0], [Ellipsis], optimize=do_opt), b
            )
        ...
        for n in range(1, 17):
            ...
            assert_equal(np.einsum('i...->...', a, optimize=do_opt), b)
            assert_equal(
                np.einsum(a, [0, Ellipsis], [Ellipsis], optimize=do_opt), b
            )
        ...
        for n in range(1, 17):
            ...
            assert_equal(
                np.einsum('...i, ...i', a, b, optimize=do_opt), np.inner(a, b)
            )
            assert_equal(
        ...
        for n in range(1, 11):
            ...
            b = np.arange(n, dtype=dtype)
            assert_equal(
                np.einsum('i..., i...', a, b, optimize=do_opt),
                np.inner(a.T, b.T).T,
            )
            assert_equal(
        ...
        for n in range(1, 17):
            ...
            assert_equal(
                np.einsum('i,j', a, b, optimize=do_opt), np.outer(a, b)
            )
            assert_equal(
                np.einsum(a, [0], b, [1], optimize=do_opt), np.outer(a, b)
            )
        ...
        with suppress_warnings() as sup:
            ...
            for n in range(1, 17):
                ...
                b = np.arange(n, dtype=dtype)
                assert_equal(
                    np.einsum('ij, j', a, b, optimize=do_opt), np.dot(a, b)
                )
                assert_equal(
                ...
                assert_equal(
                    c, np.dot(a.astype('f8'), b.astype('f8')).astype(dtype)
                )
                c[...] = 0
                np.einsum(
                    a,
            ...
            for n in range(1, 17):
                ...
                b = np.arange(n, dtype=dtype)
                assert_equal(
                    np.einsum('ji,j', a.T, b.T, optimize=do_opt),
                    np.dot(b.T, a.T),
                )
                assert_equal(
                    np.einsum(a.T, [1, 0], b.T, [1], optimize=do_opt),
                    np.dot(b.T, a.T),
                )
            ...
            if dtype != 'f2':
                assert_equal(
                    np.einsum('ij,jk,kl', a, b, c, optimize=do_opt),
                    a.dot(b).dot(c),
                )
                assert_equal(
                    np.einsum(
                        a, [0, 1], b, [1, 2], c, [2, 3], optimize=do_opt
                    ),
                    a.dot(b).dot(c),
                )
        ...
        assert_equal(
            np.einsum(
                a, [0], b, [0], c, [0], [0], dtype='?', casting='unsafe'
            ),
            np.logical_and(np.logical_and(a != 0, b != 0), c != 0),
        )
        ...
        for n in range(1, 25):
            ...
            if np.dtype(dtype).itemsize > 1:
                assert_equal(
                    np.einsum('...,...', a, a, optimize=do_opt),
                    np.multiply(a, a),
                )
                assert_equal(
                    np.einsum('i,i', a, a, optimize=do_opt), np.dot(a, a)
                )
                assert_equal(np.einsum('i,->i', a, 2, optimize=do_opt), 2 * a)
                assert_equal(np.einsum(',i->i', 2, a, optimize=do_opt), 2 * a)
                assert_equal(
                    np.einsum('i,->', a, 2, optimize=do_opt), 2 * np.sum(a)
                )
                assert_equal(
                    np.einsum(',i->', 2, a, optimize=do_opt), 2 * np.sum(a)
                )
                ...
                assert_equal(
                    np.einsum('i,->', a[1:], 2, optimize=do_opt),
                    2 * np.sum(a[1:]),
                )
                assert_equal(
                    np.einsum(',i->', 2, a[1:], optimize=do_opt),
                    2 * np.sum(a[1:]),
                )
        ...
        assert_array_equal(
            np.einsum('ij,ij->j', p, q, optimize=True), [10.0] * 2
        )
    ...
    def test_einsum_sums_int8(self):
        if (
            ...
            pytest.xfail(
                'Fails on macOS x86-64 and when using clang-cl '
    ...
    def test_einsum_misc(self):
        ...
        assert_equal(
            np.einsum('ij...,j...->i...', a, b, optimize=True), [[[2], [2]]]
        )
    ...
    def test_object_loop(self):
        ...
        objNULL = np.ndarray(
            buffer=b'\0' * np.intp(0).itemsize, shape=1, dtype=object
        )
    ...
    def test_einsum_broadcast(self):
        ...
        for opt in [True, False]:
            ...
            assert_equal(
                np.einsum('ik...,...kj->i...j', A, B, optimize=opt), ref
            )
            assert_equal(
