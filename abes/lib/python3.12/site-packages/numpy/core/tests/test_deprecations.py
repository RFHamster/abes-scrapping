...
class BuiltInRoundComplexDType(_DeprecationTestCase):
    ...
    def test_deprecated(self):
        for scalar_type in self.deprecated_types:
            ...
            self.assert_deprecated(
                round, args=(scalar,), kwargs={'ndigits': 0}
            )
...
class TestNonExactMatchDeprecation(_DeprecationTestCase):
    ...
    def test_non_exact_match(self):
        ...
        # misspelt mode check
        self.assert_deprecated(
            lambda: np.ravel_multi_index(arr, (7, 6), mode='Cilp')
        )
        # using completely different word with first character as R
        self.assert_deprecated(
            lambda: np.searchsorted(arr[0], 4, side='Random')
        )
...
class TestMatrixInOuter(_DeprecationTestCase):
    ...
    message = (
        r'add.outer\(\) was passed a numpy matrix as '
...
class TestQuantileInterpolationDeprecation(_DeprecationTestCase):
    ...
    def test_deprecated(self, func):
        self.assert_deprecated(
            lambda: func([0.0, 1.0], 0.0, interpolation='linear')
        )
        self.assert_deprecated(
            lambda: func([0.0, 1.0], 0.0, interpolation='nearest')
        )
    ...
    def test_both_passed(self, func):
        with warnings.catch_warnings():
            ...
            with pytest.raises(TypeError):
                func(
                    [0.0, 1.0], 0.0, interpolation='nearest', method='nearest'
                )
...
class TestMemEventHook(_DeprecationTestCase):
    ...
    def test_mem_seteventhook(self):
        ...
        with pytest.warns(
            DeprecationWarning, match='PyDataMem_SetEventHook is deprecated'
        ):
...
@pytest.mark.parametrize(
    'name', ['bool', 'long', 'ulong', 'str', 'bytes', 'object']
)
...
class TestRemovedGlobals:
    ...
    # (or faster).
    @pytest.mark.parametrize(
        'name', ['object', 'bool', 'float', 'complex', 'str', 'int']
    )
