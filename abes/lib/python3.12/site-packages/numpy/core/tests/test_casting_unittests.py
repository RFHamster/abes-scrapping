...
class TestCasting:
    ...
    def get_data_variation(self, arr1, arr2, aligned=True, contig=True):
        ...
        if aligned:
            new1 = as_strided(
                from_bytes[:-1].view(arr1.dtype), arr1.shape, (stride1,)
            )
            new2 = as_strided(
                to_bytes[:-1].view(arr2.dtype), arr2.shape, (stride2,)
            )
        else:
            new1 = as_strided(
                from_bytes[1:].view(arr1.dtype), arr1.shape, (stride1,)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                str(np.dtype('M8[ns]').newbyteorder()),
                None,
                Casting.equiv,
                None,
                1,
    ...
    def test_time_to_time(
        self, from_dt, to_dt, expected_casting, expected_view_off, nom, denom
    ):
        ...
        for aligned in [True, True]:
            for contig in [True, True]:
                ...
                assert_array_equal(
                    out.view('int64'), expected_out.view('int64')
                )
    ...
    def test_structured_field_offsets(self, to_dt, expected_off):
        ...
        from_dt = np.dtype(
            {'names': ['a', 'b'], 'formats': ['i4', 'f4'], 'offsets': [2, 6]}
        )
        cast = get_castingimpl(type(from_dt), type(to_dt))
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                'i',
                dict(names=['a', 'b'], formats=['i', 'i'], offsets=[2, 2]),
                None,
            ),
