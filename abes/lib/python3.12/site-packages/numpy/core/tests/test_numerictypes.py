...
def normalize_descr(descr):
    ...
    for item in descr:
        ...
        if isinstance(dtype, str):
            ...
            raise ValueError(
                'Expected a str or list and got %s' % (type(item))
            )
    return out
...
class ReadValuesNested:
    ...
    def test_nested1_acessors(self):
        ...
        if not self.multiple_rows:
            assert_equal(
                h['Info']['value'], np.array(self._buffer[1][0], dtype='c16')
            )
            assert_equal(
                h['Info']['y2'], np.array(self._buffer[1][1], dtype='f8')
            )
            assert_equal(
                h['info']['Name'], np.array(self._buffer[3][0], dtype='U2')
            )
            assert_equal(
                h['info']['Value'], np.array(self._buffer[3][1], dtype='c16')
            )
        else:
            assert_equal(
                h['Info']['value'],
                np.array(
                    [self._buffer[0][1][0], self._buffer[1][1][0]], dtype='c16'
                ),
            )
            assert_equal(
                h['Info']['y2'],
                np.array(
                    [self._buffer[0][1][1], self._buffer[1][1][1]], dtype='f8'
                ),
            )
            assert_equal(
                h['info']['Name'],
                np.array(
                    [self._buffer[0][3][0], self._buffer[1][3][0]], dtype='U2'
                ),
            )
            assert_equal(
                h['info']['Value'],
                np.array(
                    [self._buffer[0][3][1], self._buffer[1][3][1]], dtype='c16'
                ),
            )
    ...
    def test_nested2_acessors(self):
        ...
        if not self.multiple_rows:
            ...
            assert_equal(
                h['Info']['Info2']['value'],
                np.array(
                    [self._buffer[0][1][2][1], self._buffer[1][1][2][1]],
                    dtype='c16',
                ),
            )
            assert_equal(
                h['Info']['Info2']['z3'],
                np.array(
                    [self._buffer[0][1][2][3], self._buffer[1][1][2][3]],
                    dtype='u4',
                ),
            )
...
class TestMaximumSctype:
    ...
    @pytest.mark.parametrize(
        't', [np.byte, np.short, np.intc, np.int_, np.longlong]
    )
    def test_int(self, t):
        assert_equal(np.maximum_sctype(t), np.sctypes['int'][-1])

    @pytest.mark.parametrize(
        't', [np.ubyte, np.ushort, np.uintc, np.uint, np.ulonglong]
    )
    ...
    @pytest.mark.parametrize(
        't', [np.bool_, np.object_, np.str_, np.bytes_, np.void]
    )
...
@pytest.mark.xfail(
    IS_PYPY, reason='PyPy cannot modify tp_doc after PyType_Ready'
)
