...
def test_fromstring():
    ...
    a = np.array([o] * 5)
    assert_equal(
        np.fromstring(s, sep=' ', dtype=np.longdouble),
        a,
        err_msg="reading '%s'" % s,
    )
...
def test_fromstring_complex():
    for ctype in ['complex', 'cdouble', 'cfloat']:
        ...
        with assert_warns(DeprecationWarning):
            assert_equal(
                np.fromstring('1+j', dtype=ctype, sep=','), np.array([1.0])
            )
        with assert_warns(DeprecationWarning):
            assert_equal(
                np.fromstring('1+', dtype=ctype, sep=','), np.array([1.0])
            )
        with assert_warns(DeprecationWarning):
            assert_equal(
                np.fromstring('1j+1', dtype=ctype, sep=','), np.array([1j])
            )
...
class TestFileBased:
    ...
    @pytest.mark.skipif(
        string_to_longdouble_inaccurate, reason='Need strtold_l'
    )
...
class TestCommaDecimalPointLocale(CommaDecimalPointLocale):
    ...
    def test_fromstring_best_effort_float(self):
        with assert_warns(DeprecationWarning):
            assert_equal(
                np.fromstring('1,234', dtype=float, sep=' '), np.array([1.0])
            )
...
def test_longdouble_from_int(int_val):
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        if (
            np.allclose(np.finfo(np.longdouble).max, np.finfo(np.double).max)
            and w
        ):
...
def test_musllinux_x86_64_signature():
    ...
    sig = (
        (np.longdouble(-1.0) / np.longdouble(10.0))
        .newbyteorder('<')
        .tobytes()[:10]
    )
