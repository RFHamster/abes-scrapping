...
class TestAtleast2d:
    ...
    def test_r2array(self):
        ...
        assert_(
            atleast_2d([[[3, 1], [4, 5]], [[3, 5], [1, 2]]]).shape == (2, 2, 2)
        )
...
class TestConcatenate:
    ...
    def test_exceptions(self):
        ...
        for ndim in [1, 2, 3]:
            ...
            assert_raises(
                np.AxisError, np.concatenate, (a, a), axis=-(ndim + 1)
            )
        ...
        for i in range(3):
            ...
            assert_raises_regex(
                ...
                'index 0 has size 1 and the array at index 1 has size 2'.format(
                    i
                ),
                np.concatenate,
                (a, b),
    ...
    def test_out_and_dtype(self, axis, out_dtype, casting):
        ...
        if not np.can_cast(to_concat[0], out_dtype, casting=casting):
            ...
            res_out = concatenate(
                to_concat, out=out, axis=axis, casting=casting
            )
    ...
    def test_dtype_with_promotion(self, arrs, string_dt, axis):
        ...
        res = np.concatenate(
            arrs, axis=axis, dtype=string_dt, casting='unsafe'
        )
...
def test_stack_out_and_dtype(axis, out_dtype, casting):
    ...
    if not np.can_cast(to_concat[0], out_dtype, casting=casting):
        ...
        res_dtype = stack(
            to_concat, dtype=out_dtype, axis=axis, casting=casting
        )
        assert res_out is out
        assert_array_equal(out, res_dtype)
        assert res_dtype.dtype == out_dtype
...
class TestBlock:
    ...
    def test_nested(self, block):
        ...
        result = block(
            [[block([[one], [three], [four]]), two], [five, six], [zero]]
        )
        expected = np.array(
            [
                [1, 1, 1, 2, 2, 2],
