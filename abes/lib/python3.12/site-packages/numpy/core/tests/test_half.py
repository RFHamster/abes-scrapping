...
class TestHalf:
    ...
    def test_half_conversions(self):
        ...
        b_nn = b == b
        assert_equal(
            self.all_f16[b_nn].view(dtype=uint16), b[b_nn].view(dtype=uint16)
        )
    ...
    def test_half_funcs(self):
        ...
        # fill
        assert_equal(
            np.arange(10, dtype=float16), np.arange(10, dtype=float32)
        )
        ...
        a = np.array(
            [0, 0, -1, -1 / 1e20, 0, 2.0**-24, 7.629e-6], dtype=float16
        )
        ...
        # argmax
        a = np.array(
            [0, -np.inf, -2, 0.5, 12.55, 7.3, 2.1, 12.4], dtype=float16
        )
        assert_equal(a.argmax(), 4)
        a = np.array(
            [0, -np.inf, -2, np.inf, 12.55, np.nan, 2.1, 12.4], dtype=float16
        )
    ...
    def test_half_ufuncs(self):
        ...
        assert_equal(
            np.reciprocal(b), [-0.5, 0.199951171875, 1, 0.25, 0.333251953125]
        )
    ...
    def test_half_fpe(self):
        with np.errstate(all='raise'):
            ...
            assert_raises_fpe(
                'invalid', np.divide, float16(np.inf), float16(np.inf)
            )
