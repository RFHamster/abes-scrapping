...
class _SIMD_FP(_Test_Utility):
    ...
    @pytest.mark.parametrize(
        'intrin', ['max', 'maxp', 'maxn', 'min', 'minp', 'minn']
    )
...
class _SIMD_ALL(_Test_Utility):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                'self.load2_tillz, self.load2_till',
                (32, 64),
                2,
                [0xFFFF, 0x7FFF],
            ),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                'self.loadn2_tillz, self.loadn2_till',
                (32, 64),
                2,
                [0xFFFF, 0x7FFF],
            ),
    ...
    def test_memory_noncont_partial_load(self, intrin, elsizes, scale, fill):
        ...
        for stride in range(-64, 64):
            ...
            for n in lanes:
                ...
                data_stride_till = data_stride[:nscale] + fill * (
                    llanes // scale
                )
    ...
    @pytest.mark.parametrize(
        'intrin, table_size, elsize',
        [('self.lut32', 32, 32), ('self.lut16', 16, 64)],
    )
    ...
    def test_misc(self):
        ...
        select_a = self.select(
            self.cmpeq(self.zero(), self.zero()), vdata_a, vdata_b
        )
        assert select_a == data_a
        select_b = self.select(
            self.cmpneq(self.zero(), self.zero()), vdata_a, vdata_b
        )
...
for target_name, npyv in targets.items():
    ...
    for sfxes, cls in tests_registry.items():
        for sfx in sfxes:
            ...
            inhr = (cls,)
            attr = dict(
                npyv=targets[target_name], sfx=sfx, target_name=target_name
            )
            tcls = type(
