...
class TestMemmap:
    ...
    def test_roundtrip(self):
        ...
        newfp = memmap(
            self.tmpfp, dtype=self.dtype, mode='r', shape=self.shape
        )
    ...
    def test_path(self, tmp_path):
        tmpname = tmp_path / 'mmap'
        fp = memmap(
            Path(tmpname), dtype=self.dtype, mode='w+', shape=self.shape
        )
    ...
    def test_del(self):
        ...
        fp_base = memmap(
            self.tmpfp, dtype=self.dtype, mode='w+', shape=self.shape
        )
        fp_base[0] = 5
        fp_view = fp_base[0:1]
