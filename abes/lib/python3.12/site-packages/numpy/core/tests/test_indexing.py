...
class TestIndexing:
    ...
    def test_small_regressions(self):
        ...
        assert_raises(
            IndexError, a.__setitem__, np.array([1], dtype=np.intp), 1
        )
        assert_raises(
            IndexError, a.__setitem__, np.array([1], dtype=np.uint8), 1
        )
...
class TestMultiIndexingAutomated:
    ...
    def _get_multi_index(self, arr, indices):
        ...
        if ellipsis_pos is not None:
            in_indices[ellipsis_pos : ellipsis_pos + 1] = [
                slice(None, None)
            ] * (arr.ndim - ndim)
