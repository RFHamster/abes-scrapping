...
def on_powerpc():
    ...
    return platform.processor() == 'powerpc' or platform.machine().startswith(
        'ppc'
    )
...
class TestComparisons:
    ...
    def test_comparison_functions(self, dtype, py_comp, np_comp):
        ...
        assert_(
            comp_b.tolist() == comp_b_list,
            f'Failed comparison ({py_comp.__name__})',
        )
        assert_(
            comp_s1.tolist() == comp_s1_list,
            f'Failed comparison ({py_comp.__name__})',
        )
        assert_(
            comp_s2.tolist() == comp_s2_list,
            f'Failed comparison ({py_comp.__name__})',
        )
...
class TestDivision:
    def test_division_int(self):
        ...
        if 5 / 10 == 0.5:
            assert_equal(
                x / 100, [0.05, 0.1, 0.9, 1, -0.05, -0.1, -0.9, -1, -1.2]
            )
        else:
    ...
    def test_division_int_boundary(self, dtype, ex_val):
        ...
        c_div = lambda n, d: (
            0
            if d == 0
            else (fo.min if (n and n == fo.min and d == -1) else n // d)
        )
    ...
    def test_division_complex(self):
        ...
        x = np.array(
            [1.0 + 1.0 * 1j, 1.0 + 0.5 * 1j, 1.0 + 2.0 * 1j],
            dtype=np.complex128,
        )
    ...
    def test_floor_division_complex(self):
        ...
        x = np.array(
            [0.9 + 1j, -0.1 + 1j, 0.9 + 0.5 * 1j, 0.9 + 2.0 * 1j],
            dtype=np.complex128,
        )
    ...
    @pytest.mark.skipif(
        hasattr(np.__config__, 'blas_ssl2_info'), reason='gh-22982'
    )
    @pytest.mark.skipif(IS_WASM, reason="fp errors don't work in wasm")
...
class TestRemainder:
    ...
    def test_float_divmod_corner_cases(self):
        ...
        for dt in np.typecodes['Float']:
            ...
            with suppress_warnings() as sup:
                sup.filter(
                    RuntimeWarning, 'invalid value encountered in divmod'
                )
                sup.filter(
    ...
    def test_overflows(self, dividend_dtype, divisor_dtype, operation):
        ...
        if np.dtype(dividend_dtype).itemsize >= np.dtype(
            ...
            for a in arrays:
                ...
                with pytest.warns(
                    RuntimeWarning, match='overflow encountered in'
                ):
            ...
            for a in arrays:
                ...
                result = np.array(operation(a, divisor)).flatten('f')
                expected_array = np.array(
                    [self.overflow_results[operation].casted(dividend_dtype)]
                    * len(a)
                ).flatten()
                assert_array_equal(result, expected_array)
...
class TestPower:
    ...
    def test_power_complex(self):
        ...
        assert_almost_equal(
            x**3, [(1 + 2j) ** 3, (2 + 3j) ** 3, (3 + 4j) ** 3]
        )
        assert_almost_equal(
            x**4, [(1 + 2j) ** 4, (2 + 3j) ** 4, (3 + 4j) ** 4]
        )
        assert_almost_equal(
        ...
        assert_almost_equal(
            x ** (-2),
            [1 / (1 + 2j) ** 2, 1 / (2 + 3j) ** 2, 1 / (3 + 4j) ** 2],
        )
        assert_almost_equal(
            x ** (-3),
            [(-11 + 2j) / 125, (-46 - 9j) / 2197, (-117 - 44j) / 15625],
        )
        assert_almost_equal(
...
class TestLog:
    ...
    def test_log_strides(self):
        ...
        for ii in sizes:
            x_f64 = np.float64(
                np.random.uniform(low=0.01, high=100.0, size=ii)
            )
...
class TestExp:
    ...
    def test_exp_strides(self):
        ...
        for ii in sizes:
            x_f64 = np.float64(
                np.random.uniform(low=0.01, high=709.1, size=ii)
            )
            y_true = np.exp(x_f64)
            for jj in strides:
                assert_array_almost_equal_nulp(
...
class TestSpecialFloats:
    ...
    def test_log_values(self):
        ...
        with np.errstate(divide='raise'):
            for dt in ['e', 'f', 'd']:
                assert_raises(
                    FloatingPointError, np.log, np.array(0.0, dtype=dt)
                )
                assert_raises(
                    FloatingPointError, np.log2, np.array(0.0, dtype=dt)
                )
                assert_raises(
                    FloatingPointError, np.log10, np.array(0.0, dtype=dt)
                )
                assert_raises(
                    FloatingPointError, np.log1p, np.array(-1.0, dtype=dt)
                )

        with np.errstate(invalid='raise'):
            for dt in ['e', 'f', 'd']:
                assert_raises(
                    FloatingPointError, np.log, np.array(-np.inf, dtype=dt)
                )
                assert_raises(
                    FloatingPointError, np.log, np.array(-1.0, dtype=dt)
                )
                assert_raises(
                    FloatingPointError, np.log2, np.array(-np.inf, dtype=dt)
                )
                assert_raises(
                    FloatingPointError, np.log2, np.array(-1.0, dtype=dt)
                )
                assert_raises(
                    FloatingPointError, np.log10, np.array(-np.inf, dtype=dt)
                )
                assert_raises(
                    FloatingPointError, np.log10, np.array(-1.0, dtype=dt)
                )
                assert_raises(
                    FloatingPointError, np.log1p, np.array(-np.inf, dtype=dt)
                )
                assert_raises(
                    FloatingPointError, np.log1p, np.array(-2.0, dtype=dt)
                )
    ...
    def test_sincos_errors(self, callable, dtype, value):
        with np.errstate(invalid='raise'):
            assert_raises(
                FloatingPointError, callable, np.array([value], dtype=dtype)
            )
    ...
    def test_square_values(self):
        ...
        with np.errstate(over='raise'):
            assert_raises(
                FloatingPointError, np.square, np.array(1e3, dtype='e')
            )
            assert_raises(
                FloatingPointError, np.square, np.array(1e32, dtype='f')
            )
            assert_raises(
                FloatingPointError, np.square, np.array(1e200, dtype='d')
            )
    ...
    def test_tan(self):
        ...
        with np.errstate(invalid='raise'):
            for dt in ['e', 'f', 'd']:
                assert_raises(
                    FloatingPointError, np.tan, np.array(np.inf, dtype=dt)
                )
                assert_raises(
                    FloatingPointError, np.tan, np.array(-np.inf, dtype=dt)
                )
    ...
    def test_sinh(self):
        ...
        with np.errstate(over='raise'):
            assert_raises(
                FloatingPointError, np.sinh, np.array(12.0, dtype='e')
            )
            assert_raises(
                FloatingPointError, np.sinh, np.array(120.0, dtype='f')
            )
            assert_raises(
                FloatingPointError, np.sinh, np.array(1200.0, dtype='d')
            )
    ...
    def test_cosh(self):
        ...
        with np.errstate(over='raise'):
            assert_raises(
                FloatingPointError, np.cosh, np.array(12.0, dtype='e')
            )
            assert_raises(
                FloatingPointError, np.cosh, np.array(120.0, dtype='f')
            )
            assert_raises(
                FloatingPointError, np.cosh, np.array(1200.0, dtype='d')
            )
    ...
    @pytest.mark.skipif(
        sys.platform == 'win32' and sys.maxsize < 2**31 + 1,
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            (
                ...
                INF_INVALID_ERR + LTONE_INVALID_ERR,
            ),
...
class TestFPClass:
    @pytest.mark.parametrize(
        'stride', [-5, -4, -3, -2, -1, 1, 2, 4, 5, 6, 7, 8, 9, 10]
    )
...
class TestLDExp:
    ...
    def test_ldexp(self, dtype, stride):
        mant = np.array(
            [0.125, 0.25, 0.5, 1.0, 1.0, 2.0, 4.0, 8.0], dtype=dtype
        )
...
class TestFRExp:
    ...
    def test_frexp(self, dtype, stride):
        ...
        mant, exp = np.frexp(
            arr[::stride], out=(out_mant[::stride], out_exp[::stride])
        )
    ...
    def test_avx_based_ufunc(self):
        ...
        for func, prop in avx_ufuncs.items():
            ...
            for size in range(1, 32):
                ...
                x_f32 = np.float32(
                    np.random.uniform(low=minval, high=maxval, size=size)
                )
                ...
                if maxulperr == 0:
                    ...
                    assert_array_max_ulp(
                        myfunc(x_f32), np.float32(y_true128), maxulp=maxulperr
                    )
                    assert_array_max_ulp(
                        myfunc(x_f64), np.float64(y_true128), maxulp=maxulperr
                    )
...
class TestAVXFloat32Transcendental:
    def test_exp_float32(self):
        ...
        x_f64 = np.float64(x_f32)
        assert_array_max_ulp(
            np.exp(x_f32), np.float32(np.exp(x_f64)), maxulp=3
        )
    ...
    def test_log_float32(self):
        ...
        x_f64 = np.float64(x_f32)
        assert_array_max_ulp(
            np.log(x_f32), np.float32(np.log(x_f64)), maxulp=4
        )
    ...
    def test_sincos_float32(self):
        ...
        x_f64 = np.float64(x_f32)
        assert_array_max_ulp(
            np.sin(x_f32), np.float32(np.sin(x_f64)), maxulp=2
        )
        assert_array_max_ulp(
            np.cos(x_f32), np.float32(np.cos(x_f64)), maxulp=2
        )
        # test aliasing(issue #17761)
        tx_f32 = x_f32.copy()
        assert_array_max_ulp(
...
class TestLdexp:
    def _check_ldexp(self, tp):
        assert_almost_equal(
            ncu.ldexp(np.array(2.0, np.float32), np.array(3, tp)), 16.0
        )
        assert_almost_equal(
            ncu.ldexp(np.array(2.0, np.float64), np.array(3, tp)), 16.0
        )
        assert_almost_equal(
...
class TestMaximum(_FilterInvalids):
    ...
    def test_strided_array(self):
        ...
        assert_equal(
            np.maximum(arr1[::3], arr2[:3:]), np.array([-2.0, 0.0, np.nan])
        )
        assert_equal(
...
class TestMinimum(_FilterInvalids):
    ...
    def test_strided_array(self):
        ...
        assert_equal(
            np.minimum(arr1[::3], arr2[:3:]), np.array([-4.0, -1.0, np.nan])
        )
        assert_equal(
...
class TestAbsoluteNegative:
    ...
    def test_noncontiguous(self, dtype, big):
        ...
        # contig in, ncontig out
        assert_array_equal(
            np.negative(contig_in, out=ncontig_out), expect[1::2]
        )
        # ncontig in, contig out
        assert_array_equal(np.negative(ncontig_in), expect[1::2])
        # ncontig in, ncontig out
        assert_array_equal(
            np.negative(ncontig_in, out=ncontig_out), expect[1::2]
        )
        # contig in, contig out, nd stride
        data_split = np.array(np.array_split(data, 2))
...
class TestSpecialMethods:
    ...
    def test_ufunc_override_methods(self):
        ...
        assert_equal(
            res[4],
            {'axis': 0, 'dtype': None, 'keepdims': False, 'where': True},
        )
        ...
        assert_equal(res[3], (a,))
        assert_equal(
            res[4], {'dtype': 'dtype0', 'out': ('out0',), 'axis': 'axis0'}
        )

        # accumulate, kwargs
        res = np.multiply.accumulate(
            a, axis='axis0', dtype='dtype0', out='out0'
        )
        assert_equal(res[0], a)
        assert_equal(res[1], np.multiply)
        assert_equal(res[2], 'accumulate')
        ...
        assert_raises(
            ValueError, np.multiply.accumulate, a, out=('out0', 'out1')
        )
        assert_raises(
            TypeError, np.multiply.accumulate, a, 'axis0', axis='axis0'
        )
        ...
        # reduceat, kwargs
        res = np.multiply.reduceat(
            a, [4, 2], axis='axis0', dtype='dtype0', out='out0'
        )
        assert_equal(res[0], a)
        assert_equal(res[1], np.multiply)
        assert_equal(res[2], 'reduceat')
        ...
        assert_raises(
            ValueError, np.multiply.reduce, a, [4, 2], out=('out0', 'out1')
        )
        assert_raises(
            TypeError, np.multiply.reduce, a, [4, 2], 'axis0', axis='axis0'
        )
        ...
        assert_raises(
            TypeError, np.multiply.outer, a, a, sig='a', signature='a'
        )
...
class TestRationalFunctions:
    ...
    def test_builtin_long(self):
        ...
        assert_equal(
            np.lcm(a, b), [2**100 * 3**5 * 5**7, 2**100 * 3**10]
        )
...
class TestComplexFunctions:
    ...
    def test_branch_cuts_complex64(self):
        ...
        _check_branch_cut(
            np.arcsin, [-2, 2], [1j, 1j], 1, -1, True, np.complex64
        )
        _check_branch_cut(
            np.arccos, [-2, 2], [1j, 1j], 1, -1, True, np.complex64
        )
        _check_branch_cut(
        ...
        _check_branch_cut(
            np.arccosh, [-1, 0.5], [1j, 1j], 1, -1, True, np.complex64
        )
        _check_branch_cut(
            np.arctanh, [-2, 2], [1j, 1j], 1, -1, True, np.complex64
        )
        ...
        _check_branch_cut(
            np.arctan, [-2, 2], [1j, 1j], 1, 1, False, np.complex64
        )
    ...
    @pytest.mark.xfail(IS_WASM, reason="doesn't work")
    @pytest.mark.parametrize(
        'dtype', [np.complex64, np.complex_, np.longcomplex]
    )
    def test_loss_of_precision(self, dtype):
        ...
        if dtype is np.longcomplex:
            if bad_arcsinh():
                pytest.skip(
                    'Trig functions of np.longcomplex values known '
...
@pytest.mark.xfail(
    condition=platform.machine().startswith('ppc64'),
    ...
    for t, direction in itertools.product(np.sctypes['float'], (1, -1)):
        ...
        with suppress_warnings() as sup:
            ...
            if not np.isnan(np.finfo(t).tiny):
                ...
                assert_(
                    0.0 < direction * np.nextafter(t(0), t(direction)) < tiny
                )
        assert_equal(
            np.nextafter(t(0), t(direction)) / t(2.1), direction * 0.0
        )
