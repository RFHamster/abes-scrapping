...
class TestSeterr:
    ...
    def test_errobj_noerrmask(self):
        ...
        try:
            ...
            np.seterrobj(
                [umath.UFUNC_BUFSIZE_DEFAULT, umath.ERR_DEFAULT + 1, None]
            )
            # call a ufunc
            np.isnan(np.array([6]))
            ...
            for i in range(10000):
                np.seterrobj(
                    [umath.UFUNC_BUFSIZE_DEFAULT, umath.ERR_DEFAULT, None]
                )
            np.isnan(np.array([6]))
        finally:
            np.seterrobj(olderrobj)
...
class TestTypes:
    def check_promotion_cases(self, promote_func):
        ...
        assert_equal(
            promote_func(fld, np.array([c64])), np.dtype(np.complex64)
        )
        assert_equal(
        ...
        # a complex128 array.
        assert_equal(
            promote_func(np.array([f32]), c128), np.dtype(np.complex64)
        )
...
class TestMoveaxis:
    ...
    def test_errors(self):
        ...
        assert_raises_regex(
            np.AxisError, 'source.*out of bounds', np.moveaxis, x, 3, 0
        )
        assert_raises_regex(
