...
_concatenate = getattr(
    _from_nx.concatenate, '__wrapped__', _from_nx.concatenate
)
...
def _concatenate_shapes(shapes, axis):
    ...
    if any(
        shape[:axis] != first_shape_pre
        or shape[axis + 1 :] != first_shape_post
        for shape in shapes
    ):
        raise ValueError(
...
def _block(arrays, max_depth, result_ndim, depth=0):
    ...
    if depth < max_depth:
        arrs = [
            _block(arr, max_depth, result_ndim, depth + 1) for arr in arrays
        ]
