...
def _leading_trailing(a, edgeitems, index=()):
    ...
    if a.shape[axis] > 2 * edgeitems:
        return concatenate(
            (
                _leading_trailing(
                    a, edgeitems, index + np.index_exp[:edgeitems]
                ),
                _leading_trailing(
                    a, edgeitems, index + np.index_exp[-edgeitems:]
                ),
...
def _extendLine_pretty(s, line, word, line_width, next_line_prefix, legacy):
    ...
    if len(line) + max_word_length > line_width and len(line) > len(
        next_line_prefix
    ):
        s += line.rstrip() + '\n'
    ...
    def __call__(self, x):
        if not np.isfinite(x):
            with errstate(invalid='ignore'):
                ...
                return (
                    ' ' * (self.pad_left + self.pad_right + 1 - len(ret)) + ret
                )
...
class DatetimeFormat(_TimelikeFormat):
    def __init__(
        self, x, unit=None, timezone=None, casting='same_kind', legacy=False
    ):
...
def _array_repr_implementation(
    ...
    if (
        _format_options['legacy'] <= 113
        and arr.shape == ()
        and not arr.dtype.names
    ):
        lst = repr(arr.item())
