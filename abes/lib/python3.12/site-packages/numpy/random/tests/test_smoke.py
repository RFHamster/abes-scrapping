...
class RNG:
    ...
    def test_uniform_array(self):
        ...
        assert_((r <= 0).all())
        r = self.rg.uniform(
            np.array([-1.0] * 10), np.array([0.0] * 10), size=10
        )
        assert_(len(r) == 10)
        assert_((r > -1).all())
        assert_((r <= 0).all())
    ...
    def test_standard_exponential_float_log(self):
        randoms = self.rg.standard_exponential(
            10, dtype='float32', method='inv'
        )
        assert_(len(randoms) == 10)
        assert randoms.dtype == np.float32
        params_0(
            partial(
                self.rg.standard_exponential, dtype='float32', method='inv'
            )
        )
    ...
    def test_entropy_init(self):
        ...
        assert_(
            not comp_state(rg.bit_generator.state, rg2.bit_generator.state)
        )
    ...
    def test_pickle(self):
        ...
        assert_(
            comp_state(self.rg.bit_generator.state, unpick.bit_generator.state)
        )
    ...
    def test_seed_array(self):
        ...
        seed = (
            2
            ** np.mod(np.arange(1500, dtype=dtype), self.seed_vector_bits - 1)
            + 1
        )
        bg = self.bit_generator(seed)
        state1 = bg.state
...
class TestDefaultRNG(RNG):
    ...
    def test_seed(self):
        ...
        np.random.default_rng(
            [
                43660444402423911716352051725018508569,
