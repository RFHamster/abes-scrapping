...
def _ischaracter(var):
    return (
        'typespec' in var
        and var['typespec'] == 'character'
        and not isexternal(var)
    )
...
def iscomplex(var):
    return isscalar(var) and var.get('typespec') in [
        'complex',
        'double complex',
    ]
...
def isthreadsafe(rout):
    return (
        'f2pyenhancements' in rout and 'threadsafe' in rout['f2pyenhancements']
    )
...
def getfortranname(rout):
    try:
        ...
        if not name:
            errmess(
                'Failed to use fortranname from %s\n'
...
def get_f2py_modulename(source):
    ...
    with open(source) as f:
        for line in f:
            ...
            if m:
                if _f2py_user_module_name_match(
                    line
                ):   # skip *__user__* names
                    continue
                name = m.group('name')
