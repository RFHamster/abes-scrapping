...
class Expr:
    ...
    def tostring(
        self, parent_precedence=Precedence.NONE, language=Language.Fortran
    ):
        """Return a string representation of Expr."""
        if self.op in (Op.INTEGER, Op.REAL):
            precedence = (
                Precedence.SUM if self.data[0] < 0 else Precedence.ATOM
            )
            r = str(self.data[0]) + (
                f'_{self.data[1]}' if self.data[1] != 4 else ''
            )
        elif self.op is Op.COMPLEX:
            r = ', '.join(
                item.tostring(Precedence.TUPLE, language=language)
            ...
            for base, exp in sorted(self.data.items()):
                ...
                if exp == 1:
                    ...
                    if exp in range(2, 10):
                        ...
                        factor = base.tostring(
                            Precedence.TUPLE, language=language
                        )
            ...
            if name is ArithOp.DIV and language is Language.C:
                numer, denom = [
                    arg.tostring(Precedence.PRODUCT, language=language)
                    for arg in args
                ]
                r = f'{numer} / {denom}'
            ...
            cond, expr1, expr2 = [
                a.tostring(Precedence.TUPLE, language=language)
                for a in self.data
            ]
            ...
            precedence = (
                Precedence.EQ if rop in (RelOp.EQ, RelOp.NE) else Precedence.LT
            )
    ...
    def substitute(self, symbols_map):
        ...
        if self.op in (Op.ARRAY, Op.COMPLEX):
            return Expr(
                self.op,
                tuple(item.substitute(symbols_map) for item in self.data),
            )
        ...
        if self.op is Op.TERMS:
            ...
            if r is None:
                ewarn(
                    'substitute: empty TERMS expression interpreted as'
                    ' int-literal 0'
                )
                return as_number(0)
            return r
        ...
        if self.op is Op.APPLY:
            ...
            kwargs = dict(
                (k, v.substitute(symbols_map)) for k, v in kwargs.items()
            )
    ...
    def traverse(self, visit, *args, **kwargs):
        ...
        if self.op in (Op.INTEGER, Op.REAL, Op.STRING, Op.SYMBOL):
            ...
            obj = self.data[0]
            func = (
                obj.traverse(visit, *args, **kwargs)
                if isinstance(obj, Expr)
                else obj
            )
            operands = tuple(
                operand.traverse(visit, *args, **kwargs)
                for operand in self.data[1]
            )
            kwoperands = dict(
                (k, v.traverse(visit, *args, **kwargs))
    ...
    def linear_solve(self, symbol):
        ...
        if zero != as_number(0):
            raise RuntimeError(
                f'not a {symbol}-linear equation:'
                f' {a} * {symbol} + {b} == {self}'
            )
        return a, b
...
class _FromStringWorker:
    ...
    def process(self, s, context='expr'):
        ...
        m = re.match(
            r'\A(.+)\s*(@__f2py_PARENTHESIS_(ROUND|SQUARE)_\d+@)\Z', r
        )
        if m:
            ...
            if paren == 'ROUND':
                kwargs = dict(
                    (a.left, a.right) for a in args if isinstance(a, _Pair)
                )
