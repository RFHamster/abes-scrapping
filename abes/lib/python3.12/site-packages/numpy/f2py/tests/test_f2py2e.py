...
def gh23598_warn(tmpdir_factory):
    ...
    fdat = util.getpath(
        'tests', 'src', 'crackfortran', 'gh23598Warn.f90'
    ).read_text()
...
def f2cmap_f90(tmpdir_factory):
    """Generates a single f90 file for testing"""
    fdat = util.getpath(
        'tests', 'src', 'f2cmap', 'isoFortranEnvMap.f90'
    ).read_text()
...
@pytest.mark.skipif(
    (platform.system() != 'Linux') or (sys.version_info <= (3, 12)),
...
def test_f2py_skip(capfd, retreal_f77, monkeypatch):
    ...
    remaining = 'td s0'
    monkeypatch.setattr(
        sys, 'argv', f'f2py {ipath} -m test skip: {toskip}'.split()
    )
...
def test_f2py_only(capfd, retreal_f77, monkeypatch):
    ...
    tokeep = 'td s0'
    monkeypatch.setattr(
        sys, 'argv', f'f2py {ipath} -m test only: {tokeep}'.split()
    )
...
def test_file_processing_switch(
    capfd, hello_world_f90, retreal_f77, monkeypatch
):
...
def test_lower_cmod(capfd, hello_world_f77, monkeypatch):
    ...
    monkeypatch.setattr(
        sys, 'argv', f'f2py {ipath} -m test --no-lower'.split()
    )
    with util.switchdir(ipath.parent):
        f2pycli()
        out, _ = capfd.readouterr()
...
def test_latexdoc(capfd, hello_world_f90, monkeypatch):
    ...
    mname = 'blah'
    monkeypatch.setattr(
        sys, 'argv', f'f2py -m {mname} {ipath} --latex-doc'.split()
    )
...
def test_nolatexdoc(capfd, hello_world_f90, monkeypatch):
    ...
    mname = 'blah'
    monkeypatch.setattr(
        sys, 'argv', f'f2py -m {mname} {ipath} --no-latex-doc'.split()
    )
...
def test_restdoc(capfd, hello_world_f90, monkeypatch):
    ...
    mname = 'blah'
    monkeypatch.setattr(
        sys, 'argv', f'f2py -m {mname} {ipath} --rest-doc'.split()
    )
...
def test_norestexdoc(capfd, hello_world_f90, monkeypatch):
    ...
    mname = 'blah'
    monkeypatch.setattr(
        sys, 'argv', f'f2py -m {mname} {ipath} --no-rest-doc'.split()
    )
...
def test_debugcapi(capfd, hello_world_f90, monkeypatch):
    ...
    mname = 'blah'
    monkeypatch.setattr(
        sys, 'argv', f'f2py -m {mname} {ipath} --debug-capi'.split()
    )
...
def test_debugcapi_bld(hello_world_f90, monkeypatch):
    ...
    mname = 'blah'
    monkeypatch.setattr(
        sys, 'argv', f'f2py -m {mname} {ipath} -c --debug-capi'.split()
    )
