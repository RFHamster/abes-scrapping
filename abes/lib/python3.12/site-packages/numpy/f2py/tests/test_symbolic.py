...
class TestSymbolic(util.F2PyTest):
    ...
    def test_sanity(self):
        ...
        assert repr(c) == (
            'Expr(Op.COMPLEX, (Expr(Op.INTEGER, (1, 4)),'
            ' Expr(Op.INTEGER, (2, 4))))'
        )
        assert c == c
        assert c != c2
    ...
    def test_tostring_c(self):
        ...
        assert (
            Expr(Op.FACTORS, {x: 12}).tostring(language=language)
            == 'pow(x, 12)'
        )

        assert (
            as_apply(ArithOp.DIV, x, y).tostring(language=language) == 'x / y'
        )
        assert (
            as_apply(ArithOp.DIV, x, x + y).tostring(language=language)
            == 'x / (x + y)'
        )
        assert (
    ...
    def test_substitute(self):
        ...
        assert as_ternary(x, y, z).substitute({x: y + z}) == as_ternary(
            y + z, y, z
        )
    ...
    def test_fromstring(self):
        ...
        assert (
            fromstring('x ** 2 ** 3', language=Language.Python) == x**2**3
        )
        ...
        assert fromstring(
            'PERSON(name="John", age=50, shape=(/34, 23/))'
        ) == as_apply(
            as_symbol('PERSON'),
            name=as_string('"John"'),
            age=as_number(50),
    ...
    def test_traverse(self):
        ...
        assert (x + f(y, x - z)).traverse(replace_visit) == (
            z + f(y, as_number(0))
        )
