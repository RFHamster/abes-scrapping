...
cb_rout_rules = [
    ...
    if (capi_j>capi_i) {
        ...
        '_check': l_and(
            isfunction, l_not(isstringfunction), l_not(iscomplexfunction)
        ),
    },
    {  # String function
        'pyobjfrom': {
...
cb_arg_rules = [
    {  # Doc
        'docstropt': {
            l_and(isoptional, isintent_nothide): '        #pydocsign#'
        },
        'docstrreq': {
            l_and(isrequired, isintent_nothide): '        #pydocsign#'
        },
    ...
    {  # Scalars
        ...
        'error': {
            l_and(
                ...
                isintent_out,
                throw_error(
                    'intent(c,out) is forbidden for callback scalar arguments'
                ),
            ): ''
        },
        'frompyobj': [
...
""",
        ...
        '_check': l_and(
            isarray, isintent_nothide, l_or(isintent_in, isintent_inout)
        ),
        '_optional': '',
    },
    {
...
def buildcallback(rout, um):
    ...
    outmess(
        '    Constructing call-back function "cb_%s_in_%s"\n'
        % (rout['name'], um)
    )
...
"""
    ...
    optargs = stripcomma(
        replace('#docsignopt#', {'docsignopt': rd['docsignopt']})
    )
    if optargs == '':
