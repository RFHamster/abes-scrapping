...
dt2 = np.dtype(
    {'names': ['a', 'b'], 'formats': ['i4', 'i4'], 'offsets': [1, 6]}
)
...
def test_pickle_python2_python3():
    ...
    expected = np.array(
        [None, range, '\u512a\u826f', b'\xe4\xb8\x8d\xe8\x89\xaf'],
        dtype=object,
    )
...
def test_pickle_disallow(tmpdir):
    ...
    assert_raises(
        ValueError, np.load, path, allow_pickle=False, encoding='latin1'
    )
...
def test_write_version():
    ...
    for version in bad_versions:
        with assert_raises_regex(
            ValueError, 'we only support format version.*'
        ):
...
def test_unicode_field_names(tmpdir):
    ...
    arr = np.array(
        ...
        dtype=[
            ('int', int),
            (
                '\N{CJK UNIFIED IDEOGRAPH-6574}\N{CJK UNIFIED IDEOGRAPH-5F62}',
                int,
            ),
        ],
    )
            ...
            np.dtype(
                {
                    ...
                    'formats': [
                        float,
                        np.dtype('S3', metadata={'some': 'stuff'}),
                    ],
                }
            ),
            True,
