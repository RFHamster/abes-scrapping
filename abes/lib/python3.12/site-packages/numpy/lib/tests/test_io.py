...
class TestSavezLoad(RoundtripTest):
    ...
    def test_savez_filename_clashes(self):
        ...
        errors = []
        threads = [
            threading.Thread(target=writer, args=(errors,)) for j in range(3)
        ]
        for t in threads:
            t.start()
        for t in threads:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                5,
                'NpzFile {fname!r} with keys: arr_0, arr_1, arr_2, arr_3, arr_4',
            ),
...
class TestSaveTxt:
    ...
    def test_header_footer(self):
        ...
        c.seek(0)
        assert_equal(
            c.read(), asbytes('# ' + test_header_footer + '\n1 2\n3 4\n')
        )
        ...
        commentstr = '% '
        np.savetxt(
            c, a, fmt='%1d', header=test_header_footer, comments=commentstr
        )
        c.seek(0)
        assert_equal(
            c.read(),
        ...
        commentstr = '% '
        np.savetxt(
            c, a, fmt='%1d', footer=test_header_footer, comments=commentstr
        )
        c.seek(0)
        assert_equal(
            c.read(),
    ...
    @pytest.mark.skipif(
        sys.platform == 'win32', reason='files>4GB may not work'
    )
    @pytest.mark.slow
    ...
    def test_large_zip(self):
        def check_large_zip(memoryerror_raised):
            ...
            try:
                ...
                with tempdir() as tmpdir:
                    np.savez(
                        os.path.join(tmpdir, 'test.npz'), test_data=test_data
                    )
            except MemoryError:
...
class LoadTxtBase:
    ...
    def test_converters_decode(self):
        ...
        c.seek(0)
        x = self.loadfunc(
            c, dtype=np.str_, converters={0: lambda x: x.decode('UTF-8')}
        )
...
class TestLoadTxt(LoadTxtBase):
    ...
    def test_record(self):
        ...
        b = np.array(
            [('M', 64.0, 75.0), ('F', 25.0, 60.0)], dtype=mydescriptor
        )
    ...
    def test_unused_converter(self):
        ...
        c.seek(0)
        data = np.loadtxt(
            c, usecols=(1,), converters={0: lambda s: int(s, 16)}
        )
        assert_array_equal(data, [21, 42])

        c.seek(0)
        data = np.loadtxt(
            c, usecols=(1,), converters={1: lambda s: int(s, 16)}
        )
    ...
    def test_max_rows_empty_lines(self, skip, data):
        with pytest.warns(
            UserWarning, match=f'Input line 3.*max_rows={3-skip}'
        ):
            res = np.loadtxt(
                data,
                dtype=int,
...
class Testfromregex:
    def test_record(self):
        ...
        a = np.array(
            [(1.312, 'foo'), (1.534, 'bar'), (4.444, 'qux')], dtype=dt
        )
        assert_array_equal(x, a)
    ...
    def test_record_unicode(self, path_type):
        ...
        with temppath() as str_path:
            ...
            x = np.fromregex(
                path, r'(?u)([0-9.]+)\s+(\w+)', dt, encoding='UTF-8'
            )
            a = np.array(
                [
                    (1.312, 'foo' + utf8.decode('UTF-8')),
                    (1.534, 'bar'),
                    (4.444, 'qux'),
                ],
                dtype=dt,
...
class TestFromTxt(LoadTxtBase):
    ...
    def test_record(self):
        ...
        control = np.array(
            [('M', 64.0, 75.0), ('F', 25.0, 60.0)], dtype=descriptor
        )
    ...
    def test_skip_footer_with_invalid(self):
        with suppress_warnings() as sup:
            ...
            assert_raises(
                ValueError, np.genfromtxt, TextIO(basestr), skip_footer=1
            )
            #        except ValueError:
            #            pass
            a = np.genfromtxt(
            ...
            assert_equal(
                a, np.array([[1.0, 1.0], [3.0, 3.0], [4.0, 4.0], [6.0, 6.0]])
            )
            a = np.genfromtxt(
...
M   33  21.99
    ...
    def test_names_and_comments_none(self):
        ...
        control = np.array(
            [(1, 2), (3, 4)], dtype=[('col1', int), ('col2', int)]
        )
        assert_equal(test, control)
    ...
    def test_autonames_and_usecols(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            warnings.filterwarnings('always', '', np.VisibleDeprecationWarning)
            test = np.genfromtxt(
                data, usecols=('A', 'C', 'D'), names=True, dtype=None
            )
            assert_(w[0].category is np.VisibleDeprecationWarning)
        control = np.array(
    ...
    def test_unused_converter(self):
        ...
        data = TextIO('1 21\n  3 42\n')
        test = np.genfromtxt(
            data, usecols=(1,), converters={0: lambda s: int(s, 16)}
        )
        assert_equal(test, [21, 42])
        #
        data.seek(0)
        test = np.genfromtxt(
            data, usecols=(1,), converters={1: lambda s: int(s, 16)}
        )
    ...
    def test_invalid_converter(self):
        ...
        kwargs = dict(
            converters={2: strip_per, 3: strip_rand}, delimiter=',', dtype=None
        )
    ...
    def test_userconverters_with_explicit_dtype(self):
        ...
        control = np.array(
            [('2001-01-01', 1.0)], dtype=[('', '|S10'), ('', float)]
        )
        assert_equal(test, control)
    ...
    def test_utf8_userconverters_with_explicit_dtype(self):
        ...
        with temppath() as path:
            ...
            test = np.genfromtxt(
                ...
                dtype=float,
                usecols=(2, 3),
                converters={2: np.compat.unicode},
                encoding='UTF-8',
            )
        control = np.array(
    ...
    def test_fancy_dtype_alt(self):
        ...
        fancydtype = np.dtype([('x', int), ('y', [('t', int), ('s', float)])])
        test = np.genfromtxt(
            data, dtype=fancydtype, delimiter=',', usemask=True
        )
    ...
    def test_user_filling_values(self):
        ...
        ctrl = np.array(
            [(0, 2, 3), (4, 0, -999)], dtype=[(_, int) for _ in 'abc']
        )
        assert_equal(test, ctrl)
        #
        test = np.genfromtxt(TextIO(data), usecols=(0, -1), **kwargs)
    ...
    def test_with_masked_column_uniform(self):
        ...
        data = TextIO('1 2 3\n4 5 6\n')
        test = np.genfromtxt(
            data, dtype=None, missing_values='2,5', usemask=True
        )
    ...
    def test_invalid_raise(self):
        ...
        mdata.seek(0)
        assert_raises(
            ValueError, np.genfromtxt, mdata, delimiter=',', names=True
        )
    ...
    def test_invalid_raise_with_usecols(self):
        ...
        kwargs = dict(
            delimiter=',', dtype=None, names=True, invalid_raise=False
        )
    ...
    def test_single_dtype_w_explicit_names(self):
        ...
        data = '0, 1, 2.3\n4, 5, 6.7'
        mtest = np.genfromtxt(
            TextIO(data), delimiter=',', dtype=float, names='a, b, c'
        )
        ctrl = np.array(
            [(0.0, 1.0, 2.3), (4.0, 5.0, 6.7)],
            dtype=[(_, float) for _ in 'abc'],
        )
        assert_equal(mtest, ctrl)
    ...
    def test_single_dtype_w_implicit_names(self):
        ...
        data = 'a, b, c\n0, 1, 2.3\n4, 5, 6.7'
        mtest = np.genfromtxt(
            TextIO(data), delimiter=',', dtype=float, names=True
        )
        ctrl = np.array(
            [(0.0, 1.0, 2.3), (4.0, 5.0, 6.7)],
            dtype=[(_, float) for _ in 'abc'],
    ...
    def test_replace_space(self):
        ...
        test = np.genfromtxt(
            TextIO(txt), delimiter=',', names=True, dtype=None
        )
    ...
    def test_latin1(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            warnings.filterwarnings('always', '', np.VisibleDeprecationWarning)
            test = np.genfromtxt(
                TextIO(s), dtype=None, comments=None, delimiter=','
            )
            assert_(w[0].category is np.VisibleDeprecationWarning)
        ...
        with temppath() as path:
            ...
            with warnings.catch_warnings(record=True) as w:
                warnings.filterwarnings(
                    'always', '', np.VisibleDeprecationWarning
                )
                test = np.genfromtxt(
                    path, dtype=None, comments=None, delimiter=','
                )
    ...
    def test_max_rows(self):
        ...
        control = np.array(
            [(4.0, 4.0), (5.0, 5.0)], dtype=[('c', '<f8'), ('d', '<f8')]
        )
        assert_equal(test, control)
