...
def test_lookfor():
    out = StringIO()
    utils.lookfor(
        'eigenvalue', module='numpy', output=out, import_modules=False
    )
    out = out.getvalue()
...
def test_drop_metadata():
    ...
    # structured dtype
    dt = np.dtype(
        [('l1', [('l2', np.dtype('S8', metadata={'msg': 'toto'}))])],
        metadata={'msg': 'titi'},
    )
    ...
    # alignement
    dt = np.dtype(
        [('x', '<f8'), ('y', '<i4')], align=True, metadata={'msg': 'toto'}
    )
    dt_m = utils.drop_metadata(dt)
    assert _compare_dtypes(dt, dt_m) is True
    assert dt_m.metadata is None
...
@pytest.mark.parametrize(
    'dtype',
    [np.dtype('i,i,i,i')[['f1', 'f3']], np.dtype('f8'), np.dtype('10i')],
)
