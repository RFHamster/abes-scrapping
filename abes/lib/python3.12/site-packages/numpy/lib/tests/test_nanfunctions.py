...
class TestNanFunctions_MinMax:
    ...
    def test_object_array(self):
        arr = np.array(
            [[1.0, 2.0], [np.nan, 4.0], [np.nan, np.nan]], dtype=object
        )
    ...
    def test_float_special(self):
        with suppress_warnings() as sup:
            ...
            for inf in [np.inf, -np.inf]:
                ...
                if inf > 0:
                    assert_equal(
                        np.nanmedian(a, axis=0), [4.0, 7.0, -inf, 5.0]
                    )
                    assert_equal(np.nanmedian(a), 4.5)
                else:
                    assert_equal(
                        np.nanmedian(a, axis=0), [-10.0, 7.0, -inf, -9.0]
                    )
    ...
    def test_empty(self):
        ...
        for axis in [1]:
            with warnings.catch_warnings(record=True) as w:
                warnings.simplefilter('always')
                assert_equal(
                    np.nanpercentile(mat, 40, axis=axis), np.zeros([])
                )
                assert_(len(w) == 0)
...
class TestNanFunctions_Quantile:
    ...
    def test_regression(self):
        ...
        assert_equal(
            np.nanquantile(ar, q=0.5, axis=0),
            np.nanpercentile(ar, q=50, axis=0),
        )
        assert_equal(
            np.nanquantile(ar, q=0.5, axis=1),
        ...
        assert_equal(
            np.nanquantile(ar, q=[0.25, 0.5, 0.75], axis=1),
