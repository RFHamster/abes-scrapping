...
class TestPolynomial:
    ...
    def test_poly(self):
        assert_array_almost_equal(
            np.poly([3, -np.sqrt(2), np.sqrt(2)]), [1, -3, -2, 6]
        )
        ...
        assert_(
            np.isrealobj(
                np.poly(
                    [0 + 1j, -0 + -1j, 1 + 2j, 1 - 2j, 1.0 + 3.5j, 1 - 3.5j]
                )
            )
        )
        assert_(
            np.isrealobj(np.poly([1j, -1j, 1 + 2j, 1 - 2j, 1 + 3j, 1 - 3.0j]))
        )
    ...
    def test_objects(self):
        ...
        assert_equal(
            np.poly([Decimal(1), Decimal(2)]), [1, Decimal(-3), Decimal(2)]
        )
