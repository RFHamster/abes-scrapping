...
def _average_dispatcher(
    a, axis=None, weights=None, returned=None, *, keepdims=None
):
    return (a, weights)
...
def average(
    ...
    if weights is None:
        ...
        avg = avg_as_array = (
            np.multiply(a, wgt, dtype=result_dtype).sum(axis, **keepdims_kw)
            / scl
        )
...
def select(condlist, choicelist, default=0):
    ...
    for i, cond in enumerate(condlist):
        if cond.dtype.type is not np.bool_:
            raise TypeError(
                'invalid entry {} in condlist: should be boolean ndarray'.format(
                    i
                )
            )
...
def unwrap(p, discont=None, axis=-1, *, period=2 * pi):
    ...
    if boundary_ambiguous:
        ...
        _nx.copyto(
            ddmod, interval_high, where=(ddmod == interval_low) & (dd > 0)
        )
    ph_correct = ddmod - dd
...
def _create_arrays(
    broadcast_shape, dim_sizes, list_of_core_dims, dtypes, results=None
):
...
class vectorize:
    ...
    def _get_ufunc_and_otypes(self, func, args):
        ...
        if self.otypes is not None:
            ...
            if builtins.any(arg.size == 0 for arg in args):
                raise ValueError(
                    'cannot call `vectorize` on size 0 inputs '
    ...
    def _vectorize_call_with_signature(self, func, args):
        ...
        input_shapes = _calculate_shapes(
...
def _corrcoef_dispatcher(
    x, y=None, rowvar=None, bias=None, ddof=None, *, dtype=None
):
    return (x, y)
...
def corrcoef(
    x, y=None, rowvar=True, bias=np._NoValue, ddof=np._NoValue, *, dtype=None
):
    """
...
def blackman(M):
    ...
    n = arange(1 - M, M, 2)
    return (
        0.42 + 0.5 * cos(pi * n / (M - 1)) + 0.08 * cos(2.0 * pi * n / (M - 1))
    )
...
def _ureduce(a, func, keepdims=False, **kwargs):
    ...
    if keepdims:
        if axis is None:
            ...
            index_r = tuple(
                np.newaxis if i in axis else slice(None) for i in range(nd)
            )
...
def _closest_observation(n, quantiles):
    ...
    return _discret_interpolation_to_boundaries(
        (n * quantiles) - 1 - 0.5, gamma_fun
    )
