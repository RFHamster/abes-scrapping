...
def _get_fieldspec(dtype):
    ...
    if dtype.names is None:
        ...
        return [
            (name if len(f) == 2 else (f[2], name), f[0]) for name, f in fields
        ]
...
def append_fields(
    ...
    if len(data) > 1:
        data = merge_arrays(
            data, flatten=True, usemask=usemask, fill_value=fill_value
        )
    else:
        data = data.pop()
    #
        ...
        if f_dt.names is None:
            ...
            for i in range(n):
                if i == 0:
                    ...
                    fields.extend(
                        [(d, c, o + i * size) for d, c, o in subfields]
                    )
    return fields
...
def _structured_to_unstructured_dispatcher(
    arr, dtype=None, copy=None, casting=None
):
    return (arr,)
        ...
        for fname, fdtype in _get_fieldspec(dtype_n):
            if fname not in names:
                ...
                if autoconvert:
                    ...
                    raise TypeError(
                        "Incompatible type '%s' <> '%s'" % (cdtype, fdtype)
                    )
