...
def _nanmin_dispatcher(
    a, axis=None, out=None, keepdims=None, initial=None, where=None
):
    return (a, out)
...
def nanmin(
    ...
    if type(a) is np.ndarray and a.dtype != np.object_:
        ...
        if np.any(mask):
            ...
            warnings.warn(
                'All-NaN axis encountered', RuntimeWarning, stacklevel=2
            )
    return res


def _nanmax_dispatcher(
    a, axis=None, out=None, keepdims=None, initial=None, where=None
):
    return (a, out)
...
def nanmean(
    ...
    *,
    ...
    cnt = np.sum(
        ~mask, axis=axis, dtype=np.intp, keepdims=keepdims, where=where
    )
    tot = np.sum(
...
def _nanmedian_small(a, axis=None, out=None, overwrite_input=False):
    ...
    for i in range(np.count_nonzero(m.mask.ravel())):
        warnings.warn(
            'All-NaN slice encountered', RuntimeWarning, stacklevel=5
        )
...
def nanmedian(
    a, axis=None, out=None, overwrite_input=False, keepdims=np._NoValue
):
    """
...
def _nanquantile_1d(arr1d, q, overwrite_input=False, method='linear'):
    ...
    See nanpercentile for parameter usage
    """
    arr1d, overwrite_input = _remove_nan_1d(
...
def nanvar(
    ...
    *,
    ...
    cnt = np.sum(
        ~mask, axis=axis, dtype=np.intp, keepdims=_keepdims, where=where
    )
