...
class URL:
        ...
        if not encoded:
            ...
            if not netloc:  # netloc
                ...
                if password is None and username is None:
                    ...
                    raw_user = (
                        cls._REQUOTER(username) if username else username
                    )
                    raw_password = (
                        cls._REQUOTER(password) if password else password
                    )
                    netloc = cls._make_netloc(
                        raw_user, raw_password, host, port
                    )
            ...
            fragment = (
                cls._FRAGMENT_REQUOTER(fragment) if fragment else fragment
            )
            cache['scheme'] = scheme
        ...
        *,
        ...
        if encoded:
            ...
            if authority:
                ...
                _host = (
                    cls._encode_host(_host, validate_host=False)
                    if _host
                    else ''
                )
            elif host:
    ...
    def __str__(self) -> str:
        ...
        if (
        ...
        ) is not None and port == self._default_port:
            ...
            netloc = self._make_netloc(
                self.raw_user, self.raw_password, host, None
            )
    ...
    def __eq__(self, other: object) -> bool:
        ...
        if not val1.path and val1.netloc:
            ...
            val1 = tuple.__new__(
                SplitResult, (scheme, netloc, '/', query, fragment)
            )
    ...
    def _origin(self) -> 'URL':
        ...
        if '@' in netloc:
            ...
            netloc = self._make_netloc(
                None, None, encoded_host, self.explicit_port
            )
        elif not path and not query and not fragment:
            return self
        return self._from_val(
            tuple.__new__(SplitResult, (scheme, netloc, '', '', ''))
        )
    ...
    def parent(self) -> 'URL':
        ...
        if not path or path == '/':
            if fragment or query:
                val = tuple.__new__(
                    SplitResult, (scheme, netloc, path, '', '')
                )
                return self._from_val(val)
            return self
        parts = path.split('/')
        val = tuple.__new__(
            SplitResult, (scheme, netloc, '/'.join(parts[:-1]), '', '')
        )
        return self._from_val(val)
    ...
    def _make_child(
        self, paths: 'Sequence[str]', encoded: bool = False
    ) -> 'URL':
        """
        ...
        if netloc := v.netloc:
            ...
            parsed = (
                _normalize_path_segments(parsed) if needs_normalize else parsed
            )
    ...
    def _split_netloc(
        ...
        Union[str, None], Union[str, None], Union[str, None], Union[int, None]
    ]:
    ...
    def with_scheme(self, scheme: str) -> 'URL':
        ...
        if not netloc and lower_scheme in SCHEME_REQUIRES_HOST:
            ...
            raise ValueError(msg)
        val = tuple.__new__(
            SplitResult, (lower_scheme, netloc, path, query, fragment)
        )
        return self._from_val(val)
    ...
    def with_user(self, user: Union[str, None]) -> 'URL':
        ...
        if not netloc:
            raise ValueError(
                'user replacement is not allowed for relative URLs'
            )
        encoded_host = self.host_subcomponent or ''
        netloc = self._make_netloc(
            user, password, encoded_host, self.explicit_port
        )
        return self._from_val(
    ...
    def with_password(self, password: Union[str, None]) -> 'URL':
        ...
        if not netloc:
            raise ValueError(
                'password replacement is not allowed for relative URLs'
            )
        encoded_host = self.host_subcomponent or ''
        port = self.explicit_port
    ...
    def with_host(self, host: str) -> 'URL':
        ...
        if not netloc:
            raise ValueError(
                'host replacement is not allowed for relative URLs'
            )
        if not host:
            raise ValueError('host removing is not allowed')
        encoded_host = (
            self._encode_host(host, validate_host=True) if host else ''
        )
        port = self.explicit_port
        netloc = self._make_netloc(
            self.raw_user, self.raw_password, encoded_host, port
        )
        return self._from_val(
            tuple.__new__(SplitResult, (scheme, netloc, path, query, fragment))
        )
    ...
    def with_port(self, port: Union[int, None]) -> 'URL':
        ...
        if port is not None:
            if isinstance(port, bool) or not isinstance(port, int):
                raise TypeError(
                    f'port should be int or None, got {type(port)}'
                )
            if not (0 <= port <= 65535):
                raise ValueError(
        ...
        if not netloc:
            raise ValueError(
                'port replacement is not allowed for relative URLs'
            )
        encoded_host = self.host_subcomponent or ''
        netloc = self._make_netloc(
            self.raw_user, self.raw_password, encoded_host, port
    ...
    def with_fragment(self, fragment: Union[str, None]) -> 'URL':
        ...
        return self._from_val(
            tuple.__new__(
                SplitResult, (scheme, netloc, path, query, raw_fragment)
            )
        )
    ...
    def with_name(self, name: str) -> 'URL':
        ...
        if netloc:
            ...
            if parts[0] == '/':
                parts[0] = ''  # replace leading '/'
        val = tuple.__new__(
            SplitResult, (scheme, netloc, '/'.join(parts), '', '')
        )
        return self._from_val(val)
    ...
    def with_suffix(self, suffix: str) -> 'URL':
        ...
        name = (
            name + suffix
            if not old_suffix
            else name[: -len(old_suffix)] + suffix
        )
    ...
    def join(self, url: 'URL') -> 'URL':
        ...
        fragment = (
            join_fragment if join_path or join_fragment else orig_fragment
        )
        ...
        return self._from_val(
            tuple.__new__(
                SplitResult, (scheme, orig_netloc, path, query, fragment)
            )
        )
