...
def test_Connection_basics_and_content_length() -> None:
    ...
    data = p.send(
        SERVER, Response(status_code=200, headers=[('Content-Length', '11')])
    )
    ...
    data = p.send(
        SERVER, Data(data=b'1'), expect=[Data(data=b'1'), EndOfMessage()]
    )
    assert data == b'1'
...
def test_chunked() -> None:
    ...
    data = p.send(
        CLIENT, Data(data=b'1234567890', chunk_start=True, chunk_end=True)
    )
    assert data == b'a\r\n1234567890\r\n'
    data = p.send(
        CLIENT, Data(data=b'abcde', chunk_start=True, chunk_end=True)
    )
    assert data == b'5\r\nabcde\r\n'
    ...
    p.send(
        SERVER,
        Response(status_code=200, headers=[('Transfer-Encoding', 'chunked')]),
    )
...
def test_reuse_simple() -> None:
    ...
    p.send(
        ...
        [
            Response(
                status_code=200, headers=[(b'transfer-encoding', b'chunked')]
            ),
            EndOfMessage(),
        ],
    )
    ...
    p.send(
        ...
        [
            Response(
                status_code=404, headers=[(b'transfer-encoding', b'chunked')]
            ),
            EndOfMessage(),
        ],
    )
...
def test_408_request_timeout() -> None:
    ...
    p = ConnectionPair()
    p.send(
        SERVER, Response(status_code=408, headers=[(b'connection', b'close')])
    )
