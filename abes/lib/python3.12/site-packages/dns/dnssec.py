...
class SimpleDeny(Policy):
    def __init__(
        self, deny_sign, deny_validate, deny_create_ds, deny_validate_ds
    ):
        super().__init__()
...
def make_ds(
    ...
    *validating*, a ``bool``.  If ``True``, then policy is checked in
    ...
    dsrdata = (
        struct.pack('!HBB', key_id(key), key.algorithm, algorithm) + digest
    )
...
def _find_candidate_keys(
    keys: Dict[dns.name.Name, Union[dns.rdataset.Rdataset, dns.node.Node]],
    rrsig: RRSIG,
...
def _validate(
    rrset: Union[dns.rrset.RRset, Tuple[dns.name.Name, dns.rdataset.Rdataset]],
    rrsigset: Union[
        dns.rrset.RRset, Tuple[dns.name.Name, dns.rdataset.Rdataset]
    ],
    keys: Dict[dns.name.Name, Union[dns.node.Node, dns.rdataset.Rdataset]],
    origin: Optional[dns.name.Name] = None,
    now: Optional[float] = None,
    ...
    if not signer.is_absolute():
        if origin is None:
            raise ValidationFailure(
                'relative RR name without an origin specified'
            )
...
def _make_dnskey(
    ...
    *protocol*: DNSKEY protocol field as an integer.
    ...
    if isinstance(public_key, GenericPublicKey):
        ...
        return public_cls(key=public_key).to_dnskey(
            flags=flags, protocol=protocol
        )
    ...
    for algorithm in _algorithms:
        res.extend(
            dnskey_rdataset_to_cds_rdataset(
                rrname, rdataset, algorithm, origin
            )
        )
    return dns.rdataset.from_rdata_list(rdataset.ttl, res)
...
def _sign_zone_nsec(
    ...
    for name in sorted(txn.iterate_names()):
        ...
        if last_secure is not None:
            _txn_add_nsec(
                txn, last_secure, name, zone.rdclass, rrsig_ttl, rrset_signer
            )
        last_secure = name
