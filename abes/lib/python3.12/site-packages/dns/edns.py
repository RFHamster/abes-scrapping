...
class Option:
    ...
    def from_wire_parser(
        cls, otype: OptionType, parser: 'dns.wire.Parser'
    ) -> 'Option':
...
class ECSOption(Option):  # lgtm[py/missing-equals]
    ...
    def __init__(
        self, address: str, srclen: Optional[int] = None, scopelen: int = 0
    ):
        ...
        *scopelen*, an ``int``, the scope prefix length.  This value
        ...
        if nbits != 0:
            last = struct.pack(
                'B', ord(self.addrdata[-1:]) & (0xFF << (8 - nbits))
            )
    ...
    def from_text(text: str) -> Option:
        ...
        *text*, a `str`, the text form of the option.
        ...
        try:
            ...
            raise ValueError(
                'invalid scope ' + f'"{tscope}": scope must be an integer'
            )
        try:
            srclen = int(tsrclen)
        except ValueError:
    ...
    def to_wire(self, file: Optional[Any] = None) -> Optional[bytes]:
        value = (
            struct.pack('!HBB', self.family, self.srclen, self.scopelen)
            + self.addrdata
        )
        if file:
            file.write(value)
...
class CookieOption(Option):
    def __init__(self, client: bytes, server: bytes):
        ...
        if len(server) != 0 and (len(server) < 8 or len(server) > 32):
            raise ValueError(
                'server cookie must be empty or between 8 and 32 bytes'
            )
