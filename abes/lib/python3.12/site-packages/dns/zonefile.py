...
def _check_cname_and_other_data(txn, name, rdataset):
    ...
    if (
        ...
        raise CNAMEAndOtherData(
            'rdataset type is not compatible with a CNAME node'
        )
    elif (
...
class Reader:
    ...
    def __init__(
        ...
        if allow_directives is True:
            ...
            # is ignored.
            self.allowed_directives = set(
                _upper_dollarize(d) for d in allow_directives
            )
    ...
    def _rr_line(self):
        ...
        if self.force_rdtype is not None:
            ...
            try:
                ...
                raise dns.exception.SyntaxError(
                    f"unknown rdatatype '{token.value}'"
                )
    ...
    def _generate_line(self):
        ...
        def _calculate_index(
            counter: int, offset_sign: str, offset: int
        ) -> int:
        ...
        try:
            while 1:
                ...
                if token.is_eof():
                    ...
                    token.value[0] == '$' and len(self.allowed_directives) > 0
                ):
...
def read_rrsets(
    ...
    rdclass: Optional[
        Union[dns.rdataclass.RdataClass, str]
    ] = dns.rdataclass.IN,
