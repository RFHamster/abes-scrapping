...
class AsyncioQuicConnection(AsyncQuicConnection):
    def __init__(
        self, connection, address, port, source, source_port, manager=None
    ):
        super().__init__(
            connection, address, port, source, source_port, manager
        )
        self._socket = None
        ...
        try:
            ...
            async with self._socket:
                while not self._done:
                    ...
                    if (
                        address[0] != self._peer[0]
                        or address[1] != self._peer[1]
                    ):
                        continue
                    self._connection.receive_datagram(
                        datagram, address, time.time()
                    )
            ...
            if isinstance(event, aioquic.quic.events.StreamDataReceived):
                if self.is_h3():
                    ...
                    for h3_event in h3_events:
                        if isinstance(
                            h3_event, aioquic.h3.events.HeadersReceived
                        ):
                            ...
                            h3_event, aioquic.h3.events.DataReceived
                        ):
                            stream = self._streams.get(event.stream_id)
                            if stream:
...
class AsyncioQuicManager(AsyncQuicManager):
    def __init__(
        ...
        super().__init__(
            conf, verify_mode, AsyncioQuicConnection, server_name, h3
        )
