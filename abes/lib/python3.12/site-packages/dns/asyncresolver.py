...
class Resolver(dns.resolver.BaseResolver):
        ...
        *family*, an ``int``, the address family.  If socket.AF_UNSPEC
        ...
        if family == socket.AF_INET:
            ...
            v6 = await self.resolve(
                name, dns.rdatatype.AAAA, **modified_kwargs
            )
    ...
    async def canonical_name(
        self, name: Union[dns.name.Name, str]
    ) -> dns.name.Name:
    ...
    async def try_ddr(self, lifetime: float = 5.0) -> None:
        ...
        *lifetime*, a float, is the maximum time to spend attempting DDR.  The default
        ...
        try:
            ...
            nameservers = await dns._ddr._get_nameservers_async(
                answer, timeout
            )
...
async def resolve_address(
    ...
    return await get_default_resolver().resolve_address(
        ipaddr, *args, **kwargs
    )
