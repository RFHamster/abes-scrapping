...
class Zone(dns.transaction.TransactionManager):
    ...
    def __init__(
        ...
        *relativize*, a ``bool``, determine's whether domain names are
        ...
        self.nodes: MutableMapping[
            dns.name.Name, dns.node.Node
        ] = self.map_factory()
    ...
    def find_rrset(
        ...
        *create*, a ``bool``.  If true, the node will be created if it does
        ...
        rdtype = dns.rdatatype.RdataType.make(rdtype)
        covers = dns.rdatatype.RdataType.make(covers)
        rdataset = self.nodes[vname].find_rdataset(
            self.rdclass, rdtype, covers
        )
    ...
    def to_text(
        ...
        *want_origin*, a ``bool``.  If ``True``, emit a $ORIGIN line at
        ...
        self.to_file(
            temp_buffer, sorted, relativize, nl, want_comments, want_origin
        )
    ...
    def _compute_digest(
        ...
        for name, node in sorted(self.items()):
            ...
            for rdataset in sorted(
                node, key=lambda rds: (rds.rdtype, rds.covers)
            ):
                ...
                rdatas = [
                    rdata.to_digestable(self.origin) for rdata in rdataset
                ]
    ...
    def verify_digest(
        ...
        digests: Union[
            dns.rdataset.Rdataset, List[dns.rdtypes.ANY.ZONEMD.ZONEMD]
        ]
        if zonemd:
            digests = [zonemd]
        else:
        ...
        for digest in digests:
            try:
                computed = self._compute_digest(
                    digest.hash_algorithm, digest.scheme
                )
    ...
    def reader(self) -> 'Transaction':
        return Transaction(
            self, False, Version(self, 1, self.nodes, self.origin)
        )
    ...
    *check_origin*, a ``bool``.  If ``True``, the default, then sanity
    ...
    for r in xfr:
        ...
        for rrset in r.answer:
            ...
            zrds = znode.find_rdataset(
                rrset.rdclass, rrset.rdtype, rrset.covers, True
            )
