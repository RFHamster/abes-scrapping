...
class _Resolution:
    ...
    def __init__(
        ...
        self.nxdomain_responses: Dict[
            dns.name.Name, dns.message.QueryMessage
        ] = {}
    ...
    def next_request(
        ...
        while len(self.qnames) > 0:
            ...
            if self.resolver.cache:
                ...
                if (
                    answer is not None
                    and answer.response.rcode() == dns.rcode.NXDOMAIN
                ):
        ...
        #
        raise NXDOMAIN(
            qnames=self.qnames_to_try, responses=self.nxdomain_responses
        )
        ...
        if rcode == dns.rcode.NOERROR:
            ...
            if self.resolver.cache:
                self.resolver.cache.put(
                    (self.qname, self.rdtype, self.rdclass), answer
                )
            if answer.rrset is None and self.raise_on_no_answer:
                raise NoAnswer(response=answer.response)
            return (answer, True)
...
class BaseResolver:
    ...
    def reset(self) -> None:
        ...
        self.domain = dns.name.Name(
            dns.name.from_text(socket.gethostname())[1:]
        )
        ...
        if isinstance(nameservers, list):
            for nameserver in nameservers:
                ...
                if isinstance(nameserver, dns.nameserver.Nameserver):
                    ...
                    enriched_nameserver = dns.nameserver.DoHNameserver(
                        nameserver
                    )
        ...
        while True:
            ...
            while not done:
                ...
                timeout = self._compute_timeout(
                    start, lifetime, resolution.errors
                )
                try:
        ...
        try:
            ...
            if response:
                for rrs in response.authority:
                    if (
                        rrs.rdtype == dns.rdatatype.SOA
                        and rrs.rdclass == rdclass
                    ):
...
def _getaddrinfo(
    ...
    if flags & socket.AI_NUMERICHOST != 0:
        ...
        return _original_getaddrinfo(
            host, service, family, socktype, proto, flags
        )
...
def _getnameinfo(sockaddr, flags=0):
    ...
    tuples = _getaddrinfo(
        host, port, family, socket.SOCK_STREAM, socket.SOL_TCP, 0
    )
    if len(tuples) > 1:
