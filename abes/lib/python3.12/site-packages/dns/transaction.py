...
def _ensure_immutable_rdataset(rdataset):
    if rdataset is None or isinstance(
        rdataset, dns.rdataset.ImmutableRdataset
    ):
        return rdataset
...
class Transaction:
    ...
    def update_serial(
        ...
        *value*, an `int`, is an increment if *relative* is `True`, or the
        ...
        if isinstance(name, str):
            name = dns.name.from_text(name, None)
        rdataset = self._get_rdataset(
            name, dns.rdatatype.SOA, dns.rdatatype.NONE
        )
    ...
    def _delete(self, exact, args):
        try:
            ...
            if rdataset:
                if rdataset.rdclass != self.manager.get_class():
                    raise ValueError(
                        f'{method} has objects of wrong RdataClass'
                    )
                existing = self._get_rdataset(
                    name, rdataset.rdtype, rdataset.covers
                )
