    ...
    class _NetworkBackend(_CoreNetworkBackend):
        ...
        def connect_tcp(
            ...
            for address in addresses:
                ...
                attempt_expiration = _expiration_for_this_attempt(
                    2.0, expiration
                )
                try:
                    _connect(
...
def _matches_destination(af, from_address, destination, ignore_unexpected):
    ...
    if _addresses_equal(af, from_address, destination) or (
        dns.inet.is_multicast(destination[0])
        and from_address[1:] == destination[1:]
    ):
        return True
...
def _destination_and_source(
    ...
    if source_port and not source:
        ...
        try:
            ...
            raise ValueError(
                'source_port specified but address family is unknown'
            )
...
def quic(
    ...
    *server_hostname*, a ``str`` or ``None``.  This item is for backwards compatibility
    ...
    if connection:
        manager: contextlib.AbstractContextManager = contextlib.nullcontext(
            None
        )
        the_connection = connection
    else:
        manager = dns.quic.SyncQuicManager(
            verify_mode=verify, server_name=hostname
        )
        the_manager = manager  # for type checking happiness

    with manager:
        if not connection:
            the_connection = the_manager.connect(
                where, port, source, source_port
            )
        (start, expiration) = _compute_times(timeout)
    ...
    class DummyTransactionManager(dns.transaction.TransactionManager):
        def __init__(self, origin, relativize):
            self.info = (
                origin,
                relativize,
                dns.name.empty if relativize else origin,
            )
...
def inbound_xfr(
    ...
    *udp_mode*, a ``dns.query.UDPMode``, determines how UDP is used
    ...
    (_, expiration) = _compute_times(lifetime)
    if (
        query.question[0].rdtype == dns.rdatatype.IXFR
        and udp_mode != UDPMode.NEVER
    ):
