...
class Message:
    ...
    def is_response(self, other: 'Message') -> bool:
        ...
        if (
            ...
            or self.id != other.id
            or dns.opcode.from_flags(self.flags)
    ...
    def _make_opt(flags=0, payload=DEFAULT_EDNS_PAYLOAD, options=None):
        opt = dns.rdtypes.ANY.OPT.OPT(
            payload, dns.rdatatype.OPT, options or ()
        )
    ...
    def extended_errors(self) -> List[dns.edns.EDEOption]:
        ...
        return cast(
            List[dns.edns.EDEOption], self.get_options(dns.edns.OptionType.EDE)
        )
            ...
            try:
                ...
                if rdtype == dns.rdatatype.OPT:
                    ...
                    if self.keyring is None or self.keyring is True:
                        raise UnknownTSIGKey(
                            'got signed message without keyring'
                        )
                    elif isinstance(self.keyring, dict):
                        key = self.keyring.get(absolute_name)
                        if isinstance(key, bytes):
                            key = dns.tsig.Key(
                                absolute_name, key, rd.algorithm
                            )
                    ...
                    self.message.tsig = dns.rrset.from_rdata(
                        absolute_name, 0, rd
                    )
                else:
        ...
        if what == 'id':
            ...
            while True:
                ...
                if not token.is_identifier():
                    self.tok.unget(token)
                    break
                self.ednsflags = self.ednsflags | dns.flags.edns_from_text(
                    token.value
                )
        elif what == 'payload':
...
def make_query(
    ...
    *pad*, a non-negative ``int``.  If 0, the default, do not pad; otherwise add
    ...
    m.find_rrset(
        m.question, qname, rdclass, rdtype, create=True, force_unique=True
    )
