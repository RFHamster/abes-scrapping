...
class Renderer:
    ...
    def add_rdataset(self, section, name, rdataset, **kw):
        ...
        with self._track_size():
            n = rdataset.to_wire(
                name, self.output, self.compress, self.origin, **kw
            )
        self.counts[section] += n
    ...
    def add_opt(self, opt, pad=0, opt_size=0, tsig_size=0):
        ...
        if pad:
            ...
            options.append(
                dns.edns.GenericOption(dns.edns.OptionType.PADDING, pad)
            )
            opt = dns.message.Message._make_opt(
    ...
    def add_tsig(
        ...
        (tsig, _) = dns.tsig.sign(
            s, key, tsig[0], int(time.time()), request_mac
        )
